...
public partial class RelativePerformanceSettingsViewModel : BaseChartSettingsViewModel
{
    ...
    [RelayCommand]
    private void AppendPreset(RelativePerformancePreset? preset)
    {
        if (preset == null || preset.Targets == null)
            return;
        ...
        foreach (var target in preset.Targets)
        {
            if (SymbolList.Count >= WatchlistConstants.MaxRelativePerformanceTargets)
                break;

            if (!string.IsNullOrWhiteSpace(target))
            {
                var symbol = target.Trim().ToUpperInvariant();
                if (!SymbolList.Contains(symbol))
                {
                    SymbolList.Add(symbol);
                    addedAny = true;
