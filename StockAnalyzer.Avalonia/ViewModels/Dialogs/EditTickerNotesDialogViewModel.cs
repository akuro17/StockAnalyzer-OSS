...
public partial class EditTickerNotesDialogViewModel : ViewModelBase, IDisposable
{
    ...
    public EditTickerNotesDialogViewModel(
        ...
        : this(ticker, entryPrice, targetPrice, stopLoss, null, null, null, notes,
    ...
    private void ReloadForSymbol(string symbol)
    {
        ...
        if (strategy != null)
        {
            ...
            OnPropertyChanged(nameof(StopLossShortText));
            _notes = strategy.Notes ?? string.Empty;
            OnPropertyChanged(nameof(Notes));
    ...
    {
        // Save to Symbol (the currently displayed ticker), which may differ from Ticker
        // when the user has changed the symbol via the Symbol input.
    ...
    {
        PersistStrategy();
        OnApplyCallback?.Invoke(Long, ExitLong, StopLossLong, Short, ExitShort, StopLossShort, Notes);
