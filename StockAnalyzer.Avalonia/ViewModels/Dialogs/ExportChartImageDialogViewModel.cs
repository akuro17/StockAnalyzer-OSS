...
public partial class ExportChartImageDialogViewModel : ViewModelBase, IDisposable
{
    ...
    private readonly LayeredDrawingScene? _layeredScene;
    ...
    /// <summary>
    /// Renders preview (<paramref name="previewMaxWidth"/> non-null) or full-size bytes. Uses the layered export
    /// path when a scene was captured and the service supports it; otherwise the legacy path.
    /// </summary>
    private Task<byte[]> RenderExportBytesAsync(ChartImageExportOptions options, ChartImageMetadata metadata, int? previewMaxWidth)
    {
        if (_layeredScene != null && _exportService is ILayeredChartImageExportService layered)
        {
            return previewMaxWidth.HasValue
                ? layered.RenderLayeredChartPreviewAsync(
                    _snapshot, _layout, _transform, options, _themeManager,
                    _objectManager, _mainRenderer, _renderConfig, _chartType, _rulerRenderer, metadata, _layeredScene, previewMaxWidth.Value)
                : layered.RenderLayeredChartImageAsync(
                    _snapshot, _layout, _transform, options, _themeManager,
                    _objectManager, _mainRenderer, _renderConfig, _chartType, _rulerRenderer, metadata, _layeredScene);
        }

        return previewMaxWidth.HasValue
            ? _exportService.RenderChartPreviewAsync(
                _snapshot, _layout, _transform, options, _themeManager,
                _objectManager, _mainRenderer, _renderConfig, _chartType, _rulerRenderer, metadata, previewMaxWidth.Value)
            : _exportService.RenderChartImageAsync(
                _snapshot, _layout, _transform, options, _themeManager,
    ...
    private async Task RequestPreviewUpdateAsync()
    {
        ...
        try
        {
            ...
            var previewBytes = await RenderExportBytesAsync(options, metadata, PreviewMaxWidth);
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            var imageBytes = await RenderExportBytesAsync(options, metadata, null);
