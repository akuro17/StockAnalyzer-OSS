...
/// <c>fold_baseline_accuracy</c>, <c>fold_multi_logloss</c>, <c>fold_is_holdout</c>). The
/// numbers come from an inference-only pass of the exported ONNX over each walk-forward
/// fold's validation block, so they describe the final model's behavior across time slices -
/// not a re-trained CV. <c>fold_is_holdout</c> (see <see cref="IsHoldout"/>) marks the one
/// fold whose validation block is the model's own held-out split; every other fold is scored
/// with that same model over rows it was trained on, so it is a reference score, not an
/// independent cross-validation result.
/// </summary>
