...
public partial class ColumnItemViewModel : ViewModelBase
{
    ...
    private static readonly Dictionary<string, ColumnStaticInfo> InfoMap = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        { "Change", new ColumnStaticInfo(ColumnCategory.PriceVolume, "Change", "Price change for the current trading day", "Close - Open") },
        { "ChangePercent", new ColumnStaticInfo(ColumnCategory.PriceVolume, "Change %", "Percentage price change for the current trading day", "((Close - Open) / Open) * 100") },
        ...
        { "LastUpdatedUtc", new ColumnStaticInfo(ColumnCategory.Basic, "TS Updated", "Timestamp of the last time-series data synchronization", "Time-Series Retrieval Timestamp (UTC)") },
        { "MetadataLastUpdated", new ColumnStaticInfo(ColumnCategory.Basic, "Meta Updated", "Timestamp of the last metadata synchronization (excludes time series update time)", "Metadata Retrieval Timestamp (UTC)") },
