...
public class AboutSettingsViewModel : ViewModelBase, ISettingsPageViewModel
{
    ...
    /// </summary>
    internal Task EnvironmentStatusLoadTask => _refreshTask;
    ...
    public AboutSettingsViewModel(IPythonEnvironmentInspector inspector)
    {
        ...
        _refreshTask = LoadEnvironmentStatusAsync();
    }
    ...
    /// <summary>
    /// Re-probes the interpreter and refreshes the lines in place (the previous lines stay visible until the probe returns).
    /// The settings dialog caches this page, so packages installed from another page (e.g. the ONNX training tools) would
    /// otherwise stay invisible until the dialog is reopened. A probe already in flight is reused rather than duplicated.
    /// </summary>
    public Task RefreshEnvironmentStatusAsync()
    {
