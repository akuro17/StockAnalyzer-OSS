...
public partial class DrawingObjectItemViewModel : ViewModelBase
{
    ...
    private readonly Func<Guid, bool>? _canLinkAsParent;
    private readonly Action<Guid>? _linkAsParent;
    private readonly Func<Guid, bool>? _canUnlink;
    private readonly Action<Guid>? _unlink;
    private readonly Func<Guid, LinkRole>? _getLinkRole;
    ...
    public DrawingObjectItemViewModel(
        ...
        Func<Guid, bool>? canLinkAsParent = null,
        Action<Guid>? linkAsParent = null,
        Func<Guid, bool>? canUnlink = null,
        Action<Guid>? unlink = null,
        Func<Guid, LinkRole>? getLinkRole = null)
    {
        ...
        _canLinkAsParent = canLinkAsParent;
        _linkAsParent = linkAsParent;
        _canUnlink = canUnlink;
        _unlink = unlink;
        _getLinkRole = getLinkRole;
    }

    /// <summary>Role of this object in its link group (None when not linked).</summary>
    public LinkRole LinkRole => _getLinkRole?.Invoke(Model.Id) ?? LinkRole.None;

    public bool IsLinked => LinkRole != LinkRole.None;
    public bool IsLinkParent => LinkRole == LinkRole.Parent;
    public bool IsLinkChild => LinkRole == LinkRole.Child;

    /// <summary>True when "link as parent" would newly link at least one checked object under this one.</summary>
    public bool CanLinkAsParent => _canLinkAsParent?.Invoke(Model.Id) == true;

    /// <summary>True when this object or any checked object is linked (nothing to unlink otherwise).</summary>
    public bool CanUnlink => _canUnlink?.Invoke(Model.Id) == true;

    /// <summary>Re-evaluates link availability and badges; called whenever targeting or link state may have changed.</summary>
    public void RefreshLinkState()
    {
        OnPropertyChanged(nameof(LinkRole));
        OnPropertyChanged(nameof(IsLinked));
        OnPropertyChanged(nameof(IsLinkParent));
        OnPropertyChanged(nameof(IsLinkChild));
        OnPropertyChanged(nameof(CanLinkAsParent));
        OnPropertyChanged(nameof(CanUnlink));
        LinkAsParentCommand.NotifyCanExecuteChanged();
        UnlinkCommand.NotifyCanExecuteChanged();
    }

    [RelayCommand(CanExecute = nameof(CanLinkAsParent))]
    private void LinkAsParent()
    {
        if (!EffectiveEditAllowed) return;
        _linkAsParent?.Invoke(Model.Id);
    }

    [RelayCommand(CanExecute = nameof(CanUnlink))]
    private void Unlink()
    {
        _unlink?.Invoke(Model.Id);
    }
