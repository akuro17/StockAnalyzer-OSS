    ...
    public partial class WatchlistItemViewModel : ObservableObject, IFilterableSymbol
    {
        ...
        private IComparable? GetRawValueReflectionFallback(string memberName)
        {
            ...
            try
            {
                ...
                {
                    string cleanedTarget = key.Replace(" ", "").Replace("-", "").Replace("_", "").Replace("/", "").Replace("%", "").Replace("(TTM)", "").Replace("Ratio", "").ToLowerInvariant();
                    var props = typeof(WatchlistItemViewModel).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                    foreach (var p in props)
                    {
                        string cleanedProp = p.Name.Replace(" ", "").Replace("-", "").Replace("_", "").Replace("/", "").Replace("%", "").Replace("(TTM)", "").Replace("Ratio", "").ToLowerInvariant();
                        if (cleanedProp == cleanedTarget || p.Name.Equals(key, StringComparison.OrdinalIgnoreCase))
                        {
                            return p;
                        }
                    }
