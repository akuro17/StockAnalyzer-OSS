...
/// <summary>
/// Top-level node representing a user-defined custom parent category, synced into
/// <see cref="TickerListViewModel.Groups"/> alongside All Ticker/Watchlist/Portfolio (sa_improve: the
/// separate left-column category ListBox was removed in favor of selecting custom categories directly
/// from the same TreeView). A pure grouping container - never holds tickers directly. Its
/// <see cref="TickerGroupNode.Children"/> are <see cref="TickerListFolderNode"/> entries (the actual
/// ticker-holding leaves); <see cref="TickerListViewModel.GetTickersForNode"/> aggregates their
/// members when this node is selected, mirroring how the fixed Watchlists/Portfolios
/// <see cref="CategoryNode"/> aggregates its own children.
/// </summary>
public sealed partial class CustomCategoryNode : TickerGroupNode
{
    public StockAnalyzer.Core.Models.Settings.TickerParentCategorySettings Settings { get; private set; }

    public CustomCategoryNode(StockAnalyzer.Core.Models.Settings.TickerParentCategorySettings settings)
        : base(settings.Id, settings.Name)
    {
        Settings = settings;
        Children = new ObservableCollection<TickerGroupNode>();
    }

    public void UpdateSettings(StockAnalyzer.Core.Models.Settings.TickerParentCategorySettings settings)
    {
        Settings = settings;
        DisplayName = settings.Name;
    }
...
/// <summary>
/// Leaf node representing a user-defined list folder nested under a <see cref="CustomCategoryNode"/>
/// parent category ("Create New List"). The actual ticker-holding entity in the custom-category
/// hierarchy - structurally parallel to <see cref="WatchlistNode"/>/<see cref="PortfolioNode"/>.
/// <see cref="TickerListViewModel.GetTickersForNode"/> resolves its members via
/// <see cref="Settings"/>'s <see cref="StockAnalyzer.Core.Models.Settings.TickerListFolderSettings.ChildTickers"/>.
/// </summary>
public sealed partial class TickerListFolderNode : TickerGroupNode
{
    public StockAnalyzer.Core.Models.Settings.TickerListFolderSettings Settings { get; private set; }

    public TickerListFolderNode(StockAnalyzer.Core.Models.Settings.TickerListFolderSettings settings)
        : base(settings.Id, settings.Name)
    {
        Settings = settings;
        Children = new ObservableCollection<TickerGroupNode>();
    ...
    public void UpdateSettings(StockAnalyzer.Core.Models.Settings.TickerListFolderSettings settings)
    {
        Settings = settings;
        DisplayName = settings.Name;
    }
