...
public partial class DataWindowViewModel : ViewModelBase,
    ...
    IRecipient<ConfluenceSnapshotUpdatedMessage>
{
    ...
    private void ChartViewModel_PropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ...
        {
            _dispatcherService.Post(static vm => vm.OnPropertyChanged(nameof(Symbol)), this);
        }
    }
    ...
    public DataWindowViewModel(ChartViewModel chartViewModel, IMessenger messenger, IDispatcherService dispatcherService, ILocalizationService? localizationService)
    {
        ...
        _messenger.Register<DataWindowViewModel, ConfluenceSnapshotUpdatedMessage>(this, (r, m) => r.Receive(m));
    ...
    public void Receive(ConfluenceSnapshotUpdatedMessage message)
    {
        _dispatcherService.Post(static vm => {
            vm.InvalidateCache();
            if (vm._lastCrosshairData != null)
            {
                vm.Receive(new CrosshairPositionChangedMessage(vm._lastCrosshairData));
            }
        }, this);
    }
    ...
    public void Receive(CrosshairPositionChangedMessage message)
    {
        _dispatcherService.Post(static state => {
            ...
            {
                var conf = data.Confluence.Value;
                string dirStr = conf.FinalDirection switch
                {
                ...
                vm.ConfluenceScoreText = $"{conf.Score}% {dirStr}";
                vm.ConfluenceColor = conf.FinalDirection switch
                {
                    SignalDirection.Bullish => new IndicatorColor(255, 50, 205, 50),   // LimeGreen
                    SignalDirection.Bearish => new IndicatorColor(255, 255, 99, 71),   // Tomato
                    _ => new IndicatorColor(255, 128, 128, 128)                       // Gray
                };
                vm.IsConfluenceVisible = true;
            }
            else
            {
                vm.IsConfluenceVisible = false;
                ...
                if (hasResult && result != null)
                {
                    ...
                    if (seriesNames != null)
                    {
                        foreach (var n in seriesNames)
                        {
                            ...
                            if (n == "Main" && seriesNames.Any(other => other != "Main" && !other.Contains("Signal") && !other.Contains("Histogram") && !other.Contains("Trend")))
                                continue;
            ...
            foreach (var (indicator, result, validSeries, indicatorBaseName) in independentIndicators)
            {
                ...
                if (isFolder)
                {
                    ...
                    foreach (var seriesName in validSeries)
                    {
                        ...
                        var childItem = UpdateOrAddChildItem(parentItem, childIndex++, label, val, parentColor, false); // useCustomColor = false
