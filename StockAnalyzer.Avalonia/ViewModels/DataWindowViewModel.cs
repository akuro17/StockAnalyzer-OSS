...
public enum DataWindowCategory
{
    MarketData,
    TechnicalIndicators,
    DrawingTools
...
public sealed record DataWindowCategoryItem(DataWindowCategory Category, string DisplayName);
    ...
    IRecipient<IndicatorResultsUpdatedMessage>
{
    ...
    private readonly List<DataWindowCategoryItem> _availableCategoriesList = new();
    public IReadOnlyList<DataWindowCategoryItem> AvailableCategories => _availableCategoriesList;

    [ObservableProperty]
    private DataWindowCategoryItem? _selectedCategory;

    public bool IsMarketDataSelected => SelectedCategory?.Category == DataWindowCategory.MarketData;
    public bool IsIndicatorsSelected => SelectedCategory?.Category == DataWindowCategory.TechnicalIndicators;
    public bool IsDrawingToolsSelected => SelectedCategory?.Category == DataWindowCategory.DrawingTools;

    partial void OnSelectedCategoryChanged(DataWindowCategoryItem? value)
    {
        if (value == null && _availableCategoriesList.Count > 0)
        {
            SelectedCategory = _availableCategoriesList[0];
            return;
        }

        OnPropertyChanged(nameof(IsMarketDataSelected));
        OnPropertyChanged(nameof(IsIndicatorsSelected));
        OnPropertyChanged(nameof(IsDrawingToolsSelected));
    }

    [RelayCommand]
    private void NavigatePreviousCategory()
    {
        if (_availableCategoriesList.Count == 0) return;
        int currentIndex = SelectedCategory != null ? _availableCategoriesList.IndexOf(SelectedCategory) : 0;
        if (currentIndex < 0) currentIndex = 0;
        int newIndex = (currentIndex - 1 + _availableCategoriesList.Count) % _availableCategoriesList.Count;
        SelectedCategory = _availableCategoriesList[newIndex];
    }

    [RelayCommand]
    private void NavigateNextCategory()
    {
        if (_availableCategoriesList.Count == 0) return;
        int currentIndex = SelectedCategory != null ? _availableCategoriesList.IndexOf(SelectedCategory) : 0;
        if (currentIndex < 0) currentIndex = 0;
        int newIndex = (currentIndex + 1) % _availableCategoriesList.Count;
        SelectedCategory = _availableCategoriesList[newIndex];
    }
    ...
    public DataWindowViewModel(ChartViewModel chartViewModel, IMessenger messenger, IDispatcherService dispatcherService, ILocalizationService? localizationService)
    {
        ...
        // A missing key resolves to "[key]" (LocalizationManager.Get), not the bare key, so the
        // "unresolved" test must compare against that bracketed sentinel before falling back.
    ...
    {
        _availableCategoriesList.Clear();
        _availableCategoriesList.Add(new DataWindowCategoryItem(DataWindowCategory.MarketData, GetLocalizedCategoryTitle("DataWindow_Category_MarketData", "Market Data")));
        _availableCategoriesList.Add(new DataWindowCategoryItem(DataWindowCategory.TechnicalIndicators, GetLocalizedCategoryTitle("DataWindow_Category_Indicators", "Technical Indicators")));
        _availableCategoriesList.Add(new DataWindowCategoryItem(DataWindowCategory.DrawingTools, GetLocalizedCategoryTitle("DataWindow_Category_Drawings", "Drawing Tools")));
        _selectedCategory = _availableCategoriesList[0];
    }

    private string GetLocalizedCategoryTitle(string key, string fallback)
    {
        var localized = _localizationService.GetString(key);
        if (string.IsNullOrWhiteSpace(localized) || (localized.StartsWith('[') && localized.EndsWith(']')))
        {
            return fallback;
        }
        return localized;
    ...
    private abstract class IndicatorDisplayEntry
    {
        public bool IsOverlay { get; init; }
        public string? OverlayPanelId { get; init; }
        public abstract string SortKey { get; }
        public abstract void Render(DataWindowViewModel vm, ref int itemIndex, int lookupIndex);
    }

    private sealed class IndependentIndicatorEntry : IndicatorDisplayEntry
    {
        public CoreIndicatorSettings Settings { get; init; } = null!;
        public IIndicatorResult? Result { get; init; }
        public List<string> ValidSeries { get; init; } = new();
        public string BaseName { get; init; } = string.Empty;

        public override string SortKey => string.IsNullOrEmpty(Settings.ShortDisplayName)
            ? (string.IsNullOrEmpty(Settings.DisplayName) ? BaseName : Settings.DisplayName)
            : Settings.ShortDisplayName;

        public override void Render(DataWindowViewModel vm, ref int itemIndex, int lookupIndex)
        {
            string indicatorName = Settings.ShortDisplayName;
            if (string.IsNullOrEmpty(indicatorName)) indicatorName = Settings.DisplayName;

            bool isFolder = (Settings.SeriesColors?.Count ?? 0) > 1 || ValidSeries.Count > 1;

            if (isFolder)
            {
                var parentColor = Settings.Color;
                var parentItem = vm.UpdateOrAddItem(itemIndex++, indicatorName, string.Empty, parentColor, true, false);

                parentItem.IsEnabled = Settings.IsEnabled;
                parentItem.AssociateId = Settings.Id;
                ...
                foreach (var seriesName in ValidSeries)
                {
                    ...
                    var colorConfig = Settings.SeriesColors?.FirstOrDefault(c => c.TargetSeries.Contains(seriesName));
                    if (colorConfig != null)
                    {
                    ...
                    if (Result != null)
                    {
                        ...
                        if (string.IsNullOrEmpty(label))
                        {
                            label = Result.SeriesLabels.TryGetValue(seriesName, out var cachedLabel)
                                ? cachedLabel
                                : (seriesName == "Main" ? BaseName : seriesName);
                        }
                    }
                    else if (string.IsNullOrEmpty(label))
                    {
                        label = seriesName == "Main" ? BaseName : seriesName;
                    }
                    ...
                    var childItem = vm.UpdateOrAddChildItem(parentItem, childIndex++, label, val, childColor, false);
                    childItem.IsEnabled = Settings.IsEnabled;
                    childItem.AssociateId = Settings.Id;
                    childItem.ShowCheckbox = false;
                    childItem.OpenSettingsCommand = vm.OpenSettingsCommand;
                }
                while (parentItem.Children.Count > childIndex)
                {
                    var removed = parentItem.Children[parentItem.Children.Count - 1];
                    removed.PropertyChanged -= vm.Item_PropertyChanged;
            ...
            else
            {
                ...
                var item = vm.UpdateOrAddItem(itemIndex++, indicatorName, val.HasValue ? val.Value.ToString("0.000") : "N/A", c, false, false);
                item.OpenSettingsCommand = vm.OpenSettingsCommand;

                item.IsEnabled = Settings.IsEnabled;
                item.AssociateId = Settings.Id;
            }
        }
    }

    private sealed class GroupedIndicatorEntry : IndicatorDisplayEntry
    {
        public string TypeName { get; init; } = string.Empty;
        public List<(CoreIndicatorSettings Settings, IIndicatorResult Result, List<string> ValidSeries)> Items { get; init; } = new();

        public override string SortKey => TypeName;

        public override void Render(DataWindowViewModel vm, ref int itemIndex, int lookupIndex)
        {
            if (Items.Count == 0) return;

            // Sort indicators inside group alphabetically by ShortDisplayName / DisplayName
            Items.Sort(s_groupedItemComparison);

            var firstInd = Items[0].Settings;
            var parentColor = firstInd.Color;
            var parentItem = vm.UpdateOrAddItem(itemIndex++, TypeName, string.Empty, parentColor, true, false);

            parentItem.IsEnabled = Items.Any(g => g.Settings.IsEnabled);
            parentItem.AssociateId = TypeName;

            int childIndex = 0;
            foreach (var (indicator, result, validSeries) in Items)
            {
                decimal? val = null;
                ...
                var childItem = vm.UpdateOrAddChildItem(parentItem, childIndex++, displayLabel, val, c, false);

                childItem.IsEnabled = indicator.IsEnabled;
                childItem.AssociateId = indicator.Id;
                childItem.ShowCheckbox = true;
            }
            while (parentItem.Children.Count > childIndex)
            {
                var removed = parentItem.Children[parentItem.Children.Count - 1];
    ...
    private readonly List<IndicatorDisplayEntry> _indicatorEntriesBuffer = new();
    private readonly Dictionary<(bool IsOverlay, string PanelId, string BaseName), GroupedIndicatorEntry> _groupedEntriesMapBuffer = new();

    private static readonly Comparison<IndicatorDisplayEntry> s_indicatorEntryComparison = static (a, b) =>
    {
        // Tier 1: Main window first
        if (a.IsOverlay != b.IsOverlay)
        {
            return a.IsOverlay ? -1 : 1;
        }

        // Tier 2: Sub-window panel order
        if (!a.IsOverlay)
        {
            bool aDef = string.IsNullOrWhiteSpace(a.OverlayPanelId);
            bool bDef = string.IsNullOrWhiteSpace(b.OverlayPanelId);
            if (aDef != bDef)
            {
                return aDef ? -1 : 1;
            }
            if (!aDef && !bDef)
            {
                int pCmp = string.Compare(a.OverlayPanelId, b.OverlayPanelId, StringComparison.OrdinalIgnoreCase);
                if (pCmp != 0) return pCmp;
            }
        }

        // Tier 3: Indicator name ascending
        return string.Compare(a.SortKey, b.SortKey, StringComparison.OrdinalIgnoreCase);
    };

    private static readonly Comparison<(CoreIndicatorSettings Settings, IIndicatorResult Result, List<string> ValidSeries)> s_groupedItemComparison = static (x, y) =>
    {
        string nameX = string.IsNullOrEmpty(x.Settings.ShortDisplayName) ? x.Settings.DisplayName : x.Settings.ShortDisplayName;
        string nameY = string.IsNullOrEmpty(y.Settings.ShortDisplayName) ? y.Settings.DisplayName : y.Settings.ShortDisplayName;
    ...
    private void UpdateIndicatorItems(IEnumerable<CoreIndicatorSettings>? settings, IReadOnlyDictionary<string, IIndicatorResult>? results)
    {
        ...
        try
        {
            ...
            _indicatorEntriesBuffer.Clear();
            _groupedEntriesMapBuffer.Clear();
            ...
            foreach (var indicator in indicators)
            {
                ...
                bool isMultiSeriesInternal = validSeries.Count > 1 || (indicator.SeriesColors?.Count ?? 0) > 1;
                bool shouldBeGrouped = !isMultiSeriesInternal;

                string panelKey = indicator.IsOverlay ? string.Empty : (indicator.OverlayPanelId ?? string.Empty);

                if (validSeries.Count > 1 || !shouldBeGrouped)
                {
                    _indicatorEntriesBuffer.Add(new IndependentIndicatorEntry
                    {
                        IsOverlay = indicator.IsOverlay,
                        OverlayPanelId = indicator.OverlayPanelId,
                        Settings = indicator,
                        Result = result,
                        ValidSeries = validSeries,
                        BaseName = indicatorBaseName
                    });
                }
                else
                {
                    var groupKey = (indicator.IsOverlay, panelKey, indicatorBaseName);
                    if (!_groupedEntriesMapBuffer.TryGetValue(groupKey, out var groupedEntry))
                    {
                        groupedEntry = new GroupedIndicatorEntry
                        {
                            IsOverlay = indicator.IsOverlay,
                            OverlayPanelId = indicator.OverlayPanelId,
                            TypeName = indicatorBaseName
                        };
                        _groupedEntriesMapBuffer[groupKey] = groupedEntry;
                        _indicatorEntriesBuffer.Add(groupedEntry);
                    }
                    groupedEntry.Items.Add((indicator, result!, validSeries));
                }
            }

            // 3-Tier Sort:
            // Tier 1: Main window (IsOverlay == true) first, Sub-window (IsOverlay == false) second
            // Tier 2: Sub-window by panel order (Default panel first, then Panel A, B, C... ascending)
            // Tier 3: Indicator name ascending
            _indicatorEntriesBuffer.Sort(s_indicatorEntryComparison);

            // Render all sorted entries
            foreach (var entry in _indicatorEntriesBuffer)
            {
                entry.Render(this, ref itemIndex, lookupIndex);
            }
