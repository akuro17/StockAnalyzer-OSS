...
public partial class ChartViewModel : ViewModelBase, IChartRenderSettings, IRecipient<TickerSelectedMessage>, IDisposable
{
    ...
    private void PerformInitialSync()
    {
        ...
        _messenger.Send((RequestMessage<string>)request);
    ...
    private Task SyncCurrentSymbolAsync()
    {
        ...
        var syncSession = DialogService.CreateMultiSyncProgressSession();
        var vm = syncSession.ViewModel;

        // Populate settings from TickerListViewModel if available, otherwise use defaults
        var tickerListVm = App.Current?.Services?.GetService(typeof(TickerListViewModel)) as TickerListViewModel;
        vm.IsTimeSeriesSyncEnabled = tickerListVm?.IsTimeSeriesSyncEnabled ?? true;
        vm.IsMetadataSyncEnabled = tickerListVm?.IsMetadataSyncEnabled ?? true;
        vm.IsAutoSyncEnabled = tickerListVm?.IsAutoSyncEnabled ?? true;
        vm.IsFullHistoryEnabled = tickerListVm?.IsFullHistoryEnabled ?? false;
        vm.DelayMinSeconds = tickerListVm?.SyncDelayMinSeconds ?? 3.0m;
        vm.DelayMaxSeconds = tickerListVm?.SyncDelayMaxSeconds ?? 5.0m;
        vm.StartSyncPeriodYears = tickerListVm?.StartSyncPeriodYears ?? 5;
        ...
        // Simple action progress reporter
        ...
        itemVm.RequestRetry += async (ivm, retryCt) =>
        {
            ...
            try
            {
                ...
                // After successful sync, reload data to display updated candles
        ...
        // Define handlers for the window events
        ...
        syncSession.Show(DialogService.GetMainWindowOwner());
