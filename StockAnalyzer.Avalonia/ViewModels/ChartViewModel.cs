...
public partial class ChartViewModel : ViewModelBase, IChartRenderSettings, IRecipient<TickerSelectedMessage>, IRecipient<TickerDataRefreshedMessage>, IDisposable
{
    ...
    private async Task LoadDataInternalAsync(bool forceRefresh)
    {
        ...
        try
        {
             ...
             dispatcher.Post<(ChartViewModel vm, CoreCandleData[] candles)>(static (state) => 
             {
                 ...
                 RecalculateRestoredDeferredComputationObjects(vm.ObjectManager, candles);
    ...
    /// computation (regression fit, spline point extraction, volume histogram) is deferred
    /// out of their constructor into Recalculate(). Called after restoring drawings via
    /// ObjectManager.LoadSnapshot(), which only restores Points[] and never triggers this,
    /// so a restored object's derived state would otherwise stay empty until the user
    /// happens to drag it. Covers the object types handled by
    /// <see cref="Drawing.DeferredComputationRecalculator"/> (RegressionTrendObject /
    /// RangeSplineObject / FixedRangeVolumeProfileObject) -- Harmonic/AutoElliottWave/
    /// GeometricPattern have an analogous deferred-Recalculate() pattern but were not
    /// reported as broken after restore and have not been verified to need this, so they
    /// are deliberately out of scope here rather than assumed to behave the same way.
    /// </summary>
    private static void RecalculateRestoredDeferredComputationObjects(Drawing.ChartObjectManager objectManager, IReadOnlyList<CoreCandleData> candles)
    {
        if (candles.Count == 0) return;

        foreach (var obj in objectManager.Objects)
        {
            Drawing.DeferredComputationRecalculator.TryRecalculate(obj, candles);
        }
    }
