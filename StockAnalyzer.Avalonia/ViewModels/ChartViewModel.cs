...
public partial class ChartViewModel : ViewModelBase, IChartRenderSettings, IRecipient<TickerSelectedMessage>, IRecipient<TickerDataRefreshedMessage>, IRecipient<ChartPeriodChangedMessage>, IRecipient<ChartTimeframeChangedMessage>, IDisposable
{
    private static int _dtwRequestIdCounter = 0;
    private readonly Microsoft.Extensions.Logging.ILogger<ChartViewModel>? _logger;
    ...
    private readonly Services.Drawing.IChartDrawingRepository? _drawingRepository;
    private readonly Services.Drawing.DrawingDocumentSessionStore? _sessionStore;
    private readonly ISpiralAnalysisDataSource? _spiralAnalysisDataSource;
    private readonly ISeasonalityChartDataSource? _seasonalityChartDataSource;
    private string? _drawingsOwnerSymbol;
    private TimeframeType _drawingsOwnerTimeFrame;
    // Whether THIS instance has, since it started owning _drawingsOwnerSymbol/_drawingsOwnerTimeFrame,
    // itself observed real (non-empty) drawing content - either restored via Load or added by the
    // user. See ShouldPersistDrawingsSnapshot for why this matters.
    private bool _drawingsHasEverBeenNonEmptyForOwner;
    // Repository revision of the drawings file this instance last loaded or saved for the current owner key.
    // A different repository revision means another tab persisted in the meantime (this instance is stale).
    private long _drawingsBaseRevision;
    private readonly Dictionary<ChartDrawingContextType, List<DrawingLayerRecord>> _contextLayersStore = new();
    private ChartDrawingContextType _currentContextType = ChartDrawingContextType.Standard;
    ...
    /// <summary>
    /// Increments every time <see cref="Candles"/> is reassigned with freshly-fetched data (initial
    /// load, symbol/timeframe switch, or an explicit forceRefresh reload) -- LoadDataInternalAsync is
    /// the sole call site that reassigns Candles. Consumers that cache a computation keyed partly on
    /// Candles.Count (e.g. VolumeProfileViewportCoordinator) must also key on this revision, since a
    /// data correction/reload can replace every OHLCV value while leaving the candle COUNT unchanged,
    /// which a Count-only cache key would silently miss (F06:
    ...
    /// <summary>Snapshot of the data drawing objects need for (re)calculation: candles, indicator results/settings and symbol.</summary>
    public Drawing.DrawingCalculationContext CreateDrawingCalculationContext()
        => new(Candles, IndicatorResults, Indicators, Symbol);
    ...
    [ObservableProperty]
    private bool _enableExtendedLookbackForIndicators = false;
    ...
    [NotifyPropertyChangedFor(nameof(EffectiveIsSubWindowVisible))]
    [NotifyPropertyChangedFor(nameof(IsDisplayOff))]
    private bool _canToggleIndicators = false; // Safe default; set from ChartTypeCapabilitiesRegistry in OnChartTypeChanged
    ...
    private bool _isReceivingTimeframeSync;
    private bool _isReceivingPeriodSync;
    ...
    private void PerformInitialSync()
    {
        ...
        var periodReq = new CurrentPeriodRequestMessage();
        _messenger.Send(periodReq);
        if (periodReq.HasReceivedResponse)
        {
            if (MaxCandleCount != periodReq.Response)
            {
                _isReceivingPeriodSync = true;
                try
                {
                    MaxCandleCount = periodReq.Response;
                }
                finally
                {
                    _isReceivingPeriodSync = false;
                }
            }
        }

        var tfReq = new CurrentTimeframeRequestMessage();
        _messenger.Send(tfReq);
        if (tfReq.HasReceivedResponse)
        {
            if (SelectedTimeFrame != tfReq.Response)
            {
                _isReceivingTimeframeSync = true;
                try
                {
                    SelectedTimeFrame = tfReq.Response;
                }
                finally
                {
                    _isReceivingTimeframeSync = false;
                }
            }
        }
    ...
    public void Receive(ChartPeriodChangedMessage message)
    {
        if (ReferenceEquals(message.Sender, this)) return;
        if (!IsSyncEnabled) return;
        if (message.Sender is ChartViewModel senderVm && !senderVm.IsSyncEnabled) return;

        if (MaxCandleCount != message.Value)
        {
            _isReceivingPeriodSync = true;
            try
            {
                MaxCandleCount = message.Value;
            }
            finally
            {
                _isReceivingPeriodSync = false;
    ...
    public void Receive(ChartTimeframeChangedMessage message)
    {
        if (ReferenceEquals(message.Sender, this)) return;
        if (!IsSyncEnabled) return;
        if (message.Sender is ChartViewModel senderVm && !senderVm.IsSyncEnabled) return;

        if (SelectedTimeFrame != message.Value)
        {
            _isReceivingTimeframeSync = true;
            try
            {
                SelectedTimeFrame = message.Value;
            }
            finally
            {
                _isReceivingTimeframeSync = false;
    ...
    private async Task ExportChartImageAsync()
    {
        ...
        {
            await DialogService.ShowExportChartImageDialogAsync(this);
        }
    }
    ...
    private void ApplyIndicators(IEnumerable<CoreIndicatorSettings> newIndicators)
    {
        ...
        var clonedList = new List<CoreIndicatorSettings>();

        foreach (var indicator in newIndicators)
        {
            var oldId = indicator.Id;
            var clone = indicator.Clone();
            idMap[oldId] = clone.Id;
            clonedList.Add(clone);
        }

        // Remap foreign key references (SourceIndicatorId / DynamicPeriodIndicatorId)
        foreach (var clone in clonedList)
        {
            if (!string.IsNullOrEmpty(clone.SourceIndicatorId) && idMap.TryGetValue(clone.SourceIndicatorId, out var newSourceId))
            {
                clone.SourceIndicatorId = newSourceId;
            }
    ...
    private async Task ShowIndicatorPropertiesAsync(CoreIndicatorSettings? indicator)
    {
        ...
        await DialogService.ShowIndicatorPropertiesDialogAsync(indicator, ApplySingleIndicator, Indicators);
    }
    ...
    private void ApplySingleIndicator(CoreIndicatorSettings changedIndicator)
    {
        ...
        if (existing != null)
        {
            bool isMathChange = changedIndicator.MathematicalVersion > 0 
                || changedIndicator.PriceSource != existing.PriceSource 
                || changedIndicator.SourceIndicatorId != existing.SourceIndicatorId 
                || changedIndicator.DynamicPeriodIndicatorId != existing.DynamicPeriodIndicatorId;
            ...
            if (isMathChange)
            {
                ...
                if (index >= 0)
                {
                    changedIndicator.MathematicalVersion = Math.Max(existing.MathematicalVersion + 1, existing.MathematicalVersion + changedIndicator.MathematicalVersion);
    ...
    public void Dispose()
    {
        ...
        _drawingEditCoordinator?.Dispose();
        _drawingEditCoordinator = null;
    ...
    partial void OnSymbolChanged(string value)
    {
        ...
        // Drive the seasonality overlay only from an actual symbol change (user pick or a restored
        // workspace symbol) - never from the startup default-symbol auto-load, which fires
        // OnCandlesChanged but not this. Keeps a freshly opened / restored Seasonality Chart tab idle
        // until the user has actually chosen a symbol.
        _seasonalityChartDataSource?.SetActiveSymbol(value);
    ...
    private int _lastExtendedLookbackBarCount = 0;
    private CancellationTokenSource? _indicatorCalcCts;

    // Extended-Lookback (Warm-up) Calculation Control: when EnableExtendedLookbackForIndicators is
    // active and the current chart type is eligible, LoadDataInternalAsync fetches extra leading
    // historical bars beyond MaxCandleCount and stashes the full fetched array here. Candles keeps
    // exactly MaxCandleCount bars (rendered/scrollable range is never extended). CalculateIndicatorsAsync
    // calculates over this extended array and trims the results back to Candles.Count.
    private IReadOnlyList<CoreCandleData>? _extendedCalculationCandles;
    private int _extendedLookbackBarCount = 0;
    ...
    private void CalculateIndicatorsAsync()
    {
        ...
        var extendedCandlesSnapshot = _extendedCalculationCandles;
        var extendedLookbackSnapshot = _extendedLookbackBarCount;
        ...
        _ = Task.Run(async () => 
        {
            ...
            try
            {
                ...
                // Extended-Lookback (Warm-up) Calculation Control: when active (never for
                // Renko/Kagi/PointAndFigure, which already replaced targetCandles above with their
                // brick/column series), calculate over the extended candle array so long-period
                // indicators get their full warm-up input; the result is trimmed back to
                // snapshotCandles.Count below via the existing SlicedIndicatorResult mechanism.
                bool extensionActive = activeAdapter == null && extendedCandlesSnapshot != null && extendedLookbackSnapshot > 0;
                if (extensionActive)
                {
                    targetCandles = extendedCandlesSnapshot!;
                }
                ...
                if (snapshotCandles.Count == _lastCandleCount &&
                    ...
                    extendedLookbackSnapshot == _lastExtendedLookbackBarCount &&
                ...
                {
                    Dictionary<string, IReadOnlyList<CoreCandleData?>>? secondaryCandlesBySymbol = null;
                    var comparisonSymbols = snapshotSettings
                        .Where(s => s.IsEnabled && (s.TypeEnum == IndicatorType.Correlation || s.ParameterObject is StockAnalyzer.Core.Models.Parameters.CoreCorrelationParameter) && s.ParameterObject is StockAnalyzer.Core.Models.Parameters.CoreCorrelationParameter cp && !string.IsNullOrWhiteSpace(cp.ComparisonSymbol))
                        .Select(s => ((StockAnalyzer.Core.Models.Parameters.CoreCorrelationParameter)s.ParameterObject!).ComparisonSymbol.Trim().ToUpperInvariant())
                        .Distinct(StringComparer.OrdinalIgnoreCase)
                        .ToList();

                    if (comparisonSymbols.Count > 0 && !string.IsNullOrEmpty(symbol))
                    {
                        secondaryCandlesBySymbol = new Dictionary<string, IReadOnlyList<CoreCandleData?>>(StringComparer.OrdinalIgnoreCase);

                        // 1. Check for self-comparison symbols (same ticker as current chart, supporting ^ and ./- aliases)
                        var selfCandles = new List<CoreCandleData?>(snapshotCandles.Count);
                        for (int i = 0; i < snapshotCandles.Count; i++)
                        {
                            selfCandles.Add(snapshotCandles[i]);
                        }

                        var externalSymbols = new List<string>();
                        foreach (var sym in comparisonSymbols)
                        {
                            if (SymbolHelper.IsSameSymbol(sym, symbol))
                            {
                                secondaryCandlesBySymbol[sym] = selfCandles;
                                secondaryCandlesBySymbol[symbol] = selfCandles;
                            }
                            else
                            {
                        ...
                        // 2. Align external comparison symbols (if any)
                        if (externalSymbols.Count > 0)
                        {
                            var aligner = alignerService ?? new ComparisonDataAligner(dataService);
                            var coreTimeFrame = timeFrame.ToCoreTimeFrame();
                            try
                            {
                                var alignedData = await aligner.AlignAsync(symbol, externalSymbols, coreTimeFrame, snapshotCandles.Count, cts.Token).ConfigureAwait(false);
                                if (alignedData != null && alignedData.Series.Count > 0)
                                {
                                    foreach (var sym in externalSymbols)
                                    {
                                        if (alignedData.Series.TryGetValue(sym, out var candleArray) && candleArray != null)
                                        {
                                            var coreCandles = new List<CoreCandleData?>(candleArray.Length);
                                            for (int i = 0; i < candleArray.Length; i++)
                                            {
                                                var c = candleArray[i];
                                                if (c.HasValue)
                                                {
                                                    coreCandles.Add(new CoreCandleData(c.Value.Timestamp, c.Value.Open, c.Value.High, c.Value.Low, c.Value.Close, c.Value.Volume));
                                                }
                                                else
                                                {
                                                    coreCandles.Add(null);
                                                }
                                            }
                                            secondaryCandlesBySymbol[sym] = coreCandles;
                                        }
                                    }
                                }
                            }
                            ...
                            {
                                System.Diagnostics.Debug.WriteLine($"Error aligning indicator comparison data: {ex.Message}");
                            }
                        }
                    }


                    results = await _analysisPipelineService.CalculateIndicatorsAsync(targetCandles, snapshotSettings, secondaryCandlesBySymbol);

                    if (extensionActive)
                    {
                        // Trim the warm-up prefix back off so every downstream consumer (rendering,
                        // pan/zoom slicing, drawing recalculation) keeps seeing a result length equal
                        // to snapshotCandles.Count, exactly as when extension is inactive. Some
                        // indicators (e.g. Ichimoku's SenkouSpanA/B) legitimately emit a series longer
                        // than targetCandles.Count to carry a forward-shifted future projection; for
                        // those, CalculateRetainedCount extends the retained window so that trailing
                        // future data survives the trim instead of being discarded.
                        var trimmed = new Dictionary<string, IIndicatorResult>(results.Count);
                        foreach (var kvp in results)
                        {
                            int retainedCount = ExtendedLookbackHelper.CalculateRetainedCount(
                                kvp.Value, snapshotCandles.Count, targetCandles.Count);
                            trimmed[kvp.Key] = new StockAnalyzer.Avalonia.Views.Chart.SlicedIndicatorResult(
                                kvp.Value, extendedLookbackSnapshot, retainedCount, targetCandles.Count);
                        }
                        results = trimmed;
                    }
                    ...
                    _lastExtendedLookbackBarCount = extendedLookbackSnapshot;
                }
                ...
                dispatcher.Post<(ChartViewModel vm, int id, IReadOnlyDictionary<string, IIndicatorResult> results, CancellationTokenSource cts)>(static (state) => 
                {
                    ...
                    if (state.vm.Candles != null && state.vm.ObjectManager != null)
                    {
                        var context = new Drawing.DrawingCalculationContext(state.vm.Candles, state.results, state.vm.Indicators, state.vm.Symbol);
                        foreach (var obj in state.vm.ObjectManager.Objects)
                        {
                            Drawing.DeferredComputationRecalculator.TryRecalculate(obj, context);
                        }
                    }
            ...
            {
                _logger?.LogError(ex, "[ChartViewModel] Indicator calculation pipeline error: {Message}", ex.Message);
            }
            finally
            {
                if (pooledArray != null)
    ...
    [ObservableProperty] private HsvData _dotUpColor = new(1, 120, 1, 1);
    [ObservableProperty] private HsvData _dotDownColor = new(1, 0, 1, 1);
    [ObservableProperty] private HsvData _dotNaturalColor = new(1, 0, 0, 0.5);
    [ObservableProperty] private double _dotBaseRadius = ChartSettingsConstants.DefaultDotBaseRadius;
    [ObservableProperty] private DotShapeType _dotShape = DotShapeType.Circle;
    [ObservableProperty] private PriceType _dotPriceType = PriceType.Close;

    partial void OnDotUpColorChanged(HsvData value) => RequestRender(RenderReason.VisualChanged);
    partial void OnDotDownColorChanged(HsvData value) => RequestRender(RenderReason.VisualChanged);
    partial void OnDotNaturalColorChanged(HsvData value) => RequestRender(RenderReason.VisualChanged);
    partial void OnDotBaseRadiusChanged(double value) => RequestRender(RenderReason.VisualChanged);
    partial void OnDotShapeChanged(DotShapeType value) => RequestRender(RenderReason.VisualChanged);
    partial void OnDotPriceTypeChanged(PriceType value) => RequestRender(RenderReason.DataChanged);
    ...
    [ObservableProperty] private HsvData _pnfBreakoutBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
    [ObservableProperty] private HsvData _pnfBreakoutBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);
    ...
    [ObservableProperty] private int _renkoMultiWaveMaxLines = 5;
    [ObservableProperty] private HsvData _renkoMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
    [ObservableProperty] private HsvData _renkoMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);
    ...
    [ObservableProperty] private int _pnfMultiWaveMaxLines = 5;
    ...
    [ObservableProperty] private int _kagiMultiWaveMaxLines = 5;
    [ObservableProperty] private HsvData _kagiMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
    [ObservableProperty] private HsvData _kagiMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);
    ...
    [ObservableProperty] private int _tlbMultiWaveMaxLines = 5;
    partial void OnTlbMultiWaveMaxLinesChanged(int value) => RequestRender(RenderReason.VisualChanged);

    [ObservableProperty] private HsvData _tlbMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
    [ObservableProperty] private HsvData _tlbMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);
    ...
    [ObservableProperty]
    private DrawingMoveAxisMode _moveAxisMode = DrawingMoveAxisMode.XY;

    public ChartObjectManager ObjectManager { get; } = new();

    /// <summary>
    /// Independent drawing layer management service for this chart session.
    /// </summary>
    public Services.Drawing.IDrawingLayerService LayerService { get; } = new Services.Drawing.DrawingLayerService();

    private Services.Drawing.DrawingEditCoordinator? _drawingEditCoordinator;

    /// <summary>
    /// Transaction coordinator bridging drawing mutations, in-memory undo/redo history, and persistence.
    /// </summary>
    public Services.Drawing.IDrawingEditCoordinator? DrawingEditCoordinator => _drawingEditCoordinator;

    /// <summary>
    /// Re-binds or initializes the drawing coordinator for the active symbol and timeframe.
    /// </summary>
    public void UpdateDrawingCoordinator()
    {
        _drawingEditCoordinator?.Dispose();
        _drawingEditCoordinator = null;
        ...
        var key = new DrawingDocumentKey(Symbol, SelectedTimeFrame);
        var session = _sessionStore?.GetOrCreate(key) ?? new Services.Drawing.DrawingDocumentSession(key);
        var history = session.GetOrCreateHistory(_currentContextType);

        _drawingEditCoordinator = new Services.Drawing.DrawingEditCoordinator(
            ObjectManager,
            history,
            session,
            LayerService,
            candlesProvider: () => Candles,
            sessionStore: _sessionStore,
            beforeMutation: () => RefreshDrawingsIfStale());

        _drawingEditCoordinator.CommitSucceeded += (_, _) => PersistCurrentDrawings();
        OnPropertyChanged(nameof(DrawingEditCoordinator));
    }
    ...
    /// <summary>
    /// Persists the current drawing state for the currently-displayed symbol+timeframe.
    /// Called by ChartInteractionController after any operation that adds, removes, or
    /// edits a drawing object (discrete state persistence, per SA_ARCHITECTURE_RULES.md).
    /// </summary>
    public void PersistCurrentDrawings()
    {
        if (_drawingRepository == null || _drawingsOwnerSymbol == null) return;
        var snapshot = ObjectManager.GetSnapshot();
        if (!ShouldPersistDrawingsSnapshot(snapshot)) return;

        var outcome = SaveDrawingsIfCurrent(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame, BuildDrawingsPayload(snapshot));
        if (!outcome.IsAccepted)
        {
            // Another tab persisted first: keep its file, adopt it here. This tab's rejected state is preserved
            // by the repository as a ".conflict" file, so nothing is destroyed silently.
            ReloadDrawingsFromRepository(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame);
        }
    }

    /// <summary>
    /// Reloads this instance's drawings from the repository when another tab persisted a newer version of the same
    /// ticker+timeframe file. Called before this instance starts an edit, so the edit is applied on top of the latest state.
    /// </summary>
    /// <returns>True when the drawings were reloaded.</returns>
    public bool RefreshDrawingsIfStale()
    {
        if (_drawingsOwnerSymbol == null || !IsDrawingsStale()) return false;
        if (_drawingEditCoordinator?.IsEditing == true) return false; // never replace objects under an open transaction

        ReloadDrawingsFromRepository(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame);
        return true;
    }

    private bool IsDrawingsStale() =>
        _drawingRepository != null && _drawingsOwnerSymbol != null &&
        _drawingRepository.GetRevision(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame) != _drawingsBaseRevision;

    private Services.Drawing.ChartDrawingPayload BuildDrawingsPayload(Dictionary<ChartDrawingContextType, List<IChartObject>> snapshot)
    {
        _contextLayersStore[_currentContextType] = LayerService.GetAllLayers().ToList();
        return new Services.Drawing.ChartDrawingPayload
        {
            Objects = snapshot,
            Layers = new Dictionary<ChartDrawingContextType, List<DrawingLayerRecord>>(_contextLayersStore),
            LinkGroups = ObjectManager.GetLinkGroupIndexRecords()
        };
    }

    private Services.Drawing.DrawingSaveOutcome SaveDrawingsIfCurrent(string symbol, TimeframeType timeframe, Services.Drawing.ChartDrawingPayload payload)
    {
        var outcome = _drawingRepository!.SavePayloadIfCurrent(symbol, timeframe, payload, _drawingsBaseRevision);
        if (outcome.IsAccepted) _drawingsBaseRevision = outcome.Revision;
        return outcome;
    }

    /// <summary>
    /// Replaces this instance's in-memory drawings and layers with the repository content for the key and records the
    /// revision that content corresponds to.
    /// </summary>
    private void ReloadDrawingsFromRepository(string symbol, TimeframeType timeframe)
    {
        ObjectManager.Clear();
        _contextLayersStore.Clear();
        _drawingsHasEverBeenNonEmptyForOwner = false;

        if (_drawingRepository != null)
        {
            var restoredPayload = _drawingRepository.LoadPayload(symbol, timeframe, out var revision);
            _drawingsBaseRevision = revision;
            if (restoredPayload != null)
            {
                if (restoredPayload.Objects != null)
                {
                    ObjectManager.LoadSnapshot(restoredPayload.Objects);
                    if (restoredPayload.LinkGroups != null)
                    {
                        foreach (var (linkContext, linkRecords) in restoredPayload.LinkGroups)
                        {
                            ObjectManager.LoadLinkGroupIndexRecords(linkContext, linkRecords);
                        }
                    }
                    if (restoredPayload.Objects.Values.Any(list => list.Count > 0))
                    {
                        _drawingsHasEverBeenNonEmptyForOwner = true;
                    }
                }

                if (restoredPayload.Layers != null)
                {
                    foreach (var (ctx, layers) in restoredPayload.Layers)
                    {
                        _contextLayersStore[ctx] = new List<DrawingLayerRecord>(layers);
                    }

                    if (restoredPayload.Layers.Values.Any(list => list.Count > 1 || list.Any(l => l.Panel.Kind != PanelKind.Main)))
                    {
                        _drawingsHasEverBeenNonEmptyForOwner = true;
                    }
                }
        ...
        _currentContextType = GetContextType(ChartType);
        if (_contextLayersStore.TryGetValue(_currentContextType, out var currentLayers))
        {
            LayerService.InitializeLayers(currentLayers);
        }
        else
        {
            LayerService.InitializeLayers(null);
        }
    }

    /// <summary>
    /// Flushes any not-yet-persisted drawing changes synchronously. PersistCurrentDrawings'
    /// fire-and-forget Save() could otherwise be abandoned mid-write if the process shuts down
    /// before its background Task.Run completes. Called from Dispose() (best-effort, reached only
    /// if the DI container's cascading disposal runs during desktop.ShutdownRequested) and from
    /// MainWindowViewModel.ForceSaveOnShutdown() (deterministic, runs during Window.OnClosing
    /// before the process begins tearing down - the same reasoning already applied there to
    /// workspace/DetachedTabs persistence).
    /// </summary>
    public void FlushPendingDrawings()
    {
        if (_drawingRepository == null || _drawingsOwnerSymbol == null) return;
        if (IsDrawingsStale()) return; // holds no unsaved edits; must not overwrite the newer file
        var snapshot = ObjectManager.GetSnapshot();
        if (!ShouldPersistDrawingsSnapshot(snapshot)) return;

        try
        {
            SaveDrawingsIfCurrent(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame, BuildDrawingsPayload(snapshot))
                .Completion.GetAwaiter().GetResult();
        }
        catch
        {
            // Best-effort shutdown flush; must never block or crash app exit.
        }
    }

    /// <summary>
    /// Multiple ChartViewModel instances (the main chart plus any synced panel/detached charts -
    /// IsSyncEnabled defaults to true, so a ticker selection is broadcast to and reloaded by all
    /// of them independently) can each load/save the same ticker+timeframe drawings file. An
    /// instance whose own Load raced ahead of another instance's real save (getting an empty
    /// read back), or that was simply never drawn on, must not be allowed to persist "nothing"
    /// for that key and silently destroy the other instance's real content. An instance that DID
    /// observe real content at some point while owning the current key (loaded or user-added) is
    /// still allowed to save empty, since that correctly reflects the user deleting their last
    /// drawing on this particular chart.
    /// </summary>
    private bool ShouldPersistDrawingsSnapshot(Dictionary<ChartDrawingContextType, List<IChartObject>> snapshot)
    {
        if (snapshot.Values.Any(list => list.Count > 0))
        {
            _drawingsHasEverBeenNonEmptyForOwner = true;
            return true;
        }
        ...
        if (allLayers.Count > 1 || allLayers.Any(l => l.Panel.Kind != PanelKind.Main))
        {
            _drawingsHasEverBeenNonEmptyForOwner = true;
            return true;
        }

        return _drawingsHasEverBeenNonEmptyForOwner;
    }
    ...
    private void UpdateParametersFromSettings()
    {
        ...
        RenkoMultiWaveMaxLines = s.RenkoMultiWaveMaxLines;
        ...
        KagiMultiWaveMaxLines = s.KagiMultiWaveMaxLines;
        ...
        PnfMultiWaveMaxLines = s.PnfMultiWaveMaxLines;
        ...
        TlbMultiWaveMaxLines = s.ThreeLineBreakMultiWaveMaxLines;
        ...
        EnableExtendedLookbackForIndicators = s.EnableExtendedLookbackForIndicators;
    ...
    private void UpdateColorsFromSettings()
    {
        ...
        RenkoMultiWaveBullishColor = HsvData.FromHtmlSafe(s.RenkoMultiWaveBullishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor));
        RenkoMultiWaveBearishColor = HsvData.FromHtmlSafe(s.RenkoMultiWaveBearishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor));

        // --- P&F ---
        PnfBullishColor = HsvData.FromHtmlSafe(s.PnfBullishColor);
        PnfBearishColor = HsvData.FromHtmlSafe(s.PnfBearishColor);
        PnfBreakoutBullishColor = HsvData.FromHtmlSafe(s.PnfBreakoutBullishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor));
        PnfBreakoutBearishColor = HsvData.FromHtmlSafe(s.PnfBreakoutBearishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor));

        // --- Kagi ---
        KagiBullishColor = HsvData.FromHtmlSafe(s.KagiBullishColor);
        KagiBearishColor = HsvData.FromHtmlSafe(s.KagiBearishColor);
        KagiMultiWaveBullishColor = HsvData.FromHtmlSafe(s.KagiMultiWaveBullishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor));
        KagiMultiWaveBearishColor = HsvData.FromHtmlSafe(s.KagiMultiWaveBearishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor));
        ...
        TlbMultiWaveBullishColor = HsvData.FromHtmlSafe(s.ThreeLineBreakMultiWaveBullishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor));
        TlbMultiWaveBearishColor = HsvData.FromHtmlSafe(s.ThreeLineBreakMultiWaveBearishColor, HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor));
        ...
        // --- Dot ---
        DotUpColor = HsvData.FromHtmlSafe(s.DotUpColor);
        DotDownColor = HsvData.FromHtmlSafe(s.DotDownColor);
        DotNaturalColor = HsvData.FromHtmlSafe(s.DotNaturalColor);
        DotBaseRadius = s.DotBaseRadius;
        DotShape = s.DotShape;
        DotPriceType = s.DotPriceType;
    ...
    private void UpdateColorsFromTheme()
    {
        ...
        DotUpColor = HsvData.FromIndicatorColor(theme.Bullish);
        DotDownColor = HsvData.FromIndicatorColor(theme.Bearish);
        DotNaturalColor = HsvData.FromIndicatorColor(theme.Neutral);
    ...
    partial void OnMaxCandleCountChanged(int value)
    {
        ...
        if (IsSyncEnabled && !_isReceivingPeriodSync)
        {
            _messenger.Send(new ChartPeriodChangedMessage(this, value));
        }
    ...
    private void SetCandleCount(string countStr)
    {
        if (int.TryParse(countStr, out int count))
        {
            if (MaxCandleCount == count)
            {
                ...
                if (IsSyncEnabled)
                {
                    _messenger.Send(new ChartPeriodChangedMessage(this, count));
                }
                _ = LoadDataInternalAsync(true);
            }
            else
    ...
    private async Task LoadDataInternalAsync(bool forceRefresh)
    {
        ...
        try
        {
            ...
            // No symbol (empty Chart:DefaultSymbol on first launch, before the user enters one):
            // there is nothing to fetch, and drawings/cache are keyed by symbol, so skip the whole load.
            if (string.IsNullOrWhiteSpace(Symbol)) return;

            IsLoading = true;
            
            // --- LIFECYCLE MANAGEMENT: PERSIST DRAWINGS PER SYMBOL+TIMEFRAME ON LOAD ---
            // When loading new data (symbol/timeframe change or refresh), the drawings belonging to
            // the previously-displayed symbol+timeframe are saved (Day/Week/Monthly are independent
            // sets, per Y:\Temp\sa_analysis_report.md), all in-memory contexts are cleared, and any
            // drawings previously saved for the new symbol+timeframe are restored.
            if (_drawingRepository != null && _drawingsOwnerSymbol != null)
            {
                var outgoingSnapshot = ObjectManager.GetSnapshot();
                // A stale instance holds no unsaved edits (every edit persists on commit), so it must not write
                // its outdated state over the newer file another tab saved.
                if (!IsDrawingsStale() && ShouldPersistDrawingsSnapshot(outgoingSnapshot))
                {
                    // Awaited (not fire-and-forget): the Load() for the new symbol/timeframe below
                    // must not race this write, or a quick switch-away-and-back can read a stale/
                    // missing file.
                    await SaveDrawingsIfCurrent(_drawingsOwnerSymbol, _drawingsOwnerTimeFrame, BuildDrawingsPayload(outgoingSnapshot)).Completion;
                }
            }

            ReloadDrawingsFromRepository(Symbol, SelectedTimeFrame);

            _drawingsOwnerSymbol = Symbol;
            _drawingsOwnerTimeFrame = SelectedTimeFrame;
            UpdateDrawingCoordinator();
            // ----------------------------------------------------
            ...
            // Extended-Lookback (Warm-up) Calculation Control: when enabled and the chart type is
            // eligible (time-series charts + ThreeLineBreak; Renko/Kagi/PointAndFigure calculate on
            // a derived brick/column series and are excluded), fetch extra leading historical bars
            // sized to the largest active single-Period indicator, purely as calculation warm-up.
            int requiredLookback = (EnableExtendedLookbackForIndicators && ExtendedLookbackHelper.IsEligibleChartType(ChartType))
                ? ExtendedLookbackHelper.CalculateRequiredLookback(Indicators)
                : 0;
            int fetchCount = MaxCandleCount + requiredLookback;

            var data = _timeFrameManager != null
                ? await Task.Run(() => _timeFrameManager.GetCandlesAsync(Symbol, tf, fetchCount, token), token)
                : await Task.Run(() => _dataService.LoadCandlesAsync(Symbol, tf, fetchCount), token);
            ...
            // The rendered/scrollable range always stays exactly MaxCandleCount bars (never shows
            // extra candles); any surplus leading bars (fewer than requested if the data source has
            // less history, e.g. a recent IPO) are kept only for indicator warm-up input.
            int actualExtraLookback = Math.Max(0, candleCount - MaxCandleCount);
            CoreCandleData[] renderedCandles = actualExtraLookback > 0
                ? newCandles[actualExtraLookback..]
                : newCandles;
            ...
            dispatcher.Post<(ChartViewModel vm, CoreCandleData[] candles, CoreCandleData[]? extendedCandles, int extraLookback)>(static (state) =>
            {
                ...
                // Drawing tools with expensive-ish per-candle derived state (regression
                // fit, spline point extraction, volume histogram) defer computing it out
                // of their constructor into Recalculate(), which every interactive path
                // (creation, drag-then-release) remembers to call. ChartObjectManager.
                // LoadSnapshot() (used above to restore drawings for this symbol/
                // timeframe) only restores Points[] via JSON deserialization and never
                // calls Recalculate(), so a restored object's derived state stays
                // permanently empty until the user happens to drag it. For
                // RangeSplineObject this made HitTest() (which early-returns false when
                // its extracted points are empty) fail, so the restored object couldn't
                // even be selected -- let alone dragged or deleted -- until touched.
                RecalculateRestoredDeferredComputationObjects(vm.ObjectManager, candles, vm.IndicatorResults, vm.Indicators, vm.Symbol);
    ...
    /// <summary>
    /// Recalculates the derived state of drawing objects whose expensive-ish per-candle
    /// computation (regression fit, spline point extraction, volume histogram, pattern
    /// detection) is deferred out of their constructor into Recalculate(). Called after
    /// restoring drawings via ObjectManager.LoadSnapshot(), which only restores Points[]
    /// and never triggers this, so a restored object's derived state would otherwise stay
    /// empty until the user happens to drag it. Covers every object type handled by
    /// <see cref="Drawing.DeferredComputationRecalculator"/> (RegressionTrendObject /
    /// RangeSplineObject / FixedRangeVolumeProfileObject / KalmanFilterProjectionObject /
    /// HarmonicPatternObject / AutoElliottWaveObject / GeometricPatternObject).
    /// </summary>
    private static void RecalculateRestoredDeferredComputationObjects(Drawing.ChartObjectManager objectManager, IReadOnlyList<CoreCandleData> candles, IReadOnlyDictionary<string, IIndicatorResult>? indicatorResults = null, IReadOnlyList<CoreIndicatorSettings>? indicatorSettings = null, string symbol = "")
    {
        if (candles.Count == 0) return;

        var context = new Drawing.DrawingCalculationContext(candles, indicatorResults, indicatorSettings, symbol);
    ...
    public void RecalculateFutureOffset()
    {
        ...
        {
            // Indicators whose output extends past the last candle (e.g. the Ichimoku Kumo) declare how far; the margin follows the parameter.
            int projection = indicator.ParameterObject?.GetFutureProjectionBars() ?? 0;
            if (projection > 0)
            {
                 maxOffset = System.Math.Max(maxOffset, projection + StockAnalyzer.Core.ChartConstants.FutureProjectionBuffer);
            }
    ...
    partial void OnChartTypeChanged(ChartType value)
    {
        ...
        // Switch drawing object context and partitioned layers based on the new chart type.
        var newContext = GetContextType(value);
        if (newContext != _currentContextType)
        {
            _contextLayersStore[_currentContextType] = LayerService.GetAllLayers().ToList();
            ObjectManager.SwitchContext(newContext);
            _currentContextType = newContext;

            if (_contextLayersStore.TryGetValue(newContext, out var storedLayers))
            {
                LayerService.InitializeLayers(storedLayers);
            }
            else
            {
                LayerService.InitializeLayers(null);
    ...
    partial void OnCandlesChanged(IReadOnlyList<CoreCandleData> value)
    {
        ...
        _spiralAnalysisDataSource?.SetCandles(value);
    ...
    /// <summary>
    /// Centers the visible chart range on <paramref name="anchorDate"/> for the currently loaded
    /// <see cref="Candles"/> series (spec section 6.2 - Note-to-chart navigation). Resolves the
    /// nearest matching candle via <see cref="ChartAnchorDateResolver"/> (exact date match, or the
    /// nearest earlier trading day, or the nearest later one), then sets
    /// <see cref="VisibleStartIndex"/> to <c>AnchorIndex - VisibleCandleCount / 2</c>, clamped to
    /// the series bounds so the range never runs past either end.
    /// </summary>
    /// <returns>
    /// True if a candle was resolved and <see cref="VisibleStartIndex"/> was updated; false when
    /// no candles are loaded (caller should wait for data loading to finish before calling this,
    /// per spec section 6.2's "未ロード期間への対応").
    /// </returns>
    public bool JumpToAnchorDate(DateTime anchorDate, TimeFrame timeframe)
    {
        ...
        // ChartAnchorDateResolver operates on the shared Core CandleData type; Candles here is the
        // Avalonia-side CoreCandleData record (structurally identical OHLCV fields, but a distinct
        // type), so a one-off mapping is needed. This is a user-initiated, once-per-click
        // navigation action, not part of the Skia render loop, so the Zero-Allocation rendering
        // invariant does not apply.
        var coreCandles = Candles
            .Select(c => new StockAnalyzer.Core.Models.CandleData(c.Timestamp, c.Open, c.High, c.Low, c.Close, c.Volume))
            .ToList();

        var anchorIndex = ChartAnchorDateResolver.Resolve(coreCandles, anchorDate, timeframe);
        if (anchorIndex is null)
        {
            return false;
        }

        var maxStartIndex = Math.Max(0, Candles.Count - VisibleCandleCount);
        var centeredStartIndex = anchorIndex.Value - (VisibleCandleCount / 2);
        VisibleStartIndex = Math.Clamp(centeredStartIndex, 0, maxStartIndex);
        return true;
    }
    ...
    partial void OnKagiMultiWaveMaxLinesChanged(int value) => RequestRender(RenderReason.VisualChanged);
    ...
    partial void OnRenkoMultiWaveMaxLinesChanged(int value) => RequestRender(RenderReason.VisualChanged);
    ...
    partial void OnPnfMultiWaveMaxLinesChanged(int value) => RequestRender(RenderReason.VisualChanged);
    ...
    public void OnDrawingFinished(IChartObject obj)
    {
        ...
        else if (obj is Drawing.Objects.KalmanFilterProjectionObject kalmanObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                kalmanObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.ArimaProjectionObject arimaObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                arimaObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.FftProjectionObject fftObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                fftObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.HmmProjectionObject hmmObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                hmmObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.PearsonProjectionObject pearsonObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                pearsonObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.FrechetProjectionObject frechetObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                frechetObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.SsaProjectionObject ssaObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                ssaObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.SsaMultiComponentObject ssaMultiObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                ssaMultiObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.SsaSupportResistanceObject ssaSnrObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                ssaSnrObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.SsaAnomalyHighlightObject ssaAnomalyObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                ssaAnomalyObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        else if (obj is Drawing.Objects.AutoTimeCycleObject autoCycleObj)
        {
            if (_candles != null && _candles.Count > 0)
            {
                autoCycleObj.Recalculate(_candles, SelectedTimeFrame.ToTimeSpan());
                RequestRender(RenderReason.DataChanged);
            }
        }
        ...
        else if (obj is Drawing.Objects.HoughAutoLinesObject ||
                 obj is Drawing.Objects.HoughParabolicCurveObject ||
                 obj is Drawing.Objects.HoughKeyLevelsObject ||
                 obj is Drawing.Objects.HoughResonantFanObject ||
                 obj is Drawing.Objects.HoughMagneticLineObject)
        {
            if (_candles != null && _candles.Count > 0)
            {
                Drawing.DeferredComputationRecalculator.TryRecalculate(obj, _candles);
                RequestRender(RenderReason.DataChanged);
            }
        }
    }
    ...
    private async Task ExecuteDtwSearchAsync(Drawing.Objects.DtwProjectionObject dtwObj, int requestId)
    {
        ...
        try
        {
            ...
            {
                dispatcher.Post<(ChartViewModel vm, Drawing.Objects.DtwProjectionObject dtwObj)>(static (state) =>
                {
                    var (vm, dtwObj) = state;
                    if (vm._isDisposed) return;
                    dtwObj.ProjectedPath?.Clear();
                    dtwObj.MatchedStartTime = null;
                    dtwObj.MatchedEndTime = null;
                    dtwObj.IsUnmatched = true;
                    vm.RequestRender(RenderReason.DataChanged);
                }, (this, dtwObj));
            ...
            var result = await _marketStructureService.SearchSimilarPatternsAsync(
                ...
                lookback: 0,
                topK: 5,
                futureSteps: dtwObj.FutureSteps,
    ...
    public void ResetSettingsToDefault()
    {
        ...
        RenkoBullishColor = HsvData.FromIndicatorColor(theme.Bullish);
        RenkoBearishColor = HsvData.FromIndicatorColor(theme.Bearish);
        RenkoMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
        RenkoMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);

        PnfBullishColor = HsvData.FromIndicatorColor(theme.Bullish);
        PnfBearishColor = HsvData.FromIndicatorColor(theme.Bearish);
        PnfBreakoutBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
        PnfBreakoutBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);

        KagiBullishColor = HsvData.FromIndicatorColor(theme.Bullish);
        KagiBearishColor = HsvData.FromIndicatorColor(theme.Bearish);
        KagiMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
        KagiMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);

        ThreeLineBreakBullishColor = HsvData.FromIndicatorColor(theme.Bullish);
        ThreeLineBreakBearishColor = HsvData.FromIndicatorColor(theme.Bearish);
        TlbMultiWaveBullishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBullishColor);
        TlbMultiWaveBearishColor = HsvData.FromHtmlSafe(ChartSettingsConstants.DefaultBreakoutBearishColor);
    ...
    private ChartDrawingContextType GetContextType(ChartType type)
    {
        return type switch
        {
            ...
            ChartType.Dot => ChartDrawingContextType.Linear,
