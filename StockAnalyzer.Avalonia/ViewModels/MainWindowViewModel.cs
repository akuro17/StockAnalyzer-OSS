    ...
    IDisposable
{
    ...
    private void InitializePanelTabs()
    {
        // Initial setup for default tabs is now handled by PanelTabFactory in ApplyPanelTabs fallback
         if (LeftPanelTabs.Count == 0)
         {
             ...
             if (item != null) LeftPanelTabs.Add(item);
             _stateStore.LeftPanel.IsVisible = true;
             _stateStore.LeftPanel.WidthOrHeight = LayoutConstants.DefaultLeftWidth;
         }
 
         // Initialize default indicators if none exist
         if (ChartViewModel.Indicators.Count == 0)
         {
             var defaults = StockAnalyzer.Core.Models.DefaultCoreIndicatorSettings.GetDefault();
             foreach (var indicator in defaults)
             {
                 // Auto-enable SMA and Volume for immediate visual feedback
                 if (indicator.TypeEnum == StockAnalyzer.Core.Models.IndicatorType.SMA || 
                     indicator.TypeEnum == StockAnalyzer.Core.Models.IndicatorType.Volume)
                 {
        ...
        if (RightPanelTabs.Count == 0)
        {
            ...
            if (item != null) RightPanelTabs.Add(item);
        ...
        if (BottomPanelTabs.Count == 0)
        {
            ...
            if (item != null) BottomPanelTabs.Add(item);
    ...
    private async Task LoadWorkspaceAsync()
    {
        ...
        if (!string.IsNullOrEmpty(filePath))
        {
            try
            {
                await _workspaceCoordinator.InitializeWorkspaceAsync(filePath);
                
                await _dialogService.ShowAlertAsync(
                    _localizationService.GetString("Dialog_Success") ?? "Success",
                    $"Workspace loaded from {filePath}"
                );
            }
            catch (System.Exception ex)
            {
    ...
    private async Task SaveWorkspaceAsync()
    {
        ...
        if (!string.IsNullOrEmpty(filePath))
        {
            try
            {
                await _workspaceCoordinator.SaveActiveWorkspaceAsync(filePath);
            ...
            _dispatcherService.Post(() =>
            {
                ...
                if (existing != null)
                {
                    ...
                    if (isMathChange)
                    {
                        ...
                        if (index >= 0)
                        {
                            message.Value.MathematicalVersion = existing.MathematicalVersion + message.Value.MathematicalVersion;
    ...
    public void CaptureWorkspaceSettings(WorkspaceSettings settings)
    {
        ...
        // Capture TickerList column visibility
    ...
    {
        return _panelTabFactory.CreateTab(id);
    }
    ...
    public void Receive(OpenIndicatorPropertiesMessage message)
    {
        if (message.Value != null)
        {
            _ = _dialogService.ShowIndicatorPropertiesDialogAsync(message.Value);
        }
    }
