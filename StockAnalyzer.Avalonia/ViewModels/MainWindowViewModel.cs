    ...
    IDisposable
{
        ...
        IDynamicPeriodDriverService? dynamicPeriodDriverService = null)
    {
        ...
        // Auto-save when the Column Customization dropdown selection changes
        TickerListViewModel.ColumnTemplateSelector.SelectionChanged += OnTickerColumnTemplateSelectionChanged;
    }

    private void OnTickerColumnTemplateSelectionChanged(object? sender, EventArgs e)
    {
        if (!_isApplyingSettings)
        {
            _logger.LogInformation("UI Interaction: Ticker column template selection changed.");
            _layoutSaveScheduler.RequestSave(LayoutChangeReason.SelectionChanged);
        }
    }
    ...
    void IWorkspaceLayoutTarget.ImportCustomTickerCategories(System.Collections.Generic.IEnumerable<StockAnalyzer.Core.Models.Settings.TickerParentCategorySettings> categories)
    {
        TickerListViewModel?.ImportCustomTickerCategories(categories.ToList());
    }
    ...
    void IWorkspaceLayoutTarget.RestoreTickerColumnTemplateSelection(System.Collections.Generic.IReadOnlyDictionary<Guid, Guid>? selectionsByList)
    {
    ...
    [RelayCommand]
    private async Task OpenBacktestWindowAsync()
    {
        await _dialogService.ShowBacktestWindowAsync();
    }
    ...
    public void CaptureWorkspaceSettings(WorkspaceSettings settings)
    {
        ...
        settings.DeleteLatestBarsCount = 0;
        ...
        if (TickerListViewModel != null)
        {
            ...
            settings.CustomTickerCategories = TickerListViewModel.ExportCustomTickerCategories();
        }

        // Capture TickerList column visibility and widths
        // TickerListVisibleColumns always holds the Active Columns working set, even while a Column
    ...
    public void RestoreChartSettings(WorkspaceSettings settings)
    {
        ...
        TickerListViewModel.DeleteLatestBarsCount = 0;
    }
    ...
    private void BindTabToActiveChart(WorkspaceViewItem? item)
    {
        ...
        else if (item?.ViewModel is ChartViewModel cvm)
        {
            if (cvm.IsSyncEnabled)
            {
                cvm.MaxCandleCount = ChartViewModel.MaxCandleCount;
                cvm.SelectedTimeFrame = ChartViewModel.SelectedTimeFrame;
                if (!string.IsNullOrEmpty(ChartViewModel.Symbol))
                {
                    cvm.Symbol = ChartViewModel.Symbol;
                }
            }
        }
    }
    ...
    public void Receive(CurrentPeriodRequestMessage message)
    {
        message.Reply(ChartViewModel.MaxCandleCount);
    }

    public void Receive(CurrentTimeframeRequestMessage message)
    {
        message.Reply(ChartViewModel.SelectedTimeFrame);
    }
