...
/// Implements the Focus-Bind (並置型) approach with Left (比較元) & Right (比較先) containers,
...
public partial class IndicatorRegistrationViewModel : ViewModelBase, IDisposable
{
    ...
    // ===== Left Side (比較元) Configuration Properties =====
    ...
    // ===== Right Side (比較先) Configuration Properties =====
    ...
    private void LoadNavGroups()
    {
        ...
        // 1. Add "All Filters" ("All Filters") at the very top above Indicators header
        ...
        NavGroups.Add(new ScreenerGroupDisplayItem
        {
            ...
            CustomDisplayName = _localizationService["Nav_AllFilters"] ?? "All Filters"
        });

        // 2. Indicators Category Section
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService["Category_Indicators"] });
        foreach (var g in defaultGroups.Where(g => g.CategoryType == "Indicator"))
        {
            NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = false, Group = g });
        }

        // 3. Columns Category Section
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService["Category_Columns"] });
        ...
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService["Category_ChartPatterns"] });
    ...
    private void ApplyFilters()
    {
        ...
        {
            if (string.Equals(selectedGroup.CategoryType, "All", StringComparison.OrdinalIgnoreCase) ||
        ...
        // Apply strict alphabetical sort by DisplayName then ShortName for maximum readability
    ...
    private void RegisterIndicator()
    {
        ...
        var leftConfig = CreateSideConfig(
            LeftSelectedIndicator.IndicatorType ?? IndicatorType.SMA,
            LeftIndicatorSettings,
            LeftSelectedOutput,
            LeftSelectedTimeFrameItem?.Value ?? TimeFrame.D1,
            LeftOffset,
            LeftSelectedIndicator?.CategoryType ?? ScreenerItemCategoryType.Indicator);
        ...
        if (RightTargetMode == RightHandTargetMode.Indicator && RightSelectedIndicator != null)
        {
            rightConfig = CreateSideConfig(
                RightSelectedIndicator.IndicatorType ?? IndicatorType.SMA,
                RightIndicatorSettings,
                RightSelectedOutput,
                RightSelectedTimeFrameItem?.Value ?? TimeFrame.D1,
                RightOffset,
                RightSelectedIndicator?.CategoryType ?? ScreenerItemCategoryType.Indicator);
        }
    ...
    private ScreenerIndicatorSideConfig CreateSideConfig(
        ...
        TimeFrame timeFrame,
        int offset,
        ScreenerItemCategoryType categoryType = ScreenerItemCategoryType.Indicator)
    {
