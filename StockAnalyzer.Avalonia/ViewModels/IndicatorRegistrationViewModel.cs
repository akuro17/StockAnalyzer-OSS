...
/// <summary>
/// Display item for the Left Navigation Panel representing either a Category Header, Divider, Templates item, or a Selectable Group.
/// </summary>
public class ScreenerGroupDisplayItem
{
    ...
    public bool IsStandardItem => !IsHeader && !IsAllFilters && !IsDivider && !IsTemplates;
    public bool CanSelect => !IsHeader && !IsDivider;
...
public partial class IndicatorRegistrationViewModel : ViewModelBase, IDisposable
{
    ...
    private readonly TemplateCrudHelper<ScreenerIndicatorTemplate>? _templateCrud;
    ...
    public ObservableCollection<ScreenerIndicatorTemplate> Templates { get; } = new();
    public ObservableCollection<string> SelectedTemplateIndicatorNames { get; } = new();

    [ObservableProperty]
    private ScreenerIndicatorTemplate? _selectedTemplate;
    ...
    {
        _indicatorFactory = indicatorFactory ?? App.Current?.Services?.GetService<IIndicatorFactory>();
        _catalogProvider = catalogProvider ?? new ScreenerCatalogProvider();
        _localizationService = localizationService
            ?? App.Current?.Services?.GetService<ILocalizationService>()
            ?? (LocalizationManager.Instance != null ? new LocalizationService() : NullLocalizationService.Instance);
        _logger = logger ?? NullLogger<IndicatorRegistrationViewModel>.Instance;
        _templateService = templateService ?? App.Current?.Services?.GetService<ITemplateService>();
        _toastService = toastService ?? App.Current?.Services?.GetService<IToastNotificationService>();

        if (_templateService != null)
        {
            _templateCrud = new TemplateCrudHelper<ScreenerIndicatorTemplate>(_templateService, _toastService, TemplateType.Screener);
            _ = _templateCrud.LoadAllAsync(Templates, ex => _logger.LogError(ex, "Failed to load screener indicator templates"));
        }
    ...
    private void LoadNavGroups()
    {
        ...
        NavGroups.Add(new ScreenerGroupDisplayItem
        {
            ...
            CustomDisplayName = _localizationService.GetString("Nav_AllFilters")
        });

        // 2. Indicators Category Section
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService.GetString("IndicatorRegistration_NavHeader_Indicators") });
        foreach (var g in defaultGroups.Where(g => g.CategoryType == "Indicator"))
        {
            NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = false, Group = g, CustomDisplayName = ResolveGroupDisplayName(g) });
        }

        // 3. Columns Category Section
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService.GetString("IndicatorRegistration_NavHeader_Columns") });
        ...
        NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = true, HeaderTitle = _localizationService.GetString("IndicatorRegistration_NavHeader_ChartPatterns") });
        foreach (var g in defaultGroups.Where(g => g.CategoryType == "Criteria"))
        {
            NavGroups.Add(new ScreenerGroupDisplayItem { IsHeader = false, Group = g, CustomDisplayName = ResolveGroupDisplayName(g) });
        }

        // 5. Divider and Templates (placed directly under Wave Theory in Chart Patterns)
        NavGroups.Add(new ScreenerGroupDisplayItem { IsDivider = true });
        NavGroups.Add(new ScreenerGroupDisplayItem
        {
            IsTemplates = true,
            CustomDisplayName = _localizationService.GetString("Library_Templates")
        });

        SelectedGroupItem = NavGroups.FirstOrDefault(item => !item.IsHeader && !item.IsDivider);
    }

    /// <summary>Resolves a nav sub-group's localized display text from its <see cref="ScreenerIndicatorGroup.LocalizationKey"/>
    /// (e.g. "ScreenerGroup_Trend"), falling back to the internal <see cref="ScreenerIndicatorGroup.Name"/> (used for
    /// filter-matching/sorting, unaffected by this display-only resolution) if no key is set.</summary>
    private string ResolveGroupDisplayName(ScreenerIndicatorGroup group) =>
        group.LocalizationKey is { } key ? _localizationService.GetString(key) : group.Name;
    ...
    private void ApplyFilters()
    {
        ...
        else if (string.Equals(selectedGroup?.Name, ScreenerGroupNames.Price, StringComparison.OrdinalIgnoreCase))
        {
            var priceOrderMap = PriceDataHelper.PriceTypeOptions
                .Select((pt, idx) => (pt.ToString(), idx))
                .ToDictionary(x => x.Item1, x => x.idx, StringComparer.OrdinalIgnoreCase);

            results = filtered
                .OrderBy(i => priceOrderMap.TryGetValue(i.ShortName, out int order) ? order : 99)
                .ToList();
        }
        else
        {
    ...
    {
        if (value != null && (value.IsHeader || value.IsDivider)) return;

        if (value?.IsTemplates == true)
        {
    ...
    partial void OnSelectedTemplateChanged(ScreenerIndicatorTemplate? value) => RefreshSelectedTemplatePreview();

    private void RefreshSelectedTemplatePreview()
    {
        SelectedTemplateIndicatorNames.Clear();
        if (SelectedTemplate is not { } value) return;

        foreach (var entry in value.Entries)
        {
            SelectedTemplateIndicatorNames.Add(entry.DisplayName);
        }
    }
    ...
    partial void OnSelectedIndicatorChanged(ScreenerCatalogItem? value)
    {
        ...
        bool isLeft = ActiveTargetSide == TargetSide.Left || RightTargetMode != RightHandTargetMode.Indicator;
        BindToSide(value, isLeft);
    }
    ...
    private void BindToSide(ScreenerCatalogItem catalogItem, bool isLeft)
    {
        ...
        if (catalogItem.IndicatorType == IndicatorType.Price)
        {
            type = IndicatorType.Price;
            PriceType pt = Enum.TryParse<PriceType>(catalogItem.ShortName, out var p) ? p : PriceType.Close;
        ...
        else if (catalogItem.IndicatorType.HasValue)
        {
        ...
        int offset)
    {
        ...
        var config = new ScreenerIndicatorSideConfig
        {
            ...
            CustomDisplayName = (category == ScreenerItemCategoryType.Column || category == ScreenerItemCategoryType.Criteria || catalogItem?.IndicatorType == IndicatorType.Price)
                ? (catalogItem?.DisplayName ?? catalogItem?.ShortName ?? string.Empty)
                : string.Empty
        };
    ...
    private async Task SaveTemplateAsync()
    {
        ...
        if (!RegisteredEntries.Any()) return;
        ...
        await _templateCrud.SaveAsync(
            ...
            build: existing =>
            {
                var template = existing ?? new ScreenerIndicatorTemplate
                {
                    Id = Guid.NewGuid(),
                    Name = trimmedName,
                ...
                template.SetEntries(RegisteredEntries.Select(e => e.Clone()));
            ...
            onInvalid: validation => _logger.LogWarning(
                "Cannot save invalid screener template '{Name}': {Errors}", trimmedName, string.Join(", ", validation.Errors)),
            onError: ex => _logger.LogError(ex, "Failed to save screener template '{Name}'", NewTemplateName));
    }

    [RelayCommand]
    private async Task LoadTemplateAsync(ScreenerIndicatorTemplate? template)
    {
        ...
        await _templateCrud.ApplyAsync(
            ...
            apply: t =>
            {
                var newClones = t.Entries.Select(e => e.Clone()).ToList();
                RegisteredEntries.Clear();
                foreach (var entry in newClones)
                {
                    RegisteredEntries.Add(entry);
                }
            ...
            onInvalid: validation => _logger.LogWarning(
                "Cannot apply invalid screener template '{Name}': {Errors}", template.Name, string.Join(", ", validation.Errors)),
            onError: ex => _logger.LogError(ex, "Failed to load screener template '{Name}'", template.Name));
    }

    [RelayCommand]
    private async Task AppendTemplateAsync(ScreenerIndicatorTemplate? template)
    {
        if (_isDisposed || _templateCrud == null || _templateCrud.IsBusy || template == null) return;
        ...
        await _templateCrud.ApplyAsync(
            ...
            onInvalid: validation => _logger.LogWarning(
                "Cannot apply invalid screener template '{Name}': {Errors}", template.Name, string.Join(", ", validation.Errors)),
            onError: ex => _logger.LogError(ex, "Failed to append screener template '{Name}'", template.Name));
    }

    [RelayCommand]
    private async Task DeleteTemplateAsync(ScreenerIndicatorTemplate? template)
    {
        if (_isDisposed || _templateCrud == null || _templateCrud.IsBusy || template == null) return;
        ...
        await _templateCrud.DeleteAsync(
            ...
            onError: ex => _logger.LogError(
                ex, "Failed to delete screener template '{Name}' ({Id})", template.Name, template.Id));
    }
