...
namespace StockAnalyzer.Avalonia.Views.Dialogs
{
    public partial class FilterSettingsWindow : Window
    {
        ...
        {
            if (sender is ListBox listBox && listBox.SelectedItem is ColumnItemViewModel selectedField)
            {
                if (DataContext is FilterSettingsViewModel vm)
                {
                    var newRule = new FilterRuleViewModel(new StockAnalyzer.Core.Models.Settings.FilterRule { Field = selectedField.MemberName }, vm.AvailableFields);
