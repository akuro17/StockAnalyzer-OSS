...
public partial class NoteTimelineView : UserControl
{
    ...
    /// backend behavior. Also captures the TextBox's current CaretIndex and calls
    /// InsertImagePlaceholder first (Task E, Note Tab Enhancements), so Image Display Mode = Inline
    /// can later render the image at the exact position it was attached at, not just in the bottom
    /// thumbnail row.
    /// sa_minimal_fix (SAで制約確認 remediation, Finding 4): the file picker allows selecting
    /// multiple images at once (AllowMultiple=true). A prior version re-read
    /// ComposeBodyTextBox.CaretIndex from the live control on every loop iteration, assuming it would
    /// advance to track each prior iteration's InsertImagePlaceholder call (a programmatic Body
    /// mutation from the ViewModel side) - a headless-UI repro test proved Avalonia's bound TextBox
    /// does NOT advance CaretIndex for such a change (it stays at its pre-mutation value), so every
    /// image beyond the first landed at the same stale position, inserting BEFORE the previous one and
    /// reversing the selected order. Fixed by reading CaretIndex once, then tracking a local running
    /// offset advanced by each inserted token's own length - independent of any TextBox rebinding
    /// timing.
    /// </summary>
    private async void AttachImageButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        var composeBodyTextBox = this.FindControl<TextBox>("ComposeBodyTextBox");
        var insertionIndex = composeBodyTextBox?.CaretIndex ?? vm.Body.Length;
        foreach (var file in files)
        {
            ...
            vm.InsertImagePlaceholder(insertionIndex, localId);
            vm.AddPendingAttachment(memoryStream.ToArray(), file.Name, localId);
            insertionIndex += NoteImageTokenExtractor.Build(localId).Length;
        }
    }
}
