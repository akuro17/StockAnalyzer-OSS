...
public class ChartInteractionController
{
    ...
    private bool BeginHandleDrag(IChartObject obj, int handleIndex, global::Avalonia.Point point, ChartViewModel? viewModel)
    {
        ...
        LastDragPoint = point;
        if (obj is FixedRangeVolumeProfileObject frvp && frvp.LockRange)
        {
    ...
    public bool HandlePointerReleased(ChartViewModel? viewModel)
    {
        ...
        if (DraggedObject != null)
        {
            ...
            {
                var context = new DrawingCalculationContext(viewModel.Candles, viewModel.IndicatorResults, viewModel.Indicators, viewModel.Symbol);
                DeferredComputationRecalculator.TryRecalculate(objToNotify, context);
            }
        ...
        IReadOnlyList<CoreCandleData>? allCandles = null)
    {
            ...
            else if (DraggedObject is StockAnalyzer.Avalonia.Drawing.FixedRangeVolumeProfileObject frvp)
            {
                if (frvp.Points.Count >= 2 && DraggedHandleIndex >= 0 && DraggedHandleIndex < 2)
                {
                    if (frvp.LockRange && fullHistoryCandles != null && fullHistoryCandles.Count > 0)
                    {
                        ...
                        int p = FixedRangeVolumeProfileObject.FindNearestBarIndex(anchorTime, fullHistoryCandles);
                        int q = FixedRangeVolumeProfileObject.FindNearestBarIndex(chartPoint.Time, fullHistoryCandles);
                        int deltaBars = q - p;

                        frvp.Points[0] = _moveDragAnchorPoints[0];
                        frvp.Points[1] = _moveDragAnchorPoints[1];
                        frvp.TranslateBars(deltaBars, fullHistoryCandles);
                    }
                    else
                    {
        ...
        else
        {
            ...
            if (dragged is FixedRangeVolumeProfileObject frvp && frvp.LockRange && fullHistoryCandles != null && fullHistoryCandles.Count > 0 && _moveDragAnchorPoints != null && _moveDragAnchorPoints.Count >= 2)
            {
                int p = FixedRangeVolumeProfileObject.FindNearestBarIndex(anchorChart.Time, fullHistoryCandles);
                int q = FixedRangeVolumeProfileObject.FindNearestBarIndex(targetChart.Time, fullHistoryCandles);
                int deltaBars = q - p;
            ...
            if (dragged is StockAnalyzer.Avalonia.Drawing.Objects.SsaAnomalyHighlightObject ssaAnomalyMoved)
            {
                ssaAnomalyMoved.Recalculate(snapshot.Candles);
            }
            else if (dragged is not StockAnalyzer.Avalonia.Drawing.Objects.GeometricPatternObject &&
                     dragged is not StockAnalyzer.Avalonia.Drawing.Objects.HarmonicPatternObject &&
                     dragged is not StockAnalyzer.Avalonia.Drawing.Objects.AutoElliottWaveObject &&
                     dragged is not AnchoredVwapObject &&
                     dragged is not FixedRangeVolumeProfileObject &&
                     snapshot.Candles != null &&
                     DeferredComputationRecalculator.TryRecalculate(dragged, snapshot.Candles))
            {
            }
