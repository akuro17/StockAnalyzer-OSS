...
public class ChartInteractionController
{
        ...
        int activePanelIndex = -1)
    {
        ...
        if (modifiers.HasFlag(KeyModifiers.Shift))
        {
            ...
            var hitObject = viewModel.ObjectManager.GetObjectAt(chartPosition, coordinateTransform, _activePanelIndex);
            if (hitObject != null)
            {
    ...
    private void HandleEditObject(IChartObject hitObject, ChartViewModel viewModel)
    {
        ...
        _ = global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () => 
        {
            ...
            var result = await _dialogService.ShowDrawingSettingsDialogAsync(hitObject, onApply: obj =>
            {
            ...
            else if (result == DrawingSettingsResult.Changed)
            {
                if (viewModel.Candles != null)
                {
                    DeferredComputationRecalculator.TryRecalculate(hitObject, viewModel.Candles);
                }

                if (hitObject is StockAnalyzer.Avalonia.Drawing.Objects.DtwProjectionObject dtwObj)
                {
