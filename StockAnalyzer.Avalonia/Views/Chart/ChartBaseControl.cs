...
public class ChartBaseControl : Control, ICustomHitTest
{
    ...
    private readonly StockAnalyzer.Avalonia.Drawing.LayeredDrawingScene _layeredScene = new();
    private long _lastLayerRevision = -1;
    private long _lastObjectRevision = -1;
    private int _lastPanelRevision = -1;
    private StockAnalyzer.Avalonia.Drawing.ChartDrawingContextType? _lastDrawingContext;
    private readonly Dictionary<Guid, StockAnalyzer.Avalonia.Drawing.IChartObject> _cachedObjectsDict = new();
    private readonly FreehandPointerAdapter _freehandAdapter;

    private StockAnalyzer.Avalonia.Drawing.IChartObject? _currentDrawingObject
    {
        get => _freehandAdapter.IsActive ? _freehandAdapter.PreviewObject : _interactionController.CurrentDrawingObject;
    ...
    private readonly VolumeProfileViewportCoordinator _vpCoordinator;
    private readonly TimeAtPriceViewportCoordinator _tapCoordinator;

    // Layout from the most recent render pass, used to route pointer events to sub-window panels.
    private Renderers.ChartLayoutContext? _currentLayout;
    ...
    public ChartBaseControl()
    {
        ...
        var strokeCapacity = services?.GetService<StockAnalyzer.Core.Services.IStockAnalyzerSettings>()?.DrawingInteraction.MaxStrokeSamples;
        _freehandAdapter = strokeCapacity.HasValue ? new FreehandPointerAdapter(strokeCapacity.Value) : new FreehandPointerAdapter();
        ...
        var smartGuideService = services?.GetService<Services.Drawing.ISmartGuideService>();
        
        var indicatorRenderer = services?.GetService<IndicatorRenderer>() ?? new IndicatorRenderer();
        _vpCoordinator = services?.GetService<VolumeProfileViewportCoordinator>() ?? new VolumeProfileViewportCoordinator(services?.GetService<IDispatcherService>());
        indicatorRenderer.ViewportCoordinator = _vpCoordinator;
        _vpCoordinator.ResultReady += _ => InvalidateVisual();

        _tapCoordinator = services?.GetService<TimeAtPriceViewportCoordinator>() ?? new TimeAtPriceViewportCoordinator(services?.GetService<IDispatcherService>());
        indicatorRenderer.TimeAtPriceCoordinator = _tapCoordinator;
        _tapCoordinator.ResultReady += _ => InvalidateVisual();

        _pipeline = new ChartRenderPipeline(indicatorRenderer);
        
        _interactionController = (magnetService != null && dialogService != null)
            ? new ChartInteractionController(magnetService, dialogService, smartGuideService) 
            : new ChartInteractionController();
        _interactionController.LayeredScene = _layeredScene;
        ...
        _dataCoordinator.SnapshotUpdated += UpdateVolumeProfileCoordinator;
        _dataCoordinator.SnapshotUpdated += UpdateTimeAtPriceCoordinator;

        // Repaint when drawing object settings change (color, thickness, etc.)
        _interactionController.OnObjectEdited = () => InvalidateVisual();

        // Repaint when the selection-idle timer auto-deselects an object (hides its handles)
        _interactionController.OnSelectionIdleTimeout = () => InvalidateVisual();
        ...
        _interactionController.OnObjectDragged = (obj) =>
        {
            if (ViewModel != null && (obj is StockAnalyzer.Avalonia.Drawing.Objects.DtwProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.KalmanFilterProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.ArimaProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.FftProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HmmProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.SsaProjectionObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.SsaMultiComponentObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.SsaSupportResistanceObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.SsaAnomalyHighlightObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.AutoTimeCycleObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HarmonicPatternObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HoughAutoLinesObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HoughParabolicCurveObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HoughKeyLevelsObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HoughResonantFanObject
                || obj is StockAnalyzer.Avalonia.Drawing.Objects.HoughMagneticLineObject))
            {
    ...
    // The chart surface is drawn directly (no child controls), so the base Control returns a
    // NoneAutomationPeer, which the UIA bridge omits from the automation tree together with the
    // wrapper Border/Grid above it. Return a real peer so the chart area is a single addressable
    // node for assistive tech and UI automation (AutomationProperties.AutomationId="ChartCanvas").
    ...
    private void UnsubscribeFromViewModel()
    {
        if (_subscribedViewModel != null)
        {
            ...
            _subscribedViewModel.ObjectManager.Synced -= OnObjectManagerChanged;
    ...
    private void OnObjectManagerChanged()
    {
    ...
    private void OnViewModelPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        ...
        if (e.PropertyName == nameof(ChartViewModel.CurrentTool))
        {
            ...
            // Ensure the chart receives Key events (e.g. Escape) as soon as a drawing
            // tool is selected, without requiring a prior click on the chart area.
    ...
    protected void UpdateSnapshot(bool suppressMessages = false)
    {
        ...
        UpdateVolumeProfileCoordinator();
        UpdateTimeAtPriceCoordinator();
    }
    ...
    public override void Render(DrawingContext context)
    {
        ...
        if (ViewModel.ObjectManager?.InformationObject is { Snapshot: null, IsVisible: true } info &&
            ViewModel.Candles != null && ViewModel.Candles.Count > 0)
        {
            var targetCandle = ViewModel.Candles.LastOrDefault();
            if (targetCandle != null)
            {
                info.Snapshot = StockAnalyzer.Avalonia.Drawing.ChartInformationDataProvider.Extract(
                    ViewModel, targetCandle.Timestamp, targetCandle.Close);
            }
        }
    ...
    private void UpdateLayeredScene()
    {
        if (ViewModel == null) return;

        var layerService = ViewModel.LayerService;
        var currentContext = ViewModel.ObjectManager.CurrentContext;
        long layerRevision = layerService.Revision;
        long objectRevision = ViewModel.ObjectManager.Revision;
        int panelRevision = ViewModel.Indicators?.Count ?? 0;

        // Warm-path fast exit: If revisions and context have not changed, zero allocation and zero work!
        if (layerRevision == _lastLayerRevision &&
            objectRevision == _lastObjectRevision &&
            panelRevision == _lastPanelRevision &&
            currentContext == _lastDrawingContext)
        {
            return;
        }

        var objects = ViewModel.ObjectManager.Objects;
        _cachedObjectsDict.Clear();

        for (int i = 0; i < objects.Count; i++)
        {
            var obj = objects[i];
            _cachedObjectsDict[obj.Id] = obj;

            // Zero-allocation registration check
            if (!layerService.IsObjectRegistered(obj.Id))
            {
                var panelKey = obj.PanelIndex == -1
                    ? StockAnalyzer.Core.Models.Drawing.PanelKey.Main
                    : DrawingPanelResolver.TryResolvePanelKey(obj.PanelIndex, ViewModel.Indicators, ViewModel.IsSubWindowVisible) 
                      ?? StockAnalyzer.Core.Models.Drawing.PanelKey.OverlayGroup(obj.PanelIndex);
                layerService.AddObjectToLayer(obj.Id, panelKey);
            }
        }

        // Re-read layer revision in case any unassigned objects were registered
        layerRevision = layerService.Revision;

        _layeredScene.UpdateScene(
            currentContext,
            layerRevision,
            objectRevision,
            panelRevision,
            ...
            _cachedObjectsDict,
            panelKey => DrawingPanelResolver.IsPanelPresent(panelKey, ViewModel.Indicators, ViewModel.IsSubWindowVisible));

        _lastLayerRevision = layerRevision;
        _lastObjectRevision = objectRevision;
        _lastPanelRevision = panelRevision;
        _lastDrawingContext = currentContext;
    }
    ...
    /// <summary>
    /// Chooses which panel governs a pointer event: the origin panel of an in-progress gesture when
    /// one is active (<paramref name="activeGesturePanelIndex"/> is non-null), otherwise the panel
    /// currently under the pointer. The pin is symmetric — it covers the main chart
    /// (<c>-1</c>) exactly as it covers a sub-panel (<c>0..K-1</c>) — so straying the pointer over a
    /// different panel never swaps the coordinate transform mid-gesture, which would write the point
    /// back in the wrong value scale (indicator magnitude vs. price).
    /// </summary>
    internal static int ResolveGesturePanelIndex(int? activeGesturePanelIndex, int pointerPanelIndex)
        => activeGesturePanelIndex ?? pointerPanelIndex;

    /// <summary>
    /// Resolves which panel a pointer position targets and returns the coordinates + transform to
    /// hand to <see cref="ChartInteractionController"/>. When the target is the main chart (or a
    /// panel has no transform yet), this returns the existing main-chart values unchanged so
    /// main-chart interaction is byte-for-byte identical. <paramref name="activeGestureObject"/>,
    /// when set, pins the whole gesture to the panel it started on — the main chart included — for
    /// the gesture's entire lifetime, regardless of where the pointer strays.
    /// </summary>
    private (int PanelIndex, global::Avalonia.Point ChartPos, Drawing.ICoordinateTransform Transform, float MarginTop) ResolvePointerTarget(
        global::Avalonia.Point position, float renderMarginTop, Drawing.IChartObject? activeGestureObject)
    {
        var mainPos = new global::Avalonia.Point(position.X - EffectiveMarginLeft, position.Y - renderMarginTop);

        if (_currentLayout is not { } layout || layout.PanelAreas == null || layout.PanelAreas.Count == 0)
            return (-1, mainPos, _dataCoordinator.Transform, renderMarginTop);

        int panel = ResolveGesturePanelIndex(activeGestureObject?.PanelIndex, layout.ResolvePanelIndex(position));

        if (panel < 0 || panel >= layout.PanelAreas.Count)
            return (-1, mainPos, _dataCoordinator.Transform, renderMarginTop);

        var subTransform = _pipeline.SubWindowTransformCoordinator.GetTransform(panel);
        if (subTransform == null)
            return (-1, mainPos, _dataCoordinator.Transform, renderMarginTop);

        var pa = layout.PanelAreas[panel];
        var panelPos = new global::Avalonia.Point(position.X - EffectiveMarginLeft, position.Y - (float)pa.Top);
        return (panel, panelPos, subTransform, (float)pa.Top);
    }

    /// <summary>
    /// True for pointer classifications that are visible chart regions but NOT drawing surfaces —
    /// currently the reserved-but-empty volume band. Drawing hit-test, hover and shape creation are
    /// suppressed there; panning and the crosshair are not.
    /// </summary>
    private static bool IsNonDrawingSurface(int panelClass)
        => panelClass == ChartLayoutExtensions.VolumeAreaPanelIndex;
    ...
    protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        if (_freehandAdapter.IsActive)
        {
            _freehandAdapter.HandleCancel(DrawingCancelReason.ContextChanged, out _);
            _isDrawingNewShape = false;
        }
    ...
    protected override void OnPointerMoved(PointerEventArgs e)
    {
        ...
        // Captured samples use the same panel coordinates as Press and Release.
        // Clipping belongs to rendering; clamping here would alter the stroke.
        if (_freehandAdapter.IsActive)
        {
            _currentMousePosition = position;
            var freehandTarget = ResolvePointerTarget(position, renderMarginTop, _freehandAdapter.PreviewObject);

            if (_freehandAdapter.HandlePointerMoved(position, freehandTarget.ChartPos, e.Pointer.Id, freehandTarget.Transform, out bool freehandNeedsUpdate))
            {
                if (freehandNeedsUpdate)
                {
                    InvalidateVisual();
                }
                e.Handled = true;
        ...
        // Route the move to the panel of the in-progress gesture, or the panel under the pointer.
        var moveTarget = ResolvePointerTarget(position, renderMarginTop, _currentDrawingObject ?? _draggedObject);

        // The reserved-but-empty volume band (and true outside-gaps) are not drawing surfaces: skip
        // drawing hit-test / hover there so no phantom handle reacts on a main-chart object using a
        // volume-band Y. Panning (when already dragging) and the crosshair still run below.
        int panelClass = _currentLayout is { } moveLayout
            ? moveLayout.ResolvePanelIndex(position)
            : ChartLayoutExtensions.MainChartPanelIndex;
        bool suppressDrawingInteraction = IsNonDrawingSurface(panelClass)
            && _currentDrawingObject is null && _draggedObject is null
            && !_interactionController.IsDragging;

        // Delegate to Controller
        bool interactionHandled = false;
        bool needsUpdate = false;
        if (!suppressDrawingInteraction)
        {
            interactionHandled = _interactionController.HandlePointerMoved(
                position,
                moveTarget.ChartPos,
                ViewModel,
                ...
                new Rect(Bounds.Size),
                EffectiveMarginLeft,
                moveTarget.MarginTop,
                out needsUpdate,
                e.KeyModifiers,
                moveTarget.PanelIndex);
        }

        // Panning Application (Controller manages state, Control applies)
        if (!interactionHandled && _interactionController.IsDragging)
        {
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        // Before hit-testing: pick up drawings another tab persisted so the pressed object is a current one.
        // Not while a shape is being created: the preview is held by the interaction controller and only enters the
        // manager when the shape is completed, so that completing edit refreshes through DrawingEditCoordinator instead.
        if (!_interactionController.IsDrawingNewShape && !_freehandAdapter.IsActive)
        {
            ViewModel?.RefreshDrawingsIfStale();
        }
        ...
        {
             // Route the press to the panel under the pointer (main chart when panel < 0).
             var target = ResolvePointerTarget(position, renderMarginTop, _currentDrawingObject ?? _draggedObject);

             // A press on the reserved-but-empty volume band is not a drawing action: fall straight
             // through to Pan instead of a phantom pick/create (no gesture is in progress here).
             int pressPanelClass = _currentLayout is { } pressLayout
                 ? pressLayout.ResolvePanelIndex(position)
                 : ChartLayoutExtensions.MainChartPanelIndex;
             bool allowDrawingInteraction = !(IsNonDrawingSurface(pressPanelClass)
                 && _currentDrawingObject is null && _draggedObject is null);

             // Shift-click object deletion / modifier override takes precedence before drawing tools
             if (allowDrawingInteraction && e.KeyModifiers.HasFlag(KeyModifiers.Shift))
             {
                 if (_freehandAdapter.IsActive)
                 {
                     var shiftPanelKey = target.PanelIndex < 0
                         ? StockAnalyzer.Core.Models.Drawing.PanelKey.Main
                         : (StockAnalyzer.Avalonia.Drawing.DrawingPanelResolver.TryResolvePanelKey(target.PanelIndex, ViewModel?.Indicators, ViewModel?.IsSubWindowVisible ?? false)
                            ?? StockAnalyzer.Core.Models.Drawing.PanelKey.OverlayGroup(target.PanelIndex));

                     if (_freehandAdapter.HandleModifierComplete(ViewModel, target.PanelIndex, shiftPanelKey, out bool freehandNeedsUpdate))
                     {
                         _isDrawingNewShape = false;
                         if (ViewModel?.ObjectManager != null)
                         {
                             _interactionController.ArmOrResetSelectionIdleTimer(ViewModel.ObjectManager);
                         }
                         if (freehandNeedsUpdate)
                         {
                             InvalidateVisual();
                 ...
                 else if (_interactionController.HandlePointerPressed(
                     position,
                     target.ChartPos,
                     ViewModel,
                     target.Transform,
             ...
             // Freehand interaction routing:
             // 1. Existing selection handle drag takes precedence
             // 2. If no handle hit, route to FreehandPointerAdapter
             if (allowDrawingInteraction && ViewModel?.CurrentTool == DrawingTool.Freehand)
             {
                 if (_interactionController.TryHandleSelectionHandle(position, target.ChartPos, ViewModel, target.Transform))
                 {
                     _draggedObject = _interactionController.DraggedObject;
                     _draggedHandleIndex = _interactionController.DraggedHandleIndex;
                     InvalidateVisual();
                     e.Handled = true;
                     return;
                 }

                 var panelKey = target.PanelIndex < 0
                     ? StockAnalyzer.Core.Models.Drawing.PanelKey.Main
                     : (StockAnalyzer.Avalonia.Drawing.DrawingPanelResolver.TryResolvePanelKey(target.PanelIndex, ViewModel.Indicators, ViewModel.IsSubWindowVisible)
                        ?? StockAnalyzer.Core.Models.Drawing.PanelKey.OverlayGroup(target.PanelIndex));

                 if (_freehandAdapter.HandlePointerPressed(
                     position,
                     target.ChartPos,
                     e.Pointer.Id,
                     ViewModel,
                 ...
                 {
                     // R2-01: When CurrentTool is Freehand, adapter rejection or failure must NEVER fall
                     // through to legacy _interactionController.HandlePointerPressed or Pan.
                     // Consume the event strictly to prevent split/corrupted drawing state.
                     e.Handled = true;
                     return;
                 }
             }
             ...
             if (allowDrawingInteraction && _interactionController.HandlePointerPressed(
                 position,
                 target.ChartPos,
                 ViewModel,
                 target.Transform,
                 ...
                 target.PanelIndex))
             {
              ...
              _vpCoordinator.IsPointerCaptured = true;
              _tapCoordinator.IsPointerCaptured = true;
              e.Pointer.Capture(this);
              e.Handled = true;
         }
    ...
    private void ShowIndicatorContextMenu(CoreIndicatorSettings indicator, global::Avalonia.Point position)
    {
        ...
        {
            _ = ViewModel?.DialogService?.ShowIndicatorPropertiesDialogAsync(indicator, null, ViewModel?.Indicators);
        };
    ...
    private void UpdateVolumeProfileCoordinator()
    {
        if (_currentSnapshot?.IndicatorSettings == null)
        {
            _vpCoordinator.Reset();
            return;
        }

        // F05 fix: the previous FirstOrDefault picked only ONE VolumeProfile setting even when several
        // were registered, so every VP instance rendered that single one's result regardless of its own
        // Period/RowCount/Color (sa_analysis_report_VolumeProfile_PostImplementation_StrictReview_20260914.md
        // F05/A12). The coordinator now tracks one independent result per setting.Id, so every enabled VP
        // setting is requested here, and any Id no longer present (removed/disabled) is pruned so its
        // per-key timer/CTS does not leak forever.
        var vpSettings = _currentSnapshot.IndicatorSettings
            .Where(s => s.TypeEnum == IndicatorType.VolumeProfile && s.IsEnabled && s.ParameterObject is CoreVolumeProfileParameter)
            .ToList();

        _vpCoordinator.PruneStaleKeys(vpSettings.Select(s => s.Id));

        if (vpSettings.Count == 0)
        {
            _vpCoordinator.Reset();
            return;
        }

        // F06 fix: pass the ViewModel's data-generation counter through so an explicit reload/
        // correction that leaves the candle count and visible window unchanged still busts the
        // coordinator's per-key computation cache (sa_analysis_report_..._20260914.md F06).
        long dataRevision = ViewModel?.DataRevision ?? 0;
        foreach (var vpSetting in vpSettings)
        {
            _vpCoordinator.RequestUpdate(_currentSnapshot, vpSetting, dataRevision);
        }
    }

    private void UpdateTimeAtPriceCoordinator()
    {
        if (_currentSnapshot?.IndicatorSettings == null)
        {
            _tapCoordinator.Reset();
            return;
        }

        var tapSettings = _currentSnapshot.IndicatorSettings
            .Where(s => s.TypeEnum == IndicatorType.TimeAtPrice && s.IsEnabled && s.ParameterObject is CoreTimeAtPriceParameter)
            .ToList();

        _tapCoordinator.PruneStaleKeys(tapSettings.Select(s => s.Id));

        if (tapSettings.Count == 0)
        {
            _tapCoordinator.Reset();
            return;
        }
        ...
        long dataRevision = ViewModel?.DataRevision ?? 0;
        foreach (var tapSetting in tapSettings)
        {
            _tapCoordinator.RequestUpdate(_currentSnapshot, tapSetting, dataRevision);
        }
    }
    ...
    private void OnDoubleTappedHandler(object? sender, TappedEventArgs e)
    {
        ...
        float renderMarginTop = GetRenderMarginTop();
        var dblTarget = ResolvePointerTarget(pos, renderMarginTop, _currentDrawingObject ?? _draggedObject);

        _interactionController.HandleDoubleTap(pos, dblTarget.ChartPos, ViewModel, dblTarget.Transform, _currentSnapshot, e.KeyModifiers, dblTarget.PanelIndex);
    ...
    private void CancelCurrentDrawing()
    {
        ...
        // Same finalize-or-cancel logic as the physical Ctrl key press: for
        // variable-length tools (e.g. Elliott Wave/Polyline) this finalizes the
        // shape drawn so far (keeping it on the chart) instead of discarding it.
        if (ViewModel?.ObjectManager != null)
        {
            _interactionController.HandleCancelRequest(ViewModel.ObjectManager);
        }
        else
        {
            _interactionController.CancelDrawing();
        }
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        if (_freehandAdapter.IsActive && (e.Key == Key.LeftCtrl || e.Key == Key.RightCtrl || e.Key == Key.Escape))
        {
            var position = _currentMousePosition;
            float renderMarginTop = GetRenderMarginTop();
            var target = ResolvePointerTarget(position, renderMarginTop, _freehandAdapter.PreviewObject);
            var panelKey = target.PanelIndex < 0
                ? StockAnalyzer.Core.Models.Drawing.PanelKey.Main
                : (StockAnalyzer.Avalonia.Drawing.DrawingPanelResolver.TryResolvePanelKey(target.PanelIndex, ViewModel?.Indicators, ViewModel?.IsSubWindowVisible ?? false)
                   ?? StockAnalyzer.Core.Models.Drawing.PanelKey.OverlayGroup(target.PanelIndex));

            if (_freehandAdapter.HandleModifierComplete(ViewModel, target.PanelIndex, panelKey, out bool freehandNeedsUpdate))
            {
                _isDrawingNewShape = false;
                if (ViewModel?.ObjectManager != null)
                {
        ...
        if (_interactionController.HandleKeyDown(e, ViewModel))
        {
            InvalidateVisual();
            return;
        }
        ...
        if (e.Key == Key.LeftCtrl || e.Key == Key.RightCtrl)
        {
            if (ViewModel?.ObjectManager != null)
            {
                if (_interactionController.HandleCancelRequest(ViewModel.ObjectManager, ViewModel.LayerService, ViewModel.DrawingEditCoordinator))
                {
                    // Sync local state
                    _isDrawingNewShape = _interactionController.IsDrawingNewShape;
                    _currentDrawingObject = _interactionController.CurrentDrawingObject;
        ...
        // Escape: Revert to Pointer tool (cancels in-progress drawing, or simply deselects the active tool)
    ...
    protected override void OnPointerReleased(PointerReleasedEventArgs e)
    {
        ...
        _vpCoordinator.IsPointerCaptured = false;
        _vpCoordinator.NotifyPointerReleased(_currentSnapshot);
        _tapCoordinator.IsPointerCaptured = false;
        _tapCoordinator.NotifyPointerReleased(_currentSnapshot);

        if (_freehandAdapter.IsActive)
        {
            var releasePos = e.GetPosition(this);
            float renderMarginTop = GetRenderMarginTop();
            var releaseTarget = ResolvePointerTarget(releasePos, renderMarginTop, _freehandAdapter.PreviewObject);
            var panelKey = releaseTarget.PanelIndex < 0
                ? StockAnalyzer.Core.Models.Drawing.PanelKey.Main
                : (StockAnalyzer.Avalonia.Drawing.DrawingPanelResolver.TryResolvePanelKey(releaseTarget.PanelIndex, ViewModel?.Indicators, ViewModel?.IsSubWindowVisible ?? false)
                   ?? StockAnalyzer.Core.Models.Drawing.PanelKey.OverlayGroup(releaseTarget.PanelIndex));

            bool freehandNeedsUpdate = false;
            try
            {
                if (_freehandAdapter.HandlePointerReleased(
                    releasePos,
                    releaseTarget.ChartPos,
                    e.Pointer.Id,
                    ViewModel,
                    releaseTarget.Transform,
                    releaseTarget.PanelIndex,
                    panelKey,
                    out freehandNeedsUpdate))
                {
                    _isDrawingNewShape = false;
                    if (ViewModel?.ObjectManager != null)
                    {
    ...
    protected override void OnPointerCaptureLost(PointerCaptureLostEventArgs e)
    {
        ...
        // Only the pointer that owns the stroke can cancel it; another contact losing its capture is not the owner's capture loss.
        if (_freehandAdapter.IsActive && e.Pointer.Id == _freehandAdapter.Controller.OwnedPointerId)
        {
            _freehandAdapter.HandleCancel(DrawingCancelReason.CaptureLost, out bool needsUpdate);
            _isDrawingNewShape = false;
            if (needsUpdate)
            {
                InvalidateVisual();
