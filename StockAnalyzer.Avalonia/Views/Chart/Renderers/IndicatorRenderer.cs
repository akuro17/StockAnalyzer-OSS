...
public sealed class IndicatorRenderer : IChartRenderer, IDisposable
{
    ...
    public void RenderIndicator(SKCanvas canvas, Rect targetRect, ChartDataSnapshot snapshot, CoreIndicatorSettings setting, IChartRenderConfig config, bool isSubWindowContext = false, decimal? overrideMinValue = null, decimal? overrideMaxValue = null)
    {
        ...
        {
            interval = snapshot.Candles.Last().Timestamp - snapshot.Candles[snapshot.Candles.Count - 2].Timestamp;
        ...
        if (transform == null || isSubWindowContext)
        {
             ...
             {
                 var first = snapshot.Candles.First().Timestamp;
                 var last = snapshot.Candles.Last().Timestamp;
        ...
        if (snapshot.IndicatorResults != null && snapshot.IndicatorResults.TryGetValue(setting.Id, out var result))
        {
            ...
            if (!setting.IsOverlay && isSubWindowContext)
            {
                ...
                else if (transform is GenericCoordinateTransform gct)
                {
                    ...
                    if (snapshot.Candles!.Count > 0)
                    {
                        ...
                        var minT = snapshot.Candles!.First().Timestamp.Subtract(halfInt);
                        var maxT = snapshot.Candles!.Last().Timestamp.Add(halfInt);
            ...
            if (setting.TypeEnum == IndicatorType.VolumeProfile)
            {
                 ...
                 if (result.CustomData is List<Core.Analysis.VolumeBin> profile)
                 {
                     ...
                     _vpRenderer.Render(canvas, targetRect, profile, minVal, maxVal, transform, isRight, config);
                     ...
                     {
                          _vpRenderer.Render(canvas, targetRect, profile, minVal, maxVal, transform, false, config);
                     }
            ...
            if (snapshot.ChartType.IsIndexBased() && isSubWindowContext)
            {
                ...
                foreach (var seriesName in result.SeriesNames)
                {
                    if (!setting.ShowSignals && (seriesName == "BullishSignals" || seriesName == "BearishSignals" || seriesName == "BuySignals" || seriesName == "SellSignals"))
                        continue;
                    ...
                    SKColor seriesColor;
                    var colorConfig = setting.SeriesColors?.FirstOrDefault(c => c.TargetSeries.Contains(seriesName));
                    ...
                    if (isHistogram)
                    {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            histPaint.Color = val.Value >= 0 ? theme.Bullish.ToSkColor() : theme.Bearish.ToSkColor();
                            canvas.DrawRect(barX, Math.Min(y, zeroY), barWidth, Math.Abs(y - zeroY), histPaint);
                        }
                    }
                    else
                    {
                        ...
                        using var linePaint = new SKPaint
                        {
                            ...
                            StrokeWidth = (float)setting.Thickness,
                            IsAntialias = true,
                            Color = seriesColor
                        };
            ...
            foreach (var seriesName in result.SeriesNames)
            {
                // Filter Signal Markers (FR-39-12 toggle)
                if (!setting.ShowSignals && (seriesName == "BullishSignals" || seriesName == "BearishSignals" || seriesName == "BuySignals" || seriesName == "SellSignals"))
                    continue;
