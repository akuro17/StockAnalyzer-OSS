...
public sealed class MultiWavePatternRenderer : System.IDisposable
{
    ...
    // Reusable buffers for viewport-scoped Top-N confidence ranking (ZeroAllocation in steady state).
    private int[] _rankIndexBuffer = System.Array.Empty<int>();
    private double[] _rankScoreBuffer = System.Array.Empty<double>();
    private bool[] _includedBuffer = System.Array.Empty<bool>();
    ...
    public void Render(SKCanvas canvas, Rect chartArea, ChartDataSnapshot snapshot, IChartRenderConfig config)
    {
        ...
        var baseBullish = config.MultiWaveBullishColor ?? theme.Bullish;
        var baseBearish = config.MultiWaveBearishColor ?? theme.Bearish;
        ...
        // Pass 1: cull off-viewport signals first, then rank the on-screen subset by ConfidenceScore.
        // This ensures the Top-N limit reflects what is actually visible, not the whole historical buffer.
        EnsureBufferCapacity(signalCount);
        int maxLines = config.MultiWavePatternMaxLines;
        int visibleCount = 0;
        for (int s = 0; s < signalCount; s++)
        {
            ...
            _rankIndexBuffer[visibleCount] = s;
            _rankScoreBuffer[visibleCount] = signal.ConfidenceScore;
            visibleCount++;
        }

        bool applyLimit = maxLines > 0 && visibleCount > maxLines;
        if (applyLimit)
        {
            // Closure-free sort (ascending by ConfidenceScore); the top-N are the last `maxLines` entries.
            System.Array.Sort(_rankScoreBuffer, _rankIndexBuffer, 0, visibleCount);
        }

        for (int i = 0; i < signalCount; i++) _includedBuffer[i] = false;
        int rankStart = applyLimit ? visibleCount - maxLines : 0;
        for (int i = rankStart; i < visibleCount; i++) _includedBuffer[_rankIndexBuffer[i]] = true;

        for (int s = 0; s < signalCount; s++)
        {
            if (!_includedBuffer[s]) continue;
    ...
    {
        if (_rankIndexBuffer.Length >= count) return;
        int newSize = System.Math.Max(count, _rankIndexBuffer.Length * 2);
        _rankIndexBuffer = new int[newSize];
        _rankScoreBuffer = new double[newSize];
        _includedBuffer = new bool[newSize];
    }
