...
public static class TreeDataGridExtensions
{
    ...
    private static bool _isUpdatingColumns;
        ...
        if (source.Columns is System.Collections.Specialized.INotifyCollectionChanged colList)
        {
            void HookColumn(IColumn col)
            {
                if (col is System.ComponentModel.INotifyPropertyChanged npc)
                {
                    ...
                    {
                        if (_isUpdatingColumns) return; // Prevent reentrancy during programmatic sorting

                        if (args.PropertyName == "SortDirection" && sender is IColumn sortedCol)
                        {
                            var colId = GetColumnId(sortedCol);
                            if (colId != null)
                            {
                                var dir = sortedCol.SortDirection == System.ComponentModel.ListSortDirection.Ascending ? 1 : 
                                          sortedCol.SortDirection == System.ComponentModel.ListSortDirection.Descending ? 2 : 0;
            ...
            try 
            {
                ...
                foreach (var meta in sortedMeta)
                {
                    ...
                    else
                    {
                        ...
                        var header = localizationService?.GetString(meta.HeaderKey) ?? meta.HeaderKey;
                        var width = string.IsNullOrEmpty(meta.Width) || meta.Width == "Auto" 
                            ? GridLength.Auto : GridLength.Parse(meta.Width);
        ...
        switch (memberName)
        {
            case "IsChecked":
                return new TemplateColumn<WatchlistItemViewModel>(
                    ...
                    width: width);
            case "Symbol":
                return new TextColumn<WatchlistItemViewModel, string>(header, x => x.Symbol, width);
            case "Name":
                return new TextColumn<WatchlistItemViewModel, string>(header, x => x.Name, width);
            case "Change":
