...
public static class TreeDataGridExtensions
{
        ...
        switch (memberName)
        {
            ...
            case "Reminder":
                return new TemplateColumn<WatchlistItemViewModel>(
                    ...
                    cellTemplate: new FuncDataTemplate<WatchlistItemViewModel>((_, _) =>
                    {
                        return new StockAnalyzer.Avalonia.Views.Controls.ReminderCellControl();
                    }),
                    cellEditingTemplate: null,
                    options: new global::Avalonia.Controls.Models.TreeDataGrid.TemplateColumnOptions<WatchlistItemViewModel>
                    {
                        ...
                        MinWidth = new GridLength(WatchlistConstants.DefaultColumnMinWidth, GridUnitType.Pixel),
                        MaxWidth = new GridLength(WatchlistConstants.DefaultColumnMaxWidth, GridUnitType.Pixel),
                        CompareAscending = (a, b) => string.Compare(a?.Reminder, b?.Reminder, StringComparison.OrdinalIgnoreCase),
                        CompareDescending = (a, b) => string.Compare(b?.Reminder, a?.Reminder, StringComparison.OrdinalIgnoreCase)
                    },
                    width: width);
            case "Long":
        ...
        return new TemplateColumn<WatchlistItemViewModel>(
            ...
            cellTemplate: new FuncDataTemplate<WatchlistItemViewModel>((item, _) =>
            {
                ...
                if (item != null)
                {
                    ...
                    var formatted = FormatIndicatorValue(val);
                    tb.Text = formatted;
                    var tip = CreateTooltipContentTextBlock();
                    tip.Text = FormatTooltipValue(header, formatted);
                    tb.SetValue(ToolTip.TipProperty, tip);
                }
                return tb;
            }),
            cellEditingTemplate: null,
    ...
    /// <summary>
    /// Formats the header/value hover tooltip text shared by the Symbol/Name/Change/fundamental
    /// and indicator columns (Notes-column-equivalent popup): the header on its own line, followed
    /// by the exact same value the cell itself displays (no ": " separator).
    /// </summary>
    private static string FormatTooltipValue(string header, object? displayValue) =>
        $"{header}\n{(displayValue == null ? "-" : displayValue.ToString())}";

    /// <summary>
    /// Creates the shared tooltip-content TextBlock (margin + FontSize binding) used by every
    /// column-hover popup added for this feature. Built as an explicit control - rather than
    /// binding <c>ToolTip.FontSizeProperty</c> on the owner - because the owner-side property does
    /// not propagate into the ToolTip popup: the popup is rooted at the TopLevel, not the owner's
    /// visual tree, so it inherits FontSize from the Window (BaseFontSize) instead. Setting FontSize
    /// directly on the Tip content control (same pattern already used by TagCellControl's chip
    /// tooltip) is the mechanism that actually takes effect.
    /// </summary>
    private static global::Avalonia.Controls.TextBlock CreateTooltipContentTextBlock()
    {
        var tip = new global::Avalonia.Controls.TextBlock
        {
            Margin = new global::Avalonia.Thickness(WatchlistConstants.TooltipContentMargin)
        };
        tip.Bind(global::Avalonia.Controls.TextBlock.FontSizeProperty, tip.GetResourceObservable("TooltipFontSize"));
        return tip;
    }

    private static void AttachValueTooltip(global::Avalonia.Controls.TextBlock tb, string header, string displayPropertyName)
    {
        var tip = CreateTooltipContentTextBlock();
        tip.Bind(global::Avalonia.Controls.TextBlock.TextProperty, new global::Avalonia.Data.Binding(displayPropertyName)
        {
            Converter = new global::Avalonia.Data.Converters.FuncValueConverter<object?, string>(v => FormatTooltipValue(header, v))
        });
        tb.SetValue(ToolTip.TipProperty, tip);
    }
    ...
    private static IColumn CreateColoredColumn(string header, string memberName, GridLength width)
    {
        ...
        return new TemplateColumn<WatchlistItemViewModel>(
            ...
            cellTemplate: new FuncDataTemplate<WatchlistItemViewModel>((item, _) =>
            {
                ...
                AttachValueTooltip(tb, header, textProperty);
                return tb;
            }),
            cellEditingTemplate: null,
            width: width,
    ...
    private static IColumn CreateTextColumn(string header, string propertyName, GridLength width, bool leftAlign = false)
    {
        ...
        return new TemplateColumn<WatchlistItemViewModel>(
            ...
            cellTemplate: new FuncDataTemplate<WatchlistItemViewModel>((item, _) =>
            {
                ...
                AttachValueTooltip(tb, header, propertyName);
                return tb;
            }),
            cellEditingTemplate: null,
            width: width,
