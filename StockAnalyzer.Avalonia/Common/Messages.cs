...
/// This message must only be published from an explicit user action (currently the Notes cell's
/// "+" button in NotesCellControl) - never as a side effect of settings/workspace restoration -
/// because MainWindowViewModel's handler does not gate on <c>_isApplyingSettings</c> (removed:
/// that guard silently dropped the click while startup's PauseAutoSave scope was still open).
/// Sending it from a settings-restore code path would reintroduce that exact race.
/// </summary>
...
/// <summary>
/// Published when a chart's max candle count (period) changes.
/// </summary>
public class ChartPeriodChangedMessage : CommunityToolkit.Mvvm.Messaging.Messages.ValueChangedMessage<int>
{
    public object Sender { get; }
    public ChartPeriodChangedMessage(object sender, int value) : base(value)
    {
        Sender = sender;
    }
}
...
/// <summary>
/// Published when a chart's timeframe changes.
/// </summary>
public class ChartTimeframeChangedMessage : CommunityToolkit.Mvvm.Messaging.Messages.ValueChangedMessage<StockAnalyzer.Core.Models.TimeframeType>
{
    public object Sender { get; }
    public ChartTimeframeChangedMessage(object sender, StockAnalyzer.Core.Models.TimeframeType value) : base(value)
    {
        Sender = sender;
    }
}
...
/// <summary>
/// Request for the current active/main chart period (MaxCandleCount).
/// </summary>
public class CurrentPeriodRequestMessage : RequestMessage<int>
{
}

/// <summary>
/// Request for the current active/main chart timeframe (TimeframeType).
/// </summary>
public class CurrentTimeframeRequestMessage : RequestMessage<StockAnalyzer.Core.Models.TimeframeType>
{
}
