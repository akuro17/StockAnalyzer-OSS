...
public sealed class LocalizationManager
{
    ...
    /// <summary>
    /// <see cref="string.Format(string,object)"/> template for the embedded locale JSON resource of a
    /// given language code (arg 0), e.g. <c>StockAnalyzer.Avalonia.Resources.Locales.en.json</c>.
    /// Exposed so tests can enumerate/validate the shipped locales without duplicating this string.
    /// </summary>
    public const string LocaleResourceNameFormat = "StockAnalyzer.Avalonia.Resources.Locales.{0}.json";
    ...
    /// <summary>Raised after locale resources are reloaded, including a reload of the same language.</summary>
    ...
    /// <summary>
    /// Gets a localized string by key. An unknown key returns the key wrapped in brackets
    /// (<c>"[" + key + "]"</c>) so a missing translation is visible in the UI; a null or empty key
    /// returns <see cref="string.Empty"/>.
    /// </summary>
    /// <param name="key">The localization key</param>
    /// <returns>The localized string; <c>"[key]"</c> if the key is not found; empty string if <paramref name="key"/> is null or empty.</returns>
    ...
    private void LoadStrings(string languageCode, string? resourcePath)
    {
        try
        {
            ...
            var resourceName = string.Format(LocaleResourceNameFormat, languageCode);
