...
public class DialogService : IDialogService
{
    ...
    private double GetBaseFontSize() => _serviceProvider?.GetService<IFontSettingsManager>()?.BaseFontSize ?? FontDefaults.Base;
    private double GetTitleFontSize() => _serviceProvider?.GetService<IFontSettingsManager>()?.TitleFontSize ?? FontDefaults.Title;
    private double GetDetailFontSize() => _serviceProvider?.GetService<IFontSettingsManager>()?.DetailFontSize ?? FontDefaults.Detail;
    private double GetHelperFontSize() => _serviceProvider?.GetService<IFontSettingsManager>()?.HelperFontSize ?? FontDefaults.Helper;
    ...
    public async Task<DrawingSettingsResult> ShowDrawingSettingsDialogAsync(StockAnalyzer.Avalonia.Drawing.IChartObject drawing, Action<StockAnalyzer.Avalonia.Drawing.IChartObject>? onApply = null, StockAnalyzer.Avalonia.Services.Drawing.IDrawingEditCoordinator? coordinator = null, IReadOnlyList<CoreCandleData>? candles = null)
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
             return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
             {
                ...
                var dialog = new Views.Dialogs.DrawingSettingsDialog(drawing, registry, onApply, coordinator, candles);
    ...
    public async Task ShowIndicatorSettingsDialogAsync(IEnumerable<CoreIndicatorSettings> currentIndicators, Action<IEnumerable<CoreIndicatorSettings>>? onApply = null)
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                var sourceIndicatorService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.ISourceIndicatorService>();
                var driverService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.IDynamicPeriodDriverService>();
                var vm = new IndicatorSettingsDialogViewModel(this, indicatorFactory, toastService, templateService, null, sourceIndicatorService, null, driverService);
                try
                {
        ...
        IEnumerable<CoreIndicatorSettings>? allIndicators = null)
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                var sourceIndicatorService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.ISourceIndicatorService>();
                var driverService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.IDynamicPeriodDriverService>();
                var vm = new IndicatorPropertiesViewModel(indicator, messenger, dispatcher, allIndicators, this, sourceIndicatorService, driverService);
                vm.OnApplyCallback = onApply;
    ...
    public async Task ShowSourceIndicatorRegistrationDialogAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                var window = new Views.Dialogs.SourceIndicatorRegistrationWindow();
                var sourceIndicatorService = _serviceProvider?.GetRequiredService<StockAnalyzer.Core.Interfaces.ISourceIndicatorService>()
                    ?? throw new InvalidOperationException("ISourceIndicatorService must be registered in DI.");
                var indicatorFactory = _serviceProvider?.GetService<StockAnalyzer.Core.Models.Indicators.IIndicatorFactory>()
                    ?? StockAnalyzer.Core.Models.Indicators.IndicatorFactory.Default;
                var toastService = _serviceProvider?.GetService<IToastNotificationService>()
                    ?? throw new InvalidOperationException("IToastNotificationService must be registered in DI.");
                var templateService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.ITemplateService>();
                var logger = _serviceProvider?.GetService<ILogger<SourceIndicatorRegistrationViewModel>>();
                var dispatcher = _serviceProvider?.GetService<StockAnalyzer.Core.Services.IDispatcherService>();

                var vm = new SourceIndicatorRegistrationViewModel(sourceIndicatorService, indicatorFactory, toastService, templateService, dispatcher, logger);
                vm.RequestClose = () => window.Close();
                window.DataContext = vm;

                var owner = desktop.Windows.LastOrDefault(w => w.IsActive && w != window) ?? desktop.MainWindow!;
                try
                {
    ...
    public async Task ShowDynamicPeriodDriverRegistrationDialogAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                var window = new Views.Dialogs.DynamicPeriodDriverRegistrationWindow();
                var driverService = _serviceProvider?.GetRequiredService<StockAnalyzer.Core.Interfaces.IDynamicPeriodDriverService>()
                    ?? throw new InvalidOperationException("IDynamicPeriodDriverService must be registered in DI.");
                var indicatorFactory = _serviceProvider?.GetService<StockAnalyzer.Core.Models.Indicators.IIndicatorFactory>()
                    ?? StockAnalyzer.Core.Models.Indicators.IndicatorFactory.Default;
                var toastService = _serviceProvider?.GetService<IToastNotificationService>()
                    ?? throw new InvalidOperationException("IToastNotificationService must be registered in DI.");
                var templateService = _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.ITemplateService>();
                var logger = _serviceProvider?.GetService<ILogger<DynamicPeriodDriverRegistrationViewModel>>();
                var dispatcher = _serviceProvider?.GetService<StockAnalyzer.Core.Services.IDispatcherService>();

                var vm = new DynamicPeriodDriverRegistrationViewModel(driverService, indicatorFactory, toastService, templateService, dispatcher, logger);
                vm.RequestClose = () => window.Close();
                window.DataContext = vm;
        ...
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                var vm = new StockAnalyzer.Avalonia.ViewModels.Dialogs.EditTickerNotesDialogViewModel(
                    ...
                    _serviceProvider?.GetService<StockAnalyzer.Core.Interfaces.ILocalizationService>(),
                    _serviceProvider?.GetService<StockAnalyzer.Core.Models.Indicators.IIndicatorFactory>());
    ...
    public async Task ShowBacktestWindowAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                try
                {
                    var vm = _serviceProvider.GetRequiredService<ViewModels.Backtest.BacktestWindowViewModel>();
                    await vm.InitializeAsync();
                    var backtestWindow = new Views.Backtest.BacktestWindow { DataContext = vm };
                    backtestWindow.Closing += (_, _) => vm.OnWindowClosing();
                    backtestWindow.Show();
                }
                catch (Exception ex)
                {
                    _ = ShowAlertAsync(
                        LocalizationManager.Instance["Dialog_Error"] ?? "Error",
                        $"Failed to initialize Backtest window: {ex.Message}"
                    );
                }
            });
        }
    ...
    public async Task ShowTrainingWizardDialogAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(() =>
            {
                ...
                try
                {
                    var vm = _serviceProvider.GetRequiredService<TrainingWizardViewModel>();
                    var window = new TrainingWizardWindow { DataContext = vm };
                    // Cancels an in-flight run instead of leaving the trainer process running
                    // unattended if the user closes the window without clicking Cancel first.
                    window.Closed += (_, _) => vm.Dispose();
                    // Shown modelessly (no owner, no ShowDialog) so a long-running training job
                    // stays visible/actionable while the user keeps working in the main window,
                    // matching MultiSyncProgressWindow's existing long-running-task convention.
                    window.Show();
                }
                catch (Exception ex)
                {
                    _ = ShowAlertAsync(
                        LocalizationManager.Instance["Dialog_Error"] ?? "Error",
                        $"Failed to initialize Training Wizard: {ex.Message}"
                    );
                }
            });
        }
    ...
    public async Task<bool> ShowExportChartImageDialogAsync(ChartViewModel chartViewModel)
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop && desktop.MainWindow != null)
        {
            return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                // Single snapshot at dialog creation (modal dialog; snapshot/transform are frozen too).
                // Unregistered drawings => null scene => legacy path, without mutating the layer document.
                if (!StockAnalyzer.Avalonia.Drawing.LayeredDrawingSceneFactory.TryBuildSnapshotScene(
                        chartViewModel.ObjectManager,
                        chartViewModel.LayerService,
                        chartViewModel.Indicators,
                        chartViewModel.EffectiveIsSubWindowVisible,
                        out var layeredScene,
                        out _))
                {
                    layeredScene = null;
                }
    ...
    public async Task<StockAnalyzer.Core.Services.PythonSetupDecision> ShowPythonSetupConfirmationAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
             return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
             {
                ...
                manualHandler = new EventHandler<RoutedEventArgs>(async (sender, e) => 
                { 
                    cleanHandlers(); 
                    if (desktop.MainWindow?.Clipboard != null)
                    {
                        try
                        {
                            await desktop.MainWindow.Clipboard.SetTextAsync("pip install -r StockAnalyzer.Python/requirements.txt");
                        }
                        catch { }
                    }
                    result = StockAnalyzer.Core.Services.PythonSetupDecision.Manual; 
                    window.Close(); 
                });
    ...
    public async Task ShowManualSetupInstructionsAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            const string setupCommand = "pip install -r StockAnalyzer.Python/requirements.txt";
            if (desktop.MainWindow?.Clipboard != null)
            {
                try
                {
                    await desktop.MainWindow.Clipboard.SetTextAsync(setupCommand);
                }
                catch
                {
    ...
    public async Task<StockAnalyzer.Core.Services.PythonSetupDecision> ShowPythonUpdateConfirmationAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
             return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
             {
                ...
                btnManual.Click += async (_, _) => 
                { 
                    if (desktop.MainWindow?.Clipboard != null)
                    {
                        try
                        {
                            await desktop.MainWindow.Clipboard.SetTextAsync("pip install -r StockAnalyzer.Python/requirements.txt --upgrade");
                        }
                        catch { }
                    }
                    result = StockAnalyzer.Core.Services.PythonSetupDecision.Manual; 
    ...
    public async Task ShowPythonManualUpdateInstructionsAsync()
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
        {
            const string updateCommand = "pip install -r StockAnalyzer.Python/requirements.txt --upgrade";
            if (desktop.MainWindow?.Clipboard != null)
            {
                try
                {
                    await desktop.MainWindow.Clipboard.SetTextAsync(updateCommand);
                }
                catch
                {
                    // Best effort clipboard copy
