...
public class DialogService : IDialogService
{
    ...
    public async Task<(string? Symbol, bool RequestBulkImport)> ShowAddTickerDialogAsync(Guid targetProfileId)
    {
        if (Application.Current?.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop && desktop.MainWindow != null)
        {
            return await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                if (_serviceProvider == null) return (null, false);
                ...
                return (symbol, vm.IsBulkRequestRequested);
            });
        }
        return (null, false);
    }
            ...
            await global::Avalonia.Threading.Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                if (_serviceProvider != null)
                {
                    try 
                    {
                        ...
                        await screenerWindow.ShowDialog(desktop.MainWindow!);
                    }
                    catch (Exception ex)
                    {
                        await ShowAlertAsync(
