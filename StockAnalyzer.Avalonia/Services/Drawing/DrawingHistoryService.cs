...
public sealed class DrawingHistoryService : IDisposable
{
    ...
    public static bool AreSemanticallyEqual(FrozenContextState a, FrozenContextState b)
    {
        ...
        // Compare link groups (without this a pure link/unlink edit would be judged "no change" and never reach history)
        if (a.LinkGroups.Count != b.LinkGroups.Count) return false;
        for (int i = 0; i < a.LinkGroups.Count; i++)
        {
            var ga = a.LinkGroups[i];
            var gb = b.LinkGroups[i];

            if (ga.ParentObjectId != gb.ParentObjectId) return false;
            if (!ga.ChildObjectIds.SequenceEqual(gb.ChildObjectIds)) return false;
        }
