...
public sealed class DrawingDocumentSession
{
    ...
    {
        return _historyServices.GetOrAdd(contextType, _ => new DrawingHistoryService(Key));
    }
    ...
    public DrawingDocumentSession(DrawingDocumentKey key)
    {
        Key = key;
    }
...
public sealed class DrawingDocumentSessionStore
{
    ...
    {
        return _sessions.GetOrAdd(key, static k => new DrawingDocumentSession(k));
    }
    ...
    /// <summary>
    /// Enforces the global session history payload budget (default: 256 MiB).
    ...
    /// </summary>
    public int EnforceGlobalHistoryBudget(long maxGlobalBytes = DrawingInteractionLimits.MaxSessionHistoryPayloadBytes)
    {
        int evictedCount = 0;
        while (TotalHistoryPayloadBytes > maxGlobalBytes)
        {
