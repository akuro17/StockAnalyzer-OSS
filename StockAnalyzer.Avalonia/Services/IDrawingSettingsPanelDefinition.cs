...
public interface IDrawingSettingsPanelDefinition
{
    ...
    /// <summary>Requests the shared metadata-driven editor in addition to the generic controls.</summary>
    bool UsesDynamicSettings => false;

    /// <summary>Metadata tags hidden by the shared editor when <see cref="UsesDynamicSettings"/> is true.</summary>
    IReadOnlyCollection<string>? DynamicHiddenParameterTags => null;

    /// <summary>Whether this tool uses the shared stroke Color control (GenericColorPanel). Default: true.</summary>
    bool SupportsStrokeColor => true;

    /// <summary>Whether this tool uses the shared Line Thickness control (ThicknessPanel). Default: true.</summary>
    bool SupportsStrokeThickness => true;
