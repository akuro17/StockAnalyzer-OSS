...
public sealed class WorkspaceCoordinator : IWorkspaceCoordinator
{
    ...
    public async Task InitializeWorkspaceAsync(string? customPath = null)
    {
        ...
        try
        {
            ...
            {
                s.Coord.RestoreLayoutSettings(s.Settings);
                s.Coord.RestoreTabCollections(s.Settings);
                s.Coord.RestoreDetachedWindows(s.Settings);
                s.Coord.RestoreWatchlistAndColumns(s.Settings);
    ...
    private void RestoreLayoutSettings(WorkspaceSettings? settings)
    {
        ...
        if (settings == null)
        {
            ...
            // Fallback: Initialize default indicators if none exist in the main chart view model
            if (_viewModel.GetIndicators().Count == 0)
            {
                var defaults = DefaultCoreIndicatorSettings.GetDefault();
                var toApply = new List<CoreIndicatorSettings>();
                foreach (var indicator in defaults)
                {
                    if (indicator.TypeEnum == IndicatorType.SMA || 
                        indicator.TypeEnum == IndicatorType.Volume)
                    {
                        indicator.IsEnabled = true;
                        toApply.Add(indicator);
                    }
                }
                _viewModel.ApplyIndicators(toApply);
            }
            return;
        }
