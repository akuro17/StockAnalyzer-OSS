...
public sealed class WorkspaceCoordinator : IWorkspaceCoordinator
{
    ...
    private void RestoreLayoutSettings(WorkspaceSettings? settings)
    {
        ...
        // Restore indicators. Each indicator is healed in isolation so that one corrupted
        // or schema-mismatched entry (e.g. a workspace file saved by a newer build) is
        // skipped and logged rather than throwing out of this method -- which would abort
        // the *entire* workspace restore (ticker, tabs, watchlists included, not just
        // indicators) via InitializeWorkspaceAsync's outer catch/fallback.
        if (settings.Indicators != null)
        {
            var healthyIndicators = new List<CoreIndicatorSettings>();
            foreach (var indicator in settings.Indicators)
            {
                if (indicator == null)
                {
                    _logger.LogWarning("Skipping null indicator entry in workspace settings.");
                    continue;
                }
                ...
                try
                {
                    ...
                    healthyIndicators.Add(indicator);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Failed to restore indicator {DisplayName} (TypeEnum={TypeEnum}); skipping it.", indicator.DisplayName, indicator.TypeEnum);
                }
            }
            _viewModel.ApplyIndicators(healthyIndicators);
        }
        else
        {
    ...
    private void RestoreWatchlistAndColumns(WorkspaceSettings settings)
    {
        ...
        if (settings.WatchlistProfiles != null)
        {
            ...
            // Must run before ImportFilterSettings: a saved FilterNode's ParentId can point at a
            // custom category or list folder (Add Filter is available on both), and
            // ImportFilterSettings resolves ParentId via FindNodeById(Groups, ...) - if that parent
            // doesn't exist in Groups yet, the filter is silently dropped (and, since it never gets
            // re-attached, is permanently lost on the very next workspace save).
            if (settings.CustomTickerCategories != null)
            {
                _viewModel.ImportCustomTickerCategories(settings.CustomTickerCategories);
            }

            if (settings.TagFilters != null)
            {
                _logger.LogInformation("Restoring {Count} TagFilters from workspace settings.", settings.TagFilters.Count);
                _viewModel.ImportFilterSettings(settings.TagFilters);
            }
            else
            {
                _logger.LogInformation("Workspace settings has no TagFilters to restore.");
            }
        }
        else
        {
            _logger.LogInformation("WatchlistProfiles is null; skipping watchlist/filter restoration.");
        }
