...
public partial class App : Application, IDisposable, IThemeVariantDispatcher
{
    ...
    /// <summary>
    /// Gets the current <see cref="App"/> instance in use. Returns null instead of throwing when
    /// <see cref="Application.Current"/> is unset or is a different Application type (e.g. the
    /// headless Application that Avalonia.Headless.XUnit's [AvaloniaFact] installs process-wide
    /// for UI tests) — most callers already guard access with <c>App.Current?.</c>.
    /// </summary>
    ...
    public override void Initialize()
    {
        ...
        StockAnalyzer.Avalonia.Services.FontSettingsManager.SeedDefaultResources(Resources);
    }
        ...
        try
        {
            if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
            {
                ...
                if (Services == null)
                {
                    ...
                    StockAnalyzer.Core.Services.UserStrategyMetadataRepository.MarketDataProvider =
                        Services.GetRequiredService<StockAnalyzer.Core.Services.IMarketDataProvider>() as StockAnalyzer.Core.Services.ParquetMarketDataProvider;
                    ...
                    // Awaited synchronously (small local JSON read) so IsUpdateSuppressed is
                    // guaranteed set before any Python-dependent UI action can trigger
                    // PythonService.InitializeAsync(), which reads it - a fire-and-forget
                    // ContinueWith here would race against that lazily-triggered call. Run on a
                    // thread-pool thread via Task.Run rather than blocking directly: this method
                    // executes on the Avalonia UI thread, whose SynchronizationContext would
                    // otherwise be captured by the awaits inside LoadAsync() and never get a
                    // chance to run their continuations while this same thread sits blocked on
                    // GetResult() - a classic sync-over-async deadlock that hung app startup.
                    var pythonSettingsManager = Services.GetRequiredService<IPythonSettingsManager>();
                    Task.Run(() => pythonSettingsManager.LoadAsync()).GetAwaiter().GetResult();
                    if (!pythonSettingsManager.ShowUpdateConfirmationDialog)
                    {
                        pythonService.IsUpdateSuppressed = true;
                    }
                ...
                Drawing.DrawingThemeContext.Initialize(themeManager, fontSettingsManager, chartSettingsManager, iconDrawingService);
                ...
                // Ticker Note spec section 4.5: detect (never auto-delete) orphaned attachment
                // files once per launch, in the background so it never delays startup. The result
                // is stashed in OrphanedAttachmentScanResultHolder for the trash dialog's "Orphaned
                // Files" tab; a status-bar notice is only raised when there's actually something to
                // report (MainWindowViewModel.Receive(OrphanedAttachmentsDetectedMessage)).
                _ = Task.Run(async () =>
                {
                    try
                    {
                        var cleanupService = Services.GetRequiredService<StockAnalyzer.Core.Services.Notes.OrphanedAttachmentCleanupService>();
                        var resultHolder = Services.GetRequiredService<StockAnalyzer.Core.Services.Notes.OrphanedAttachmentScanResultHolder>();
                        var report = await cleanupService.DetectOrphansAsync().ConfigureAwait(false);
                        resultHolder.SetLatestReport(report);

                        if (report.HasOrphans)
                        {
                            global::Avalonia.Threading.Dispatcher.UIThread.Post(() =>
                                WeakReferenceMessenger.Default.Send(
                                    new StockAnalyzer.Avalonia.Common.OrphanedAttachmentsDetectedMessage(report.OrphanedFileNames.Count)));
                        }
                    }
                    catch
                    {
    ...
    private void SyncThemeResources(ThemeColors colors)
    {
        ...
        var secText = ToAvColor(colors.ShellSecondaryText);
        Resources["Color.Text.Primary"] = text;
        Resources["Color.Foreground.Primary"] = text;
        Resources["Color.Text.Secondary"] = secText;
        Resources["Color.Foreground.Secondary"] = secText;
        SetOrUpdateBrush("Brush.Text.Secondary", secText);
