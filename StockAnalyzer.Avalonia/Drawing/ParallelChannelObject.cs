...
public class ParallelChannelObject : IChartObject
{
    ...
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        if (IsSelected)
        {
            ...
            canvas.DrawCircle((float)s1.X, (float)s1.Y, 5, selPaint);
            canvas.DrawCircle((float)s2.X, (float)s2.Y, 5, selPaint);
            canvas.DrawCircle((float)s3.X, (float)s3.Y, 5, selPaint);
        }
    }
