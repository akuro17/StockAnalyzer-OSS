...
public class SsaProjectionRenderer
{
    public void Render(SKCanvas canvas, IChartObject obj, ICoordinateTransform transform, bool isSelected)
    {
        if (obj is not SsaProjectionObject ssaObj || ssaObj.Points.Count < 2) return;
        ...
        // 3.5 Draw In-Sample Reconstructed Curve (if enabled)
        if (ssaObj.ShowReconstructedPath && ssaObj.ReconstructedPath != null && ssaObj.ReconstructedPath.Count > 1)
        {
            using var reconPaint = new SKPaint
            {
                Color = activeColor.WithAlpha(210),
                Style = SKPaintStyle.Stroke,
                StrokeWidth = (float)Math.Max(1.0, ssaObj.Thickness),
                IsAntialias = true
            };

            using var reconPath = new SKPath();
            var firstRecon = ssaObj.ReconstructedPath[0];
            var rStart = transform.ChartToScreen(new ChartPoint(new DateTime((long)firstRecon.X), (decimal)firstRecon.Y));
            reconPath.MoveTo((float)rStart.X, (float)rStart.Y);

            for (int i = 1; i < ssaObj.ReconstructedPath.Count; i++)
            {
                var pt = ssaObj.ReconstructedPath[i];
                var sPt = transform.ChartToScreen(new ChartPoint(new DateTime((long)pt.X), (decimal)pt.Y));
                reconPath.LineTo((float)sPt.X, (float)sPt.Y);
            }

            canvas.DrawPath(reconPath, reconPaint);
        }

        // 4. Draw Uncertainty Band (if enabled)
        if (ssaObj.ShowConfidenceBand &&
            ssaObj.UpperBandPath != null && ssaObj.UpperBandPath.Count > 1 &&
            ssaObj.LowerBandPath != null && ssaObj.LowerBandPath.Count > 1)
        {
            // Filled fan area
            using var bandPath = new SKPath();
            var firstUpper = ssaObj.UpperBandPath[0];
            ...
            for (int i = 1; i < ssaObj.UpperBandPath.Count; i++)
            {
                var pt = ssaObj.UpperBandPath[i];
                var sPt = transform.ChartToScreen(new ChartPoint(new DateTime((long)pt.X), (decimal)pt.Y));
                bandPath.LineTo((float)sPt.X, (float)sPt.Y);
            }

            for (int i = ssaObj.LowerBandPath.Count - 1; i >= 0; i--)
            {
                var pt = ssaObj.LowerBandPath[i];
                var sPt = transform.ChartToScreen(new ChartPoint(new DateTime((long)pt.X), (decimal)pt.Y));
                bandPath.LineTo((float)sPt.X, (float)sPt.Y);
            }
            bandPath.Close();
            ...
            using var boundaryPaint = new SKPaint
            {
                ...
                Style = SKPaintStyle.Stroke,
                StrokeWidth = (float)Math.Max(1.0, ssaObj.Thickness * 0.75),
                IsAntialias = true,
            ...
            var firstLower = ssaObj.LowerBandPath[0];
            ...
            for (int i = 1; i < ssaObj.LowerBandPath.Count; i++)
            {
                var pt = ssaObj.LowerBandPath[i];
                var sPt = transform.ChartToScreen(new ChartPoint(new DateTime((long)pt.X), (decimal)pt.Y));
        ...
        if (ssaObj.ProjectedPath != null && ssaObj.ProjectedPath.Count > 1)
        {
            using var pathPaint = new SKPaint
            {
                Color = ssaObj.SkiaColor,
                Style = SKPaintStyle.Stroke,
                StrokeWidth = isSelected ? (float)ssaObj.Thickness + 1.5f : (float)ssaObj.Thickness + 0.5f,
                IsAntialias = true,
                PathEffect = SKPathEffect.CreateDash(new float[] { 5, 5 }, 0)
            };

            using var path = new SKPath();
            var firstVal = ssaObj.ProjectedPath[0];
            ...
            for (int i = 1; i < ssaObj.ProjectedPath.Count; i++)
            {
                var val = ssaObj.ProjectedPath[i];
