...
public class HarmonicPatternRenderer : IDisposable
{
    ...
    {
        // Background
        _bgPaint.Color = new SKColor(hObj.SkiaColor.Red, hObj.SkiaColor.Green, hObj.SkiaColor.Blue, 30);
        canvas.DrawRect(rect, _bgPaint);

        // Border (same stroke-width behavior as RectangleObject: use Thickness directly,
        // no extra selection-time boost)
        _borderPaint.Color = isSelected ? hObj.SkiaColor : hObj.SkiaColor.WithAlpha(100);
        _borderPaint.StrokeWidth = (float)hObj.Thickness;
