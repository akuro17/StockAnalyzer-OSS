...
public sealed class TimeAtPriceBehavior : TwoClickBehavior<TimeAtPriceObject>
{
    protected override TimeAtPriceObject CreateInstance(ChartPoint p, IEnumerable<CoreCandleData>? _)
        => new TimeAtPriceObject(p, p);

    protected override void OnPointUpdated(TimeAtPriceObject obj, ChartPoint chartPoint, IEnumerable<CoreCandleData>? candles)
    {
        if (candles != null) obj.Recalculate(candles);
    }
}
