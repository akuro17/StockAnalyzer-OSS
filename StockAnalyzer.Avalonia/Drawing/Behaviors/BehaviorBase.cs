...
public abstract class ThreePointBehavior<T> : IDrawingToolBehavior where T : IChartObject
{
    ...
    public void UpdatePoint(IChartObject obj, int drawingStep, ChartPoint chartPoint, IEnumerable<CoreCandleData>? candles = null)
    {
        ...
        OnPointUpdated((T)obj, drawingStep, chartPoint, candles);
    }
    ...
    /// <summary>
    /// Optional per-step customization hook, invoked after the default mirroring logic above.
    /// Override to replace a point with tool-specific derived geometry (e.g., midpoint interpolation,
    /// locked axis) instead of the raw cursor position.
    /// </summary>
    protected virtual void OnPointUpdated(T obj, int drawingStep, ChartPoint chartPoint, IEnumerable<CoreCandleData>? candles) { }
}
