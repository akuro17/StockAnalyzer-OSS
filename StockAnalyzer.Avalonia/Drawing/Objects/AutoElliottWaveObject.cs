...
/// </summary>
public class AutoElliottWaveObject : IChartObject, IDisposable, IDrawingCalculatedValuesProvider
{
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        ElliottWaveResult wave = (HoveredResultIndex >= 0 && HoveredResultIndex < CachedResults.Count)
        ...
        string typeStr = (wave.IsImpulse ? "Impulse (1-5)" : "Corrective (A-C)") + (wave.IsBullish ? " Bullish" : " Bearish");
        decimal lastPrice = wave.WavePoints.Count > 0 ? wave.WavePoints[^1].Price : 0m;

        return new DrawingCalculatedValue[]
        {
            new DrawingCalculatedValue("WaveType", "Wave Type", null, typeStr, color),
            new DrawingCalculatedValue("Phase", "Phase", null, wave.CurrentPhase.ToString(), color),
            new DrawingCalculatedValue("Confidence", "Confidence", (decimal)wave.ConfidenceScore, $"{wave.ConfidenceScore * 100:F1}%", color),
            new DrawingCalculatedValue("LastPoint", "End Price", lastPrice, $"{lastPrice:F2}", IndicatorColor.Gray)
        };
    }
}
