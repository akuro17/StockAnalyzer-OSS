...
/// </summary>
public class CurveTrendObject : RelativeGeometricRenderer, IDrawingCalculatedValuesProvider
{
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        var p1 = Points[1]; // End
        var p2 = Points[2]; // Control Point
        var color = new IndicatorColor(Color.A, Color.R, Color.G, Color.B);

        double dt = (p1.Time - p0.Time).TotalSeconds;
        decimal curvePrice = p0.Price;

        if (Math.Abs(dt) > 1e-4)
        {
            double t = (timestamp - p0.Time).TotalSeconds / dt;
            t = Math.Clamp(t, 0.0, 1.0);
            decimal u = (decimal)(1.0 - t);
            decimal tt = (decimal)t;
            curvePrice = (u * u) * p0.Price + 2m * (u * tt) * p2.Price + (tt * tt) * p1.Price;
        }

        decimal delta = p1.Price - p0.Price;
        decimal baselineMid = (p0.Price + p1.Price) / 2m;
        decimal curvature = p2.Price - baselineMid;

        return new DrawingCalculatedValue[]
        {
            new DrawingCalculatedValue("CurvePrice", "Curve Price", curvePrice, $"{curvePrice:F3}", color),
            ...
            new DrawingCalculatedValue("Curvature", "Curvature Offset", curvature, $"{curvature:+0.000;-0.000;0.000}", IndicatorColor.Gray)
        };
    }
}
