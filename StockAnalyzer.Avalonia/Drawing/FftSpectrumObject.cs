...
public class FftSpectrumObject : IChartObject, IDisposable, IDrawingCalculatedValuesProvider
{
    ...
    public PriceField PriceField { get; set; } = PriceField.MedianHL;
    ...
    public void Recalculate(IEnumerable<CoreCandleData> candles)
    {
        ...
        Func<CoreCandleData, double> selector = PriceField switch
        {
            PriceField.Close => c => (double)c.Close,
            PriceField.Open => c => (double)c.Open,
            PriceField.High => c => (double)c.High,
            PriceField.Low => c => (double)c.Low,
            PriceField.TypicalHLC => c => (double)((c.High + c.Low + c.Close) / 3m),
            PriceField.WeightedHLCC => c => (double)((c.High + c.Low + 2m * c.Close) / 4m),
            _ => c => (double)((c.High + c.Low) / 2m) // MedianHL
        };
