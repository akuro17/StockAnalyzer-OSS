...
internal static class ChartObjectInstanceFactory
{
    ...
    private static object? BuildPlaceholder(Type parameterType)
    {
        ...
        if (parameterlessCtor != null) return Activator.CreateInstance(parameterType);

        // Covers IEnumerable<T>, IReadOnlyList<T>, IReadOnlyCollection<T>
        if (parameterType.IsInterface && parameterType.IsGenericType)
        {
            var genDef = parameterType.GetGenericTypeDefinition();
            if (genDef == typeof(System.Collections.Generic.IEnumerable<>) ||
                genDef == typeof(System.Collections.Generic.IReadOnlyList<>) ||
                genDef == typeof(System.Collections.Generic.IReadOnlyCollection<>))
            {
                var elemType = parameterType.GetGenericArguments()[0];
