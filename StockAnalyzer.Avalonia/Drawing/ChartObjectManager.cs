...
public class ChartObjectManager
{
    ...
    // Link groups (one registry per context, mirroring _contextStore)
    private readonly Dictionary<ChartDrawingContextType, ObjectLinkGroupRegistry> _linkRegistries
        = new Dictionary<ChartDrawingContextType, ObjectLinkGroupRegistry>();
    private ObjectLinkGroupRegistry _currentLinks = new ObjectLinkGroupRegistry();
    ...
    public ChartObjectManager()
    {
        ...
        foreach (ChartDrawingContextType type in Enum.GetValues(typeof(ChartDrawingContextType)))
        {
            ...
            _linkRegistries[type] = new ObjectLinkGroupRegistry();
        }
        _currentObjects = _contextStore[_currentContext];
        _currentLinks = _linkRegistries[_currentContext];
    }
    ...
    private Objects.InformationObject? _informationObject;

    /// <summary>
    /// Gets the current InformationObject instance if one exists in the active context, otherwise null.
    /// Provides O(1) allocation-free access for high-frequency render/hover paths.
    /// </summary>
    public Objects.InformationObject? InformationObject
    {
        get
        {
            if (_informationObject != null) return _informationObject;
            for (int i = 0; i < _currentObjects.Count; i++)
            {
                if (_currentObjects[i] is Objects.InformationObject info)
                {
                    _informationObject = info;
                    return info;
                }
            }
            return null;
    ...
    /// <summary>
    /// Monotonically increasing revision counter incremented on every object mutation or context switch.
    ...
    public void SwitchContext(ChartDrawingContextType newContext)
    {
        ...
        _currentLinks = GetLinkRegistry(newContext);
        RecalculateZIndices();
        _lockedIds.Clear();
        _hiddenIds.Clear();
    ...
    public bool AddObject(IChartObject chartObject)
    {
        ...
        if (chartObject is Objects.InformationObject info)
        {
            if (InformationObject != null) return false;
            _informationObject = info;
        }
    ...
    public bool RemoveObject(Guid id)
    {
        ...
        if (_currentObjects[index] is Objects.InformationObject)
        {
            _informationObject = null;
        }

        IChartObject removedObject = _currentObjects[index];
        _currentObjects.RemoveAt(index);
        _selectedIds.Remove(id);
        _lockedIds.Remove(id);
        _hiddenIds.Remove(id);
        _currentLinks.OnObjectRemoved(id);
        RecalculateZIndices();
        NotifyChanged();
        DisposeIfNoLongerManaged(removedObject);
        return true;
    }
    ...
    /// If targetPanelIndex is specified, only objects belonging to that panel are evaluated.
    /// </summary>
    public IChartObject? GetObjectAt(global::Avalonia.Point screenPoint, ICoordinateTransform transform, int? targetPanelIndex = null)
    {
        var list = _currentObjects;
        for (int i = list.Count - 1; i >= 0; i--)
        {
            var obj = list[i];
            if (targetPanelIndex.HasValue && obj.PanelIndex != targetPanelIndex.Value) continue;
    ...
    /// <summary>
    /// Render all visible objects in current context belonging to the specified panel (sorted by ZIndex).
    /// panelIndex: -1 for Main Chart, 0..K-1 for SubWindow panels.
    /// </summary>
    public void RenderPanel(SKCanvas canvas, int panelIndex, ICoordinateTransform transform)
    {
        var list = _currentObjects;
        ...
        {
            var obj = list[i];
            if (obj.PanelIndex == panelIndex && IsVisible(obj.Id))
            {
    ...
    /// Defaults to Main Chart objects (PanelIndex == -1) for backwards compatibility.
    /// </summary>
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        RenderPanel(canvas, -1, transform);
    }
    ...
    /// </summary>
    internal static IReadOnlyList<ChartPoint> GetAnchorCandidatePoints(IChartObject obj)
    {
    ...
    {
        var retiredObjects = new List<IChartObject>();
        using (BeginBatch())
        {
            for (int i = _currentObjects.Count - 1; i >= 0; i--)
            {
                ...
                if (!IsLocked(obj.Id))
                {
                    if (obj is Objects.InformationObject)
                    {
                        _informationObject = null;
                    }
                    _selectedIds.Remove(obj.Id);
                    ...
                    _currentLinks.OnObjectRemoved(obj.Id);
        ...
        DisposeRetiredObjects(retiredObjects);
    }
    ...
    public void Clear()
    {
        ...
        {
            AddRetiredObjects(retiredObjects, contextList);
            contextList.Clear();
        }
        foreach (var registry in _linkRegistries.Values)
        {
            registry.Clear();
        }
        DisposeRetiredObjects(retiredObjects);
    ...
    private ObjectLinkGroupRegistry GetLinkRegistry(ChartDrawingContextType context)
    {
        if (!_linkRegistries.TryGetValue(context, out var registry))
        {
            registry = new ObjectLinkGroupRegistry();
            _linkRegistries[context] = registry;
        }
        return registry;
    }

    /// <summary>
    /// Links <paramref name="childIds"/> under <paramref name="parentId"/> in the current context.
    /// Every id must exist in the current context. Fires <see cref="Changed"/> only when the link state changed.
    /// </summary>
    public LinkResult LinkObjects(Guid parentId, IReadOnlyList<Guid> childIds)
    {
        if (GetObject(parentId) == null) return LinkResult.UnknownObject;
        if (childIds != null)
        {
            for (int i = 0; i < childIds.Count; i++)
            {
                if (GetObject(childIds[i]) == null) return LinkResult.UnknownObject;
            }
        }

        var result = _currentLinks.TryLink(parentId, childIds!);
        if (result == LinkResult.Ok) NotifyChanged();
        return result;
    }

    /// <summary>Dissolves every group in the current context containing any of <paramref name="operandIds"/>. Returns groups dissolved.</summary>
    public int UnlinkObjects(IReadOnlyCollection<Guid> operandIds)
    {
        int dissolved = _currentLinks.UnlinkGroupsOf(operandIds);
        if (dissolved > 0) NotifyChanged();
        return dissolved;
    }

    /// <summary>Members ([parent, children...]) of the link group containing the object, allocation-free.</summary>
    public bool TryGetLinkedMembers(Guid id, out IReadOnlyList<Guid> members) => _currentLinks.TryGetMembers(id, out members);

    public LinkRole GetLinkRole(Guid id) => _currentLinks.GetRole(id);

    /// <summary>Link groups of the given context as persistence records (runtime ids mapped to persistent ids when a map is supplied).</summary>
    public IReadOnlyList<StockAnalyzer.Core.Models.Drawing.DrawingLinkGroupRecord> GetLinkGroupRecords(
        ChartDrawingContextType context,
        IReadOnlyDictionary<Guid, Guid>? runtimeToPersistentIds = null)
        => GetLinkRegistry(context).ToRecords(runtimeToPersistentIds);

    /// <summary>
    /// Restores link groups of the given context from persistence records. Members no longer present in that context
    /// are dropped. Does not fire <see cref="Changed"/> (restoration is not a user mutation).
    /// </summary>
    public void LoadLinkGroups(
        ChartDrawingContextType context,
        IReadOnlyList<StockAnalyzer.Core.Models.Drawing.DrawingLinkGroupRecord>? records,
        IReadOnlyDictionary<Guid, Guid>? persistentToRuntimeIds)
    {
        var live = new HashSet<Guid>();
        if (_contextStore.TryGetValue(context, out var list))
        {
            for (int i = 0; i < list.Count; i++) live.Add(list[i].Id);
        }
        GetLinkRegistry(context).Load(records, persistentToRuntimeIds, live);
    }

    /// <summary>Link groups of every context that has any, addressed by object position (for the legacy drawing payload).</summary>
    public Dictionary<ChartDrawingContextType, List<DrawingLinkGroupIndexRecord>> GetLinkGroupIndexRecords()
    {
        var result = new Dictionary<ChartDrawingContextType, List<DrawingLinkGroupIndexRecord>>();
        foreach (var (context, registry) in _linkRegistries)
        {
            if (registry.GroupCount == 0 || !_contextStore.TryGetValue(context, out var list)) continue;
            var records = registry.ToIndexRecords(list);
            if (records.Count > 0) result[context] = new List<DrawingLinkGroupIndexRecord>(records);
        }
        return result;
    }

    /// <summary>Restores link groups from positional records after <see cref="LoadSnapshot"/>. Does not fire <see cref="Changed"/>.</summary>
    public void LoadLinkGroupIndexRecords(
        ChartDrawingContextType context,
        IReadOnlyList<DrawingLinkGroupIndexRecord>? records)
    {
        if (!_contextStore.TryGetValue(context, out var list)) return;
        GetLinkRegistry(context).LoadFromIndexRecords(records, list);
    }
    ...
    public void LoadSnapshot(Dictionary<ChartDrawingContextType, List<IChartObject>> data)
    {
        ...
        {
            // Replaced objects invalidate their links; callers restore groups afterwards via LoadLinkGroups.
            GetLinkRegistry(kv.Key).Clear();
            if (_contextStore.TryGetValue(kv.Key, out var list))
            {
                AddRetiredObjects(retiredObjects, list);
                list.Clear();
    ...
    private void DisposeRetiredObjects(List<IChartObject> retiredObjects)
    {
        for (int i = 0; i < retiredObjects.Count; i++)
        {
            DisposeIfNoLongerManaged(retiredObjects[i]);
        }
    }

    private void DisposeIfNoLongerManaged(IChartObject chartObject)
    {
        foreach (var contextList in _contextStore.Values)
        {
            for (int i = 0; i < contextList.Count; i++)
            {
                if (ReferenceEquals(contextList[i], chartObject)) return;
            }
        }

        if (chartObject is IDisposable disposable)
        {
            disposable.Dispose();
        }
    }

    private static void AddRetiredObjects(List<IChartObject> retiredObjects, List<IChartObject> objects)
    {
        for (int i = 0; i < objects.Count; i++)
        {
            IChartObject candidate = objects[i];
            bool alreadyRetired = false;
            for (int j = 0; j < retiredObjects.Count; j++)
            {
                if (ReferenceEquals(retiredObjects[j], candidate))
                {
                    alreadyRetired = true;
                    break;
                }
