...
public class ChartObjectManager
{
    ...
    private readonly Dictionary<Guid, DrawingMoveAxisMode> _moveAxisModes = new();
    ...
    /// <summary>
    /// Checks whether an explicit movement axis constraint mode is set for the object.
    /// </summary>
    public bool HasExplicitMoveAxisMode(Guid id) => _moveAxisModes.ContainsKey(id);

    /// <summary>
    /// Gets the movement axis constraint mode for the specified object.
    /// </summary>
    public DrawingMoveAxisMode GetMoveAxisMode(Guid id)
    {
        if (_moveAxisModes.TryGetValue(id, out var mode))
            return mode;
    ...
    /// Locked or invisible objects are ignored.
    /// </summary>
    public IChartObject? GetObjectAt(global::Avalonia.Point screenPoint, ICoordinateTransform transform)
    {
        ...
        {
            var obj = list[i];
            if (!IsVisible(obj.Id) || IsLocked(obj.Id)) continue;
    ...
    /// <summary>
    /// Select object by ID.
    /// Enforces Selection Soundness (Invariant I-03): locked or invisible objects cannot be selected.
    /// </summary>
    public bool SelectObject(Guid id)
    {
        var obj = _currentObjects.FirstOrDefault(o => o.Id == id);
        if (obj == null || !IsVisible(id) || IsLocked(id))
        {
            return false;
        }
    ...
    public bool ToggleLock(Guid id)
    {
        ...
        bool currentlyLocked = IsLocked(id);
        if (currentlyLocked)
        {
            _lockedIds.Remove(id);
            obj.IsLocked = false;
