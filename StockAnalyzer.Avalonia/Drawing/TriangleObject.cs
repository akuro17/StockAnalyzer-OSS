...
public class TriangleObject : IChartObject, IDisposable
{
    ...
    private const double TriangleAngleArcEdgeRatioLimit = 0.45;
    ...
    private readonly SKPaint _arcPaint = new()
    {
        Style = SKPaintStyle.Stroke,
        StrokeWidth = 1,
        IsAntialias = true
    ...
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        {
            _angleTextPaint.Color = SkiaColor;
            _angleTextPaint.TextSize = DrawingThemeContext.DrawingFontSize;
        ...
        global::Avalonia.Point secondNeighbor)
    {
        ...
        double v1x = firstNeighbor.X - vertex.X;
        double v1y = firstNeighbor.Y - vertex.Y;
        double v2x = secondNeighbor.X - vertex.X;
        double v2y = secondNeighbor.Y - vertex.Y;
        double len1 = Math.Sqrt(v1x * v1x + v1y * v1y);
        double len2 = Math.Sqrt(v2x * v2x + v2y * v2y);
        if (len1 <= Math.Sqrt(ChartConstants.TriangleMinimumSquaredEdgeLength) ||
            len2 <= Math.Sqrt(ChartConstants.TriangleMinimumSquaredEdgeLength))
        {
            return;
        }

        // Calculate sweep angle for drawing the arc
        double alpha1 = Math.Atan2(v1y, v1x) * MathConstants.RadToDeg;
        double alpha2 = Math.Atan2(v2y, v2x) * MathConstants.RadToDeg;
        double sweep = (alpha2 - alpha1) % 360.0;
        if (sweep > 180.0) sweep -= 360.0;
        if (sweep < -180.0) sweep += 360.0;

        // Draw Arc with short-edge safety clamping
        float radius = (float)Math.Min(ChartConstants.AngleArcRadius, Math.Min(len1, len2) * TriangleAngleArcEdgeRatioLimit);
        if (radius > 1f)
        {
            var arcRect = new SKRect((float)vertex.X - radius, (float)vertex.Y - radius, (float)vertex.X + radius, (float)vertex.Y + radius);
            canvas.DrawArc(arcRect, (float)alpha1, (float)sweep, false, _arcPaint);
        }

        // Place angle text along the angle bisector
        double u1x = v1x / len1;
        double u1y = v1y / len1;
        double u2x = v2x / len2;
        double u2y = v2y / len2;
        ...
        {
            dirX = bisectorX / bisectorLen;
            dirY = bisectorY / bisectorLen;
        }
        else
        {
            // Degenerate/flat case fallback to centroid direction
            ...
            dirX = centroidX - vertex.X;
            dirY = centroidY - vertex.Y;
            double cLen = Math.Sqrt(dirX * dirX + dirY * dirY);
            if (cLen > 0.001)
            {
                dirX /= cLen;
                dirY /= cLen;
        ...
        float textOffset = radius + ChartConstants.AngleTextOffset;
        float textX = (float)(vertex.X + dirX * textOffset);
        float textY = (float)(vertex.Y + dirY * textOffset -
