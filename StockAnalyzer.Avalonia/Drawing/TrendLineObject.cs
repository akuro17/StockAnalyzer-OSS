...
public class TrendLineObject : RelativeGeometricRenderer, IDrawingCalculatedValuesProvider
{
    ...
    [Category("Projection")]
    [DisplayName("Show Projection")]
    [ParameterTag(DrawingParameterTags.Projection)]
    public bool ShowProjection { get; set; } = false; // Default OFF as requested

    [Category("Projection")]
    [DisplayName("Projection Columns")]
    [Range(1, 100)]
    [ParameterTag(DrawingParameterTags.Projection)]
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        decimal priceAtTime = p1.Price;
        if (dt != 0)
        {
            double ratio = (timestamp - p1.Time).TotalSeconds / dt;
            priceAtTime = p1.Price + (decimal)ratio * (p2.Price - p1.Price);
        }

        return new DrawingCalculatedValue[]
        {
            new DrawingCalculatedValue("TrendLinePrice", "TrendLine Price", priceAtTime, $"{priceAtTime:F3}", color),
            new DrawingCalculatedValue("P1Price", "P1 (Start)", p1.Price, $"{p1.Price:F3}", IndicatorColor.Gray),
            new DrawingCalculatedValue("P2Price", "P2 (End)", p2.Price, $"{p2.Price:F3}", IndicatorColor.Gray)
        };
    }
}
