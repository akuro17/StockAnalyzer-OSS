...
public static class DrawingPanelResolver
{
    ...
    /// <summary>
    /// Returns true when the panel identified by <paramref name="key"/> is currently displayed.
    /// The main panel is always present; indicator panels are present only when they resolve to a display index.
    /// </summary>
    public static bool IsPanelPresent(
        PanelKey key,
        IReadOnlyList<CoreIndicatorSettings>? indicatorSettings,
        bool isSubWindowVisible)
        => key.Kind == PanelKind.Main || TryGetPanelIndex(key, indicatorSettings, isSubWindowVisible).HasValue;
