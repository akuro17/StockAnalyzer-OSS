...
/// </summary>
public class LineTextObject : RelativeGeometricRenderer, ILineTextAnnotatedObject, IDrawingCalculatedValuesProvider
{
    ...
    /// <summary>When true, skips rendering line geometry so underlying candles and grid lines remain visible. Does not affect text annotations.</summary>
    public bool IsTransparent { get; set; } = false;

    /// <summary>Obsolete: Use <see cref="IsTransparent"/> instead.</summary>
    [Obsolete("Use IsTransparent instead.")]
    public bool MatchBackgroundColor
    {
        get => IsTransparent;
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        decimal linePrice = DrawingMath.InterpolatePrice(p1, p2, timestamp);

        var values = new List<DrawingCalculatedValue>();
        values.Add(new DrawingCalculatedValue("LinePrice", "Line Price", linePrice, $"{linePrice:F3}", color));

        decimal priceDelta = p2.Price - p1.Price;
        decimal deltaPct = DrawingMath.CalculatePercentageChange(p1.Price, p2.Price);
        values.Add(new DrawingCalculatedValue("PriceDelta", "Price Change", priceDelta, $"{priceDelta:+0.000;-0.000;0.000} ({deltaPct:+0.00;-0.00;0.00}%)", IndicatorColor.Gray));

        if (!string.IsNullOrWhiteSpace(TopText))
        {
            values.Add(new DrawingCalculatedValue("TopText", "Top Annotation", null, TopText, IndicatorColor.Gray));
        }

        if (!string.IsNullOrWhiteSpace(BottomText))
        {
            values.Add(new DrawingCalculatedValue("BottomText", "Bottom Annotation", null, BottomText, IndicatorColor.Gray));
        }
