...
public static class NurbsConicFactory
{
    ...
    /// <summary>
    /// Builds a 3-point bounded conic arc path (0 &lt; w1 &lt; 1; pass <see cref="InvSqrt2"/> for an
    /// exact circular arc) into destinationPath. Unlike the parabola (w1 = 1.0) and hyperbola
    /// (w1 &gt; 1.0), this weight range produces a closed elliptical/circular arc segment bounded
    /// between points3[0] and points3[2].
    /// </summary>
    public static void BuildConicArcPath(SKPath destinationPath, ReadOnlySpan<SKPoint> points3, double centerWeight)
    {
        if (destinationPath == null || points3.Length < 3) return;
        double w1 = Math.Clamp(centerWeight, 0.01, 0.99);
        Span<double> weights = stackalloc double[3] { 1.0, w1, 1.0 };
        NurbsMath.BuildNurbsPath(destinationPath, 2, points3[..3], weights, Conic3PointKnots);
    }
}
