...
public abstract class RelativeGeometricRenderer : IChartObject, IDisposable
{
    ...
    protected readonly SKPaint _cachedHandlePaint;
    ...
    protected RelativeGeometricRenderer()
    {
        ...
        _cachedHandlePaint = new SKPaint 
        { 
    ...
    public virtual void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        if (IsSelected)
        {
            _cachedHandlePaint.Color = SKColors.Red;
            foreach (var point in Points)
            {
                var screenPt = transform.ChartToScreen(point);
                canvas.DrawCircle((float)screenPt.X, (float)screenPt.Y, ChartConstants.DefaultHandleRadius, _cachedHandlePaint);
            }
        }
    }
    ...
    /// FR-39-5-01, FR-39-5-03: 1:1アスペクト比を前提とした斜め45度線の数学的定義
    /// </summary>
    ...
    /// FR-39-5-02: 描画時にマスの「角」や「端（高値/安値）」を検出し、自動スナップする
    /// </summary>
