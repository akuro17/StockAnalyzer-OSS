...
/// </summary>
public class TargetPriceProjectionObject : IChartObject, IDisposable, IDrawingCalculatedValuesProvider
{
    ...
    internal SKPaint TextPaintForTesting => _textPaint;
    ...
    [Category("Appearance")]
    public Color FontColor { get; set; } = DrawingThemeContext.MainTextColor;

    public const string CategoryProjectionTargets = "Projection Targets";

    [DisplayName("Show Equal Distance (N)")]
    [Category(CategoryProjectionTargets)]
    ...
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        _textPaint.Color = new SKColor(FontColor.R, FontColor.G, FontColor.B, FontColor.A);
        _textPaint.TextSize = (float)Math.Max(1.0, FontSize);
        ...
        if (IsSelected)
        {
            for (int i = 0; i < Points.Count; i++)
            {
                ...
                SelectionHandleRenderer.Draw(canvas, sp, i == AnchorPointIndex ? DrawingThemeContext.AnchorPointColor : (SKColor?)null, radius: ChartConstants.SelectedHandleRadius);
            }
        }
    }
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        foreach (var target in targets)
        {
            ...
            {
                decimal diff = target.Price - currentPrice.Value;
                string diffSign = diff >= 0 ? "+" : "";
                formatted = $"{target.Price:F3} ({diffSign}{diff:F3})";
            }
            else
            {
                formatted = $"{target.Price:F3}";
            }

            string label = target.Name switch
            {
                "V" => "Target V (Double Return)",
                "N" => "Target N (Equal Distance)",
                "%" => "Target % (Equal Ratio)",
                "E" => "Target E (Expansion)",
                "NT" => "Target NT (Secondary)",
                _ => $"Target {target.Name}"
            };

            values.Add(new DrawingCalculatedValue($"Target_{target.Name}", label, target.Price, formatted, color));
        }
