...
/// </summary>
public class TargetPriceProjectionObject : IChartObject
{
    ...
    public List<ProjectionTarget> CalculateTargetPrices()
    {
        ...
        if (Points.Count < 2) return targets;
    ...
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        using var connectPaint = new SKPaint
        {
            Color = SkiaColor.WithAlpha(160),
        ...
        using var projectionPaint = new SKPaint
        {
            Color = SkiaColor,
            StrokeWidth = (float)Thickness + 0.5f,
            Style = SKPaintStyle.Stroke,
            IsAntialias = true
        };
        ...
        foreach (var target in targets)
        {
            ...
            canvas.DrawLine(projLineStartX, targetY, projLineEndX, targetY, projectionPaint);
            ...
            float labelWidth = textPaint.MeasureText(labelText);
            ...
            canvas.DrawCircle(projLineStartX, targetY, ChartConstants.DefaultHandleRadius - 1, markerPaint);
        }

        // Draw anchor handles when selected
        if (IsSelected)
        {
            using var handlePaint = new SKPaint { Color = SKColors.Red, Style = SKPaintStyle.Fill, IsAntialias = true };
            for (int i = 0; i < Points.Count; i++)
            {
                var sp = transform.ChartToScreen(Points[i]);
                canvas.DrawCircle((float)sp.X, (float)sp.Y, ChartConstants.SelectedHandleRadius, handlePaint);
            }
        }
    }
