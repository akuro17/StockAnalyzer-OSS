...
public class RangeSplineObject : RelativeGeometricRenderer, IExtremaLevelSupport, IAxisProjectable
{
    ...
    public PriceField PriceField { get; set; } = PriceField.Close;
    ...
    public void Recalculate(IEnumerable<CoreCandleData>? candles)
    {
        ...
        if (candles is IReadOnlyList<CoreCandleData> list)
        {
            ...
            if (startIndex != -1 && endIndex != -1 && startIndex <= endIndex)
            {
                ...
                {
                    var c = list[i];
                    _extractedPoints.Add(new ChartPoint(c.Timestamp, ExtractPrice(c, PriceField)));
                }
            }
        }
        else
    ...
    /// <summary>
    /// Extracts price with decimal precision based on the specified PriceField (FR-61-7-02).
    /// </summary>
