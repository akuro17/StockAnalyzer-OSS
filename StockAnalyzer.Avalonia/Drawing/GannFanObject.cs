...
/// </summary>
public class GannFanObject : IChartObject, IDrawingCalculatedValuesProvider
{
    ...
    public void Render(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        if (IsSelected)
        {
            SelectionHandleRenderer.Draw(canvas, startScreen, AnchorPointIndex == 0 ? DrawingThemeContext.AnchorPointColor : (SKColor?)null, radius: ChartConstants.SelectedHandleRadius);
            SelectionHandleRenderer.Draw(canvas, endScreen, AnchorPointIndex == 1 ? DrawingThemeContext.AnchorPointColor : (SKColor?)null, radius: ChartConstants.SelectedHandleRadius);
        }
    }
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        decimal range = Math.Abs(p2.Price - p1.Price);
        decimal dy = p2.Price - p1.Price;
        double timeProgress = (timestamp - p1.Time).TotalSeconds / dt;

        var values = new List<DrawingCalculatedValue>(_ratios.Length + 1)
        {
        ...
        foreach (var ratio in _ratios)
        {
            decimal slopeFactor = (decimal)ratio.price / (decimal)ratio.time;
            decimal fanPrice = p1.Price + (decimal)timeProgress * dy * slopeFactor;
            values.Add(new DrawingCalculatedValue($"Gann_{ratio.time}x{ratio.price}", $"Gann {ratio.time}x{ratio.price}", fanPrice, $"{fanPrice:F3}", color));
        }
