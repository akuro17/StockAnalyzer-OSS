...
/// <summary>
/// NURBS Parabola drawing object using 3-point quadratic rational B-spline (w1 = 1.0).
/// Uses 3 anchor points: Points[0] = Start, Points[1] = Vertex/Apex, Points[2] = End.
/// Default color matches Arrow tool (Colors.Green).
/// </summary>
public class NurbsParabolaObject : RelativeGeometricRenderer
{
    public override ChartObjectType Type => ChartObjectType.NurbsParabola;

    public NurbsParabolaObject() : base()
    {
        Color = Colors.Green;
    }

    public NurbsParabolaObject(ChartPoint p0, ChartPoint p1, ChartPoint p2) : this()
    {
        Points.Add(p0);
        Points.Add(p1);
        Points.Add(p2);
    ...
    public NurbsParabolaObject(IEnumerable<ChartPoint> points) : this()
    {
        if (points != null)
        {
    ...
    protected override void DrawGeometry(SKCanvas canvas, ICoordinateTransform transform)
    {
        ...
        _cachedPath.Rewind();
        NurbsConicFactory.BuildParabolaPath(_cachedPath, pts);
    ...
    public override bool HitTest(global::Avalonia.Point screenPoint, ICoordinateTransform transform, double tolerance = ChartConstants.DefaultHitTestTolerance)
    {
        ...
        Span<double> weights = stackalloc double[3] { 1.0, 1.0, 1.0 };
