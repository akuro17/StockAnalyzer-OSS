...
/// </summary>
public class CatenaryCurveObject : RelativeGeometricRenderer, IDrawingCalculatedValuesProvider
{
    ...
    public IReadOnlyList<DrawingCalculatedValue> GetCalculatedValues(DateTime timestamp, decimal? currentPrice = null)
    {
        ...
        var p0 = Points[0]; // Start
        var p1 = Points[1]; // End
        var p2 = Points[2]; // Sag Control
        var color = new IndicatorColor(Color.A, Color.R, Color.G, Color.B);

        decimal baselineMid = (p0.Price + p1.Price) / 2m;
        decimal sag = p2.Price - baselineMid;
        decimal delta = p1.Price - p0.Price;

        double dt = (p1.Time - p0.Time).TotalSeconds;
        decimal catenaryPrice = p0.Price;

        if (Math.Abs(dt) > 1e-4)
        {
            double u = (timestamp - p0.Time).TotalSeconds / dt;
            decimal linearPrice = DrawingMath.InterpolatePrice(p0, p1, timestamp);
            decimal sagContribution = 4m * (decimal)(u * (1.0 - u)) * sag;
            catenaryPrice = linearPrice + sagContribution;
        }

        return new DrawingCalculatedValue[]
        {
            new DrawingCalculatedValue("CatenaryPrice", "Catenary Price", catenaryPrice, $"{catenaryPrice:F3}", color),
            new DrawingCalculatedValue("Sag", "Sag Depth", sag, $"{sag:+0.000;-0.000;0.000}", IndicatorColor.Gray),
            new DrawingCalculatedValue("Delta", "Price Delta", delta, $"{delta:+0.000;-0.000;0.000}", delta >= 0 ? IndicatorColor.Bullish : IndicatorColor.Bearish)
        };
    }
}
