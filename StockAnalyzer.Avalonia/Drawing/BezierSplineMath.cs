...
public static class BezierSplineMath
{
        ...
        SpiralDirection direction = SpiralDirection.Clockwise)
    {
        ...
        if (!Enum.IsDefined(typeof(SpiralDirection), direction)) return;
        ...
        {
            double nextTheta = currentTheta + directionSign * (Math.PI / 2.0);
            ...
            SKPoint tCur = new SKPoint(
                (float)(tangentScale * currentR * (b * cosCur - directionSign * sinCur)),
                (float)(tangentScale * currentR * (b * sinCur + directionSign * cosCur))
            );

            double cosNext = Math.Cos(nextTheta);
            double sinNext = Math.Sin(nextTheta);
            SKPoint tNext = new SKPoint(
                (float)(tangentScale * nextR * (b * cosNext - directionSign * sinNext)),
                (float)(tangentScale * nextR * (b * sinNext + directionSign * cosNext))
            );
        ...
        SpiralDirection direction = SpiralDirection.Clockwise)
    {
        ...
        if (!Enum.IsDefined(typeof(SpiralDirection), direction)) return false;
        ...
        Span<float> destinationDirectionAngles)
    {
        ...
        for (int i = 0; i < count - 1; i++)
        {
            ...
            destinationDirectionAngles[written] = MathF.Atan2(pNext.Y - pCurr.Y, pNext.X - pCurr.X) * MathConstants.RadToDegF;
            written++;
        }
