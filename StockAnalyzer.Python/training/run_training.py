...
_RESERVED_TRAINER_FLAGS: frozenset = frozenset({
    "data-dir", "feature-mode", "window", "horizon", "wf-splits", "gap", "out", "arch",
})
...
def _run_post_eval(cfg: JobConfig, data_dir: Path, out_path: Path) -> None:
    ...
    if cfg.oos_tail_days and cfg.oos_tail_days > 0:
        main_symbols = {}
        for sym, arr in symbols.items():
            main_arr, _main_d, _oos_arr, _oos_d = ds.oos_split(arr, dates[sym], cfg.oos_tail_days)
    ...
    try:
        for row in evaluate_folds(predict, main_symbols, n_splits=cfg.n_splits, gap=cfg.gap, **common):
            _metric(row)
    except Exception as exc:  # noqa: BLE001
...
def _run_selfcheck() -> None:
    ...
    # "target-type" is not pre-reserved (no trainer implements it yet); it passes through like
    # any other hyperparameter rather than raising, until a PR wires it into the trainers.
    tt_cfg = JobConfig.from_json(json.dumps({**base, "hyperparameters": {"target-type": "regression"}}))
    tt_argv = _build_trainer_argv(tt_cfg, Path("d"), Path("o.onnx"))
    assert "--target-type" in tt_argv and tt_argv[tt_argv.index("--target-type") + 1] == "regression"
