                ...
                if method == "ping":
                    ...
                    if global_df is None:
                        ...
                        try:
                            ...
                            w = int(args.get('windowSize', 64))
                            if w < 4:
                                w = 4
                            l = int(args.get('embeddingDimension', 20))
                            l = max(2, min(l, w // 2))
                            k = w - l + 1
                            r = int(args.get('numComponents', 2))
                            r = max(1, min(r, min(l, k)))

                            price_source = args.get('priceSource', 'Close')
                            if price_source == 'Typical' and 'High' in global_df.columns and 'Low' in global_df.columns and 'Close' in global_df.columns:
                                prices = ((global_df['High'] + global_df['Low'] + global_df['Close']) / 3.0).values
                            elif price_source == 'Weighted' and 'High' in global_df.columns and 'Low' in global_df.columns and 'Close' in global_df.columns:
                                prices = ((global_df['High'] + global_df['Low'] + 2.0 * global_df['Close']) / 4.0).values
                            elif price_source == 'Median' and 'High' in global_df.columns and 'Low' in global_df.columns:
                                prices = ((global_df['High'] + global_df['Low']) / 2.0).values
                            else:
                                prices = global_df['Close'].values
                            ...
                            if n >= w:
                                for t in range(w - 1, n):
                                    sub = prices[t - w + 1 : t + 1]

                                    # Degenerate case: flat constant series (epsilon-tolerance)
                                    if np.ptp(sub) <= 1e-10:
                                        trend[t] = float(sub[-1])
                                        continue

                                    # Trajectory matrix (Hankel: shape (l, k))
                                    X = np.lib.stride_tricks.sliding_window_view(sub, window_shape=l).T
                                    ...
                                    # Direct outer-product reconstruction of selected r components (no division by singular value)
                                    # At causal endpoint k = w - 1, the anti-diagonal set {(i, j) | i+j = w-1} has size 1 (only (l-1, k-1)).
                                    # Therefore, diagonal averaging simplifies exactly to the single element X_rec[l-1, k-1] in O(r) dot product.
                                    val_last = float(np.dot(U[l - 1, :r] * s[:r], Vt[:r, k - 1]))
                            ...
                            response["result"] = {
                                "ssa": [float(x) if x is not None else None for x in trend]
                            }
                        except Exception as e:
                            response["status"] = "error"
                            response["error"] = str(e)
