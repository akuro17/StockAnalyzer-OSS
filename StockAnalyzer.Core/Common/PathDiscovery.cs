...
public static class PathDiscovery
{
    ...
    public static string ResolveTemplatesDirectory(StockAnalyzer.Core.Models.Templates.TemplateType? type = null)
    {
        ...
        if (type.HasValue)
        {
            templatesDir = Path.Combine(templatesDir, type.Value.ToString());
        }
        try
        {
        ...
        catch (Exception)
        {
            try
            {
                var fallbackBase = Path.Combine(Path.GetTempPath(), "StockAnalyzer", "Templates");
                templatesDir = type.HasValue ? Path.Combine(fallbackBase, type.Value.ToString()) : fallbackBase;
                if (!Directory.Exists(templatesDir))
                {
                    Directory.CreateDirectory(templatesDir);
                }
            ...
            {
                var baseFallback = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Templates");
                templatesDir = type.HasValue ? Path.Combine(baseFallback, type.Value.ToString()) : baseFallback;
                try
                {
                    if (!Directory.Exists(templatesDir))
                    {
                ...
                {
                    templatesDir = AppDomain.CurrentDomain.BaseDirectory;
                }
            }
        }
        return templatesDir;
