...
namespace StockAnalyzer.Core.Services.Notes
{
    public interface INotesSettingsManager : INotifyPropertyChanged
    {
        ...
        /// both the main timeline/detail page and the Trash/orphaned-files views). Independent of the
        /// app's shell text colors so changing Settings &gt; Theme never affects Notes.</summary>
        IndicatorColor BodyTextColor { get; }

        /// <summary>Background color applied to every Note card (Settings &gt; Notes, both the main
        /// timeline/detail page and the Trash/orphaned-files views). Independent of the app's shell
        /// background color so changing Settings &gt; Theme never affects Notes.</summary>
        ...
        /// to the same value as <see cref="BodyTextColor"/> but does not track later changes to it.</summary>
