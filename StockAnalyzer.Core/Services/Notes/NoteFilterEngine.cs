...
public sealed record NoteFilterCriteria
{
    ...
    /// <summary>The single ticker selected in the "銘柄" dropdown, if any.</summary>
    public string? SelectedTicker { get; init; }
...
/// <c>(Scope) AND (RegisteredTag OR) AND (period) AND (search) AND (hashtag)</c>.
...
public static class NoteFilterEngine
{
    ...
    public static bool Matches(Note note, NoteFilterCriteria criteria)
    {
        ...
        if (criteria.SelectedTicker is not null &&
            !string.Equals(note.RelatedTicker, criteria.SelectedTicker, StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
