...
public static class CandlePatternDetector
{
    ...
    public static CandlePatternType DetectPattern(IReadOnlyList<CandleData> candles)
    {
        ...
        // Doji (body is extremely small relative to range, avgBody, OR ATR)
        bool isDoji = body <= (range * 0.05m) || body <= (avgBody * 0.1m);
        if (atr > 0) isDoji = isDoji || body <= (atr * ChartConstants.CandlePatternDojiAtrRatio);
        if (body == 0) isDoji = true; // absolute zero is definitely Doji
