...
public interface IReverseWatchAnalysisService
{
    ...
    /// </summary>
    ReverseWatchCurveData Calculate(IEnumerable<CandleData> candles, int period = 25, string stockCode = "", bool isMaBased = true, bool isLogScaleVolume = false, int dataCount = 0);
}
...
public class ReverseWatchAnalysisService : IReverseWatchAnalysisService
{
    ...
    public ReverseWatchCurveData Calculate(IEnumerable<CandleData> candles, int period = 25, string stockCode = "", bool isMaBased = true, bool isLogScaleVolume = false, int dataCount = 0)
    {
        if (period <= 0)
        ...
        // Pass 2: Calculate Bounds for the target visible curve points
        ...
        int targetCount = dataCount > 0 ? Math.Min(points.Count, dataCount) : points.Count;
        int startIndex = points.Count - targetCount;
        ...
        // Pass 3: Assign Phases based on Position relative to Target Window Center (Midpoint of Bounds)
        ...
        foreach (var p in points)
        {
            ...
            double degrees = angle * MathConstants.RadToDeg;
