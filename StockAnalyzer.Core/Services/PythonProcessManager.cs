...
namespace StockAnalyzer.Core.Services
{
    public class PythonProcessManager : IResilienceStateProvider, IAsyncDisposable
    {
        ...
        private WindowsJobObjectHandle? _jobHandle;
        private readonly string _pythonScriptPath;
        private readonly string _pipeName;
        // Windows: NamedPipeServerStream (Python connects as a plain byte-stream file client, no pywin32).
        // Unix: NamedPipeClientStream (Python binds the AF_UNIX socket that .NET maps named pipes onto).
        private PipeStream? _pipeClient;
        private BoundedLineReader? _pipeReader;
        ...
        {
            // Circuit breaker must be the OUTERMOST strategy (added first), wrapping retry
            // (added second/innermost). Polly executes added-first strategies as the outer
            // wrapper, so with circuit breaker outermost, a single logical call's retries all
            // happen *inside* one circuit-breaker-tracked execution -- the breaker only sees
            // one aggregate outcome (after retries are exhausted), not each individual retry
            // attempt. With the reverse order (retry outermost), every one of the up-to-3
            // retry attempts for a SINGLE failing call was independently counted by the
            // breaker, so one transient IOException could trip MinimumThroughput=3 by itself
            // within seconds -- turning a single brief hiccup into a full 30s (BreakDurationMs)
            // "circuit breaker open" outage.
        ...
        public async Task StartAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // Start Python Process securely using ArgumentList
                ...
                // On Windows the server instance must exist before Python starts so its open() cannot race the listener.
                ...
                if (OperatingSystem.IsWindows())
                {
                    ...
                    _pipeClient = pipeServer;
                    serverConnectTask = pipeServer.WaitForConnectionAsync(cts.Token);
                    // A wait abandoned by CleanupConnection faults on disposal; observe it so it is not reported as unobserved.
                    _ = serverConnectTask.ContinueWith(static t => _ = t.Exception, TaskContinuationOptions.OnlyOnFaulted | TaskContinuationOptions.ExecuteSynchronously);
                }
                ...
                // Manager) rather than shut down gracefully, Windows closes our Job Object handle
                // as part of process teardown, which kills this Python child too. No-op elsewhere.
                _jobHandle = WindowsJobObject.TryAssign(_pythonProcess);
                ...
                try
                {
                    while (!_pipeClient!.IsConnected)
                    {
                        ...
                        if (serverConnectTask != null)
                        {
                            // Poll so a Python process that died before connecting is noticed promptly.
                            var pollTask = Task.Delay(_settings.PipeConnectPollIntervalMs, cts.Token);
                            var finished = await Task.WhenAny(serverConnectTask, pollTask);
                            if (finished == serverConnectTask) await serverConnectTask;
                        }
                        else
                        {
                            try { await ((NamedPipeClientStream)_pipeClient).ConnectAsync(_settings.PipeConnectPollIntervalMs, cts.Token); }
                ...
                {
                    var processExited = IsProcessDead();
                    CleanupConnection();
                    if (processExited) throw new Exception("Python process exited before the pipe connected.");
                    throw new TimeoutException($"Pipe connection timed out after {_settings.PipeConnectionTimeoutMs}ms.");
                }
                catch
                {
                    // Cancel the pending wait and discard the process and pipe instance completely.
                    CleanupConnection();
                    throw;
                }

                _pipeReader = new BoundedLineReader(_pipeClient, _settings.PythonMaxResponseBytes);
        ...
        public Task<string> SendCommandAsync(string method, object? args = null, CancellationToken cancellationToken = default)
        {
            return ExecuteWithResilienceAsync(
                static async (context, state) =>
                {
                    ...
                    return await manager.WithRequestTimeoutAsync(context.CancellationToken, async token =>
                    {
                        await manager._pipeWriter!.WriteAsync((jsonRequest + "\n").AsMemory(), token);
                        await manager._pipeWriter.FlushAsync(); // Ensure JSON is sent

                        var responseJson = await manager._pipeReader!.ReadLineAsync(token);
        ...
        public Task SendBinaryDataAsync(ReadOnlyMemory<byte> data, CancellationToken cancellationToken = default)
        {
            return ExecuteWithResilienceAsync(
                static async (context, state) =>
                {
                    ...
                    return await manager.WithRequestTimeoutAsync(context.CancellationToken, async token =>
                    {
                        await manager._pipeClient!.WriteAsync(state.data, token);
                        await manager._pipeClient.FlushAsync(token);
                        return true;
                    });
                },
        ...
        /// <summary>
        /// Bounds one request/response exchange by <c>PythonRequestTimeoutMs</c>. A hung Python process surfaces as a
        /// <see cref="TimeoutException"/> (not retried per command: the stream is desynchronized, so the connection is
        /// torn down and the caller's transaction is re-run from the start).
        /// </summary>
        internal async Task<T> WithRequestTimeoutAsync<T>(CancellationToken callerToken, Func<CancellationToken, Task<T>> operation)
        {
            using var timeoutCts = CancellationTokenSource.CreateLinkedTokenSource(callerToken);
            timeoutCts.CancelAfter(_settings.PythonRequestTimeoutMs);
            try
            {
            ...
            {
                throw new TimeoutException($"Python did not complete the request within {_settings.PythonRequestTimeoutMs}ms.");
            }
        }
        ...
        private async Task HealthMonitorLoopAsync(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    // Self-heal whenever the connection is known to be down (_isConnected
                    // false, e.g. after CleanupConnection() tore the pipe down on an IOException
                    // -- which also nulls _pythonProcess, making IsProcessDead() true too, but
                    // that's a coincidence of CleanupConnection's own teardown, not something
                    // this check should depend on), or when the process died out from under an
                    // otherwise-still-"connected" state. Requiring _isConnected before checking
                    // IsProcessDead() (the original condition) meant this loop could never
                    // trigger once any prior failure had already flipped _isConnected to false.
                    if (!_isConnected || IsProcessDead())
                    {
                        Debug.WriteLine("[PythonProcessManager] Health check failed. Connection is down. Attempting Self-Healing via resilience pipeline...");
