...
public class ComparisonDataAligner : IComparisonDataAligner
{
        ...
        CancellationToken ct = default)
    {
        ...
        // De-duplicate comparison symbols and remove primary symbol if present (supporting ^ and ./- aliases)
        var uniqueSymbols = comparisonSymbols
            .Where(s => !SymbolHelper.IsSameSymbol(s, primarySymbol))
        ...
        // Also bind any comparison symbols that represent the primary symbol (e.g. self-comparison GSPC == GSPC, ^GSPC == GSPC)
        foreach (var sym in comparisonSymbols)
        {
            if (SymbolHelper.IsSameSymbol(sym, primarySymbol) && !series.ContainsKey(sym))
            {
                series[sym] = primaryAligned;
            }
        }
