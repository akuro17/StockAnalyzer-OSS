...
namespace StockAnalyzer.Core.Services
{
    public class PythonService : IPythonService, IAsyncDisposable
    {
        ...
        public async Task<string> CalculateFftCycleAsync(int windowSize = IndicatorDefaultConstants.FftCycleDefaultWindowSize)
        {
            if (_processManager == null) throw new InvalidOperationException("External process not initialized.");

            return await _processManager.SendCommandAsync("calculate_fft_cycle", new { windowSize = windowSize });
        }

        public async Task<string> CalculateFourierTransformAsync(int targetPeriod = IndicatorDefaultConstants.FourierTransformDefaultTargetPeriod)
        {
            if (_processManager == null) throw new InvalidOperationException("External process not initialized.");

            return await _processManager.SendCommandAsync("calculate_fourier_transform", new { targetPeriod = targetPeriod });
        }
        ...
        public async Task<string> CalculateSsaAsync(int windowSize = IndicatorDefaultConstants.SsaDefaultWindowSize, int embeddingDimension = IndicatorDefaultConstants.SsaDefaultEmbeddingDimension, int numComponents = IndicatorDefaultConstants.SsaDefaultNumComponents, PriceType priceSource = PriceType.Close)
        {
            if (_processManager == null) throw new InvalidOperationException("External process not initialized.");

            return await _processManager.SendCommandAsync("calculate_ssa", new
            {
