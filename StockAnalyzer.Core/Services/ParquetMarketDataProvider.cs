...
public class ParquetMarketDataProvider : IMarketDataProvider
{
        ...
        ILogger<ParquetMarketDataProvider>? logger = null)
    {
        ...
        _baseDataPath = Common.PathDiscovery.ResolveDataPath(settings.Value.DailyDataPath, MarketDataSettings.DefaultDailyDataPath, "*.parquet");
    ...
    public async Task<int> DeleteLatestTickerDataAsync(string symbol, int count)
    {
        if (count <= 0) return 0;
        ...
        if (!File.Exists(filePath))
        {
            _logger.LogWarning("Cannot delete latest time series data: Parquet file not found for ticker {Ticker}", symbol);
            return 0;
        }

        DateTime cutoffDate;
        using (await _dbManager.AcquireLockAsync("DeleteLatestTickerData"))
        {
            var connection = _dbManager.GetConnection();
            if (connection is not DbConnection dbConnection)
            {
            ...
            cmd.CommandText = $"SELECT date FROM read_parquet('{EscapeDuckDbPath(filePath)}') ORDER BY date DESC LIMIT ?";
            ...
            cutoffDate = dates.Min();
        }

        return await DeleteTickerDataFromDateAsync(symbol, cutoffDate);
    }
        ...
        try
        {
            var meta = await _pythonService.RunAsync(py =>
            {
                try 
                {
                    ...
                    var scriptDir = Path.GetDirectoryName(PythonScriptLocator.Resolve("data_provider.py"))
                        ?? throw new DirectoryNotFoundException("Python data provider directory could not be resolved.");
    ...
    internal static string EscapeDuckDbPath(string path) => path.Replace("'", "''");
}
