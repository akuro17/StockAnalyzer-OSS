...
public class ParquetMarketDataProvider : IMarketDataProvider
{
    ...
    public async Task<IReadOnlyList<CandleData>> GetTickersDataAsync(string ticker, TimeFrame timeFrame)
    {
        ...
        {
            var altSymbol = ticker.Contains('.') ? ticker.Replace('.', '-') : (ticker.Contains('-') ? ticker.Replace('-', '.') : null);
            if (altSymbol != null)
            {
                var altFilePath = Path.Combine(_baseDataPath, $"{altSymbol}.parquet").Replace("\\", "/");
        ...
        using (await _dbManager.AcquireLockAsync("GetTickersData"))
        {
            await _resiliencePipeline.ExecuteAsync(async ct =>
            {
                ...
                command.CommandText = $"SELECT * FROM read_parquet('{EscapeDuckDbPath(filePath)}') ORDER BY date ASC";
    ...
    public Task<System.DateTimeOffset?> GetTimeSeriesLastUpdatedAsync(string symbol)
    {
        if (string.IsNullOrWhiteSpace(symbol))
        {
            return Task.FromResult<System.DateTimeOffset?>(null);
        }

        var filePath = Path.Combine(_baseDataPath, $"{symbol}.parquet");
        if (File.Exists(filePath))
        {
            try
            {
                var lastWriteTime = File.GetLastWriteTimeUtc(filePath);
                return Task.FromResult<System.DateTimeOffset?>(new System.DateTimeOffset(lastWriteTime));
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to get last write time of Parquet file for ticker {Ticker}", symbol);
            }
        }
        return Task.FromResult<System.DateTimeOffset?>(null);
    }
