...
public static class FundamentalsCalculator
{
    public static TickerMetadata CalculateDerived(TickerMetadata meta, IReadOnlyList<CandleData>? candles = null)
    {
        ...
        {
            dividendYieldCalculated = meta.DividendYield.Value;
        }
        ...
        {
            decimal growthPct = Math.Abs(meta.EarningsGrowth.Value) < 2.0m ? meta.EarningsGrowth.Value * 100m : meta.EarningsGrowth.Value;
            if (growthPct != 0m)
            {
                pegRatio = meta.TrailingPE.Value / growthPct;
            }
        }
        ...
        {
            netCashRatio = ((meta.TotalCash.Value - meta.TotalDebt.Value) / marketCap.Value) * 100m;
        }
