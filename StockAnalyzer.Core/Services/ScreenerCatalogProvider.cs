...
public class ScreenerCatalogProvider : IScreenerCatalogProvider
{
    ...
    public IReadOnlyList<ScreenerCatalogItem> GetCatalogItems(IIndicatorFactory? indicatorFactory = null)
    {
        ...
        // 1.1 Price group items (all 15 PriceType options in order)
        foreach (var priceType in PriceDataHelper.PriceTypeOptions)
        {
            indicatorItems.Add(new ScreenerCatalogItem
            {
                CategoryType = ScreenerItemCategoryType.Indicator,
                GroupName = ScreenerGroupNames.Price,
                IndicatorType = IndicatorType.Price,
                ShortName = priceType.ToString(),
                DisplayName = PriceDataHelper.FormatPriceTypeLabel(priceType)
            });
        }
        ...
        var sortedIndicators = indicatorItems
            ...
            .ThenBy(i => i.GroupName == ScreenerGroupNames.Price
                ? (priceOrderMap.TryGetValue(i.ShortName, out int order) ? order : 99)
                : 0)
    ...
    public IReadOnlyList<string> GetOutputSeriesNames(IndicatorType type, IIndicatorFactory? indicatorFactory = null)
    {
        ...
        // IFFT Instantaneous Phase: the base "Main" series (0-360 deg) has no public alias
        // property (see the indicator class for why), so it would otherwise be unselectable here
        // even though it is the indicator's primary output. Inject its friendly display name
        // directly; ScreenerValueExtractor's HasSeries fallback resolves an unmatched OutputName
        // back to the actual "Main" series automatically (the same mechanism the type.ToString()
        // fallback above already relies on for single-series indicators like SMA).
        if (type == IndicatorType.IFFTInstantaneousPhase &&
            !list.Contains(CoreIfftInstantaneousPhaseIndicator.ScreenerPhaseAngleOutputName))
        {
            list.Insert(0, CoreIfftInstantaneousPhaseIndicator.ScreenerPhaseAngleOutputName);
        }

        // Polar Phase: same rationale as IFFT Instantaneous Phase above -- its 0-360 deg "Main"
        // series has no public alias property, so inject its friendly name here so it stays
        // selectable in the screener (ScreenerValueExtractor's HasSeries fallback resolves it
        // back to the actual "Main" series).
        if (type == IndicatorType.PolarPhase &&
            !list.Contains(CorePolarPhaseIndicator.ScreenerPhaseAngleOutputName))
        {
            list.Insert(0, CorePolarPhaseIndicator.ScreenerPhaseAngleOutputName);
        }

        // Polar Cycle Angular Frequency: same rationale -- once it emits named diagnostic series
        // (PhaseAngularVelocity / PhaseFrequencyError) the reflective discovery above stops falling
        // back to the enum name for its "Main" (cycle angular frequency) series, so inject the
        // friendly name here to keep it selectable in the screener.
        if (type == IndicatorType.PolarCycleAngularFrequency &&
            !list.Contains(CorePolarCycleAngularFrequencyIndicator.ScreenerCycleAngularFrequencyOutputName))
        {
            list.Insert(0, CorePolarCycleAngularFrequencyIndicator.ScreenerCycleAngularFrequencyOutputName);
        }
