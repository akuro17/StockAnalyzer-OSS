...
public interface IMarketDataProvider
{
    ...
    /// <summary>
    /// Permanently deletes the latest <paramref name="count"/> time series rows for the specified symbol.
    /// If <paramref name="count"/> is 0 or negative, or if the file does not exist, returns 0 without deleting.
    /// </summary>
    /// <param name="symbol">The ticker symbol.</param>
    /// <param name="count">The number of latest rows to delete.</param>
    /// <returns>The number of rows deleted.</returns>
    Task<int> DeleteLatestTickerDataAsync(string symbol, int count) => System.Threading.Tasks.Task.FromResult(0);
}
