...
public static class ChartTypeCapabilitiesRegistry
{
    private static readonly Dictionary<ChartType, ChartTypeCapabilities> _capabilities = new()
    {
        ...
        [ChartType.Dot] = new ChartTypeCapabilities
        {
            IsTimeBased = true,
            IsIndexBased = false,
            HasStandardHeader = true,
        ...
        // Renko / P&F / Kagi / ThreeLineBreak: インデックスベース変換系。指標を一切描画しない
        // (SupportsIndicators = false, CanToggleIndicators = false)。
        // 根拠と可視性コントラクトは docs/SA_UI_INTERACTION.md セクション 27 / 27.1 を参照。
        [ChartType.Renko] = new ChartTypeCapabilities
        {
            IsCompactType = true,
            IsIndexBased = true,
            SupportsIndicators = false,
            CanToggleIndicators = false,
        },
        ...
        [ChartType.Invisible] = new ChartTypeCapabilities
        {
            IsTimeBased = true,
            IsIndexBased = false,
            HasStandardHeader = true,
