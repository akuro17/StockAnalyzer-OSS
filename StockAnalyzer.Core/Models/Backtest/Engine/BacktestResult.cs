...
public sealed class BacktestResult
{
    ...
    private readonly byte[] _reproducibilityHash;

    /// <summary>Returns a defensive copy so neither the constructor argument nor a previous getter result aliases the stored hash.</summary>
    public byte[] ReproducibilityHash => (byte[])_reproducibilityHash.Clone();
        ...
        bool isInsufficientData)
    {
        ...
        _reproducibilityHash = (byte[])(reproducibilityHash ?? throw new ArgumentNullException(nameof(reproducibilityHash))).Clone();
