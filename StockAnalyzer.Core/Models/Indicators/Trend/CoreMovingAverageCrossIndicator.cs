...
public class CoreMovingAverageCrossIndicator : CoreIndicatorBase
{
    ...
    {
        if (parameters is CoreDmaParameter p) // Reusing Dma parameter as it has two periods (Period/Displacement fits well for Short/Long)
        {
            ShortPeriod = p.Displacement; // Mapping Displacement as Short
            LongPeriod = p.Period;      // Mapping Period as Long
        }
    }
    ...
    protected override IIndicatorResult CalculateCore(IReadOnlyList<CoreCandleData> candles)
    {
        ...
        // Calculate SMAs
        CalculateSma(candles, ShortPeriod, ShortMA);
        CalculateSma(candles, LongPeriod, LongMA);
    ...
    private static void CalculateSma(IReadOnlyList<CoreCandleData> candles, int period, List<decimal?> target)
    {
        decimal sum = 0;
        for (int i = 0; i < candles.Count; i++)
        {
