...
namespace StockAnalyzer.Core.Models.Indicators.Oscillators
{
    ...
    public class CoreConnorsRsiIndicator : CoreIndicatorBase
    {
        ...
        protected override IIndicatorResult CalculateCore(IReadOnlyList<CoreCandleData> candles)
        {
            ...
            {
                int prev = streak[i - 1];
                if (candles[i].Close > candles[i - 1].Close) streak.Add(prev >= 0 ? prev + 1 : 1);
                else if (candles[i].Close < candles[i - 1].Close) streak.Add(prev <= 0 ? prev - 1 : -1);
                else streak.Add(0);
            }

            // RSI of price
            var rsi = new CoreRsiIndicator { Period = RsiPeriod };
            ...
            // Percent rank of ROC
            var roc = new CoreRocIndicator { Period = 1 };
