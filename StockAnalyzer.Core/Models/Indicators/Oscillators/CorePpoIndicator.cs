    ...
    public class CorePpoIndicator : CoreIndicatorBase
    {
        ...
        protected override IIndicatorResult CalculateCore(IReadOnlyList<CoreCandleData> candles)
        {
            ...
            for (int i = 0; i < candles.Count; i++)
            {
                ...
                else if (fastEma == null) { fastEma = candles.Take(FastPeriod).Average(c => c.Close); fastEmas.Add(fastEma); }
                else { fastEma = (candles[i].Close - fastEma.Value) * fastMult + fastEma.Value; fastEmas.Add(fastEma); }
            }
            ...
            for (int i = 0; i < candles.Count; i++)
            {
                ...
                if (slowEma == null) { slowEma = candles.Take(SlowPeriod).Average(c => c.Close); }
                else { slowEma = (candles[i].Close - slowEma!.Value) * slowMult + slowEma.Value; }
