...
public abstract class CoreIndicatorBase : ICoreIndicator
{
        ...
        else
        {
            ...
            try
            {
                ...
                if (paramType != null)
                {
                    ...
                    SeedPeriodFromIndicatorDefault(settings.ParameterObject);
    ...
    /// <summary>
    /// Overwrites <paramref name="parameterObject"/>'s writable public <c>int Period</c> property
    /// (if it has one) with this indicator's own current <c>Period</c> value (if it has one), when
    /// both exist. Fixes a naming-convention-discovered parameter class (e.g.
    /// <see cref="Parameters.CoreSmaParameter"/> for <see cref="MovingAverages.CoreSmaIndicator"/>)
    /// whose own hardcoded default can drift out of sync with the indicator class's own hardcoded
    /// default -- as found for SMA and EMA (indicator default 20, parameter class default 14,
    /// inherited by <see cref="Parameters.CoreEmaParameter"/>) -- without touching either class's
    /// own default field value, since <see cref="Parameters.CoreSmaParameter"/>'s bare default is
    /// separately relied upon elsewhere (e.g. <c>IndicatorRegistrationViewModel.BindToSide</c>'s
    /// generic screener-catalog fallback) where no such drift exists to correct. A no-op for every
    /// indicator whose two defaults already agree (the vast majority).
    /// </summary>
    private void SeedPeriodFromIndicatorDefault(CoreIndicatorParameterBase? parameterObject)
    {
        ...
        try
        {
            var indicatorPeriod = GetType().GetProperty("Period", BindingFlags.Public | BindingFlags.Instance);
            var paramPeriod = parameterObject.GetType().GetProperty("Period", BindingFlags.Public | BindingFlags.Instance);
            if (indicatorPeriod is null || indicatorPeriod.PropertyType != typeof(int)
                || paramPeriod is null || paramPeriod.PropertyType != typeof(int) || !paramPeriod.CanWrite)
            {
                return;
            }

            var currentPeriod = (int)(indicatorPeriod.GetValue(this) ?? 0);
            if (currentPeriod > 0)
            {
                paramPeriod.SetValue(parameterObject, currentPeriod);
            }
        }
        catch (AmbiguousMatchException)
        {
            // GetProperty("Period") matched more than one property (a derived `new` shadowing a
            // base member). Cannot pick one safely - leave the parameter object's Period as-is.
        }
    }
    ...
    protected virtual IIndicatorResult CalculateSeriesCore(IReadOnlyList<decimal?> series, IReadOnlyList<decimal?>? dynamicPeriods = null)
    {
        ...
        decimal baselineVal = series[firstValidIdx]!.Value;
        var syntheticCandles = new List<CoreCandleData>(series.Count);
        var baseTime = DateTime.Today;
        for (int i = 0; i < series.Count; i++)
        {
            decimal val = series[i] ?? baselineVal;
            syntheticCandles.Add(new CoreCandleData(baseTime.AddDays(i), val, val, val, val, 0));
        }

        var result = CalculateCore(syntheticCandles);
        if (firstValidIdx > 0 && result.IsSuccessful)
        {
            var sanitizedSeries = new Dictionary<string, IReadOnlyList<decimal?>>(StringComparer.OrdinalIgnoreCase);
            foreach (var seriesName in result.SeriesNamesList)
            {
                var originalList = result.GetSeries(seriesName);
                var sList = new List<decimal?>(originalList.Count);
                for (int i = 0; i < originalList.Count; i++)
                {
                    sList.Add(i < firstValidIdx ? null : originalList[i]);
                }
                sanitizedSeries[seriesName] = sList;
            }

            return IndicatorResult.Success(sanitizedSeries, result.CustomData, result.SignalProvider);
        }
