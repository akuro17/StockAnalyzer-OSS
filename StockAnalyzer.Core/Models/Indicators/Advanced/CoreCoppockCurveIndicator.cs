...
public class CoreCoppockCurveIndicator : CoreIndicatorBase
{
    ...
    protected override IIndicatorResult CalculateCore(IReadOnlyList<CoreCandleData> candles)
    {
        ...
        for (int i = 0; i < candles.Count; i++)
        {
           ...
           decimal rocLong = (candles[i].Close - candles[i - LongRocPeriod].Close) / candles[i - LongRocPeriod].Close * 100;
           decimal rocShort = (candles[i].Close - candles[i - ShortRocPeriod].Close) / candles[i - ShortRocPeriod].Close * 100;
