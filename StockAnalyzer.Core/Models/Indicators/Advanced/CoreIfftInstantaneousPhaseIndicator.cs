...
public class CoreIfftInstantaneousPhaseIndicator : CoreIndicatorBase
{
    ...
    protected override IIndicatorResult CalculateCore(IReadOnlyList<CoreCandleData> candles)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            double deg = (phase[i] * MathConstants.RadToDeg) % FullTurnDeg;
            if (deg < 0.0)
            {
                deg += FullTurnDeg;
            }
            ...
            LeadSine.Add((decimal)Math.Sin(phase[i] + LeadSineShiftDegrees * MathConstants.DegToRad));
        }
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i > 0 && !double.IsNaN(phase[i]) && !double.IsNaN(phase[i - 1]))
            {
                ...
                while (d > Math.PI) d -= 2.0 * Math.PI;
                deltaDeg = d * MathConstants.RadToDeg;
            }
