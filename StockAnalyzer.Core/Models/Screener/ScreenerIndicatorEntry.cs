...
public partial class ScreenerIndicatorEntry : ObservableObject, IScreeningCondition
{
    ...
    public bool IsMet(IReadOnlyList<CandleData> candles, Portfolio.TickerMetadata metadata)
    {
        ...
        decimal? extractedVal = ScreenerValueExtractor.Default.ExtractValueNullable(LeftHand, candles, metadata);
        if (!extractedVal.HasValue) return false;

        decimal leftVal = extractedVal.Value;
        ...
        return Operator switch
        {
            ...
            ComparisonOperator.Equal => leftVal == rightVal,
            ComparisonOperator.NotEqual => leftVal != rightVal,
            _ => leftVal > rightVal
        };
    }
    ...
    /// <summary>
    /// Creates a deep clone of this screener indicator entry.
    /// </summary>
    public ScreenerIndicatorEntry Clone()
    {
        return new ScreenerIndicatorEntry
        {
            Label = Label,
            LeftHand = LeftHand?.Clone() ?? new ScreenerIndicatorSideConfig(),
            Operator = Operator,
            TargetMode = TargetMode,
            RightNumericValue = RightNumericValue,
            RightStringValue = RightStringValue,
            RightHand = RightHand?.Clone() ?? new ScreenerIndicatorSideConfig(),
