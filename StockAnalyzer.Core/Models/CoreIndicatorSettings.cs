...
public partial class CoreIndicatorSettings : ObservableObject
{
    ...
    partial void OnPriceSourceChanged(PriceType value)
    {
        ...
        {
            var prop = ParameterObject.GetType().GetProperty("PriceSource");
            if (prop != null && prop.CanWrite && prop.PropertyType == typeof(PriceType))
            {
                var curVal = (PriceType)prop.GetValue(ParameterObject)!;
                if (curVal != value)
                {
                    prop.SetValue(ParameterObject, value);
                }
            }
        }
    ...
    public CoreIndicatorParameterBase? ParameterObject
    {
        ...
        set
        {
            if (_parameterObject != value)
            {
                ...
                {
                    _parameterObject.PropertyChanged += OnParameterChanged;
                    var prop = _parameterObject.GetType().GetProperty("PriceSource");
                    if (prop != null && prop.PropertyType == typeof(PriceType))
                    {
                        var val = (PriceType)prop.GetValue(_parameterObject)!;
                        if (_priceSource != val)
                        {
                            _priceSource = val;
    ...
    {
        if (e.PropertyName == "PriceSource" && sender != null)
        {
            var prop = sender.GetType().GetProperty("PriceSource");
            if (prop != null && prop.PropertyType == typeof(PriceType))
            {
                var val = (PriceType)prop.GetValue(sender)!;
                if (_priceSource != val)
                {
                    _priceSource = val;
                    OnPropertyChanged(nameof(PriceSource));
    ...
    public void UpdateDisplayName()
    {
        if (TypeEnum == IndicatorType.Price)
        {
            ...
            DisplayName = UseShortName ? PriceSource.ToString() : Indicators.PriceDataHelper.FormatPriceTypeLabel(PriceSource);
            return;
        }

        string shortName = TypeEnum.HasValue ? TypeEnum.Value.ToString() : "Unknown";
        string longName = TypeEnum.HasValue ? TypeEnum.Value.GetDescription() : "Unknown";
        ...
        if (ParameterObject != null)
        {
            ...
            DisplayName = UseShortName ? ShortDisplayName : ParameterObject.GetDisplayName(longName);
        }
        else
        {
            ShortDisplayName = shortName;
            DisplayName = UseShortName ? shortName : longName;
        }
    ...
    private static readonly System.Reflection.FieldInfo?[] SubscriberFields =
    {
    ...
    /// <summary>
    /// MemberwiseClone copies the event delegate fields of <see cref="ObservableObject"/>, so a fresh copy would notify the subscribers of the
    /// original (e.g. the chart, which recalculates on every change) whenever the copy is edited. The fields are private to the base class and
    /// field-like events cannot be reset from a derived class, hence the reflection; a test guards that the fields still exist.
    /// </summary>
    private void DetachInheritedSubscribers()
    {
        foreach (var field in SubscriberFields)
        {
    ...
    public CoreIndicatorSettings Clone()
    {
        ...
        clone.DetachInheritedSubscribers();
    ...
    public CoreIndicatorSettings Snapshot()
    {
        ...
        snapshot.DetachInheritedSubscribers();
        if (ParameterObject != null)
        {
