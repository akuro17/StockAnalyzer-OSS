...
public sealed record TrainingJobConfig
{
    ...
    /// <see cref="Services.IndicatorChannelExporter"/>, set by <see cref="Services.TrainingOrchestrator"/>
    /// immediately before this config is serialized to the wire JSON -- never populated by the wizard
    /// UI. Present only when <see cref="FeatureSpec"/> contains at least one
    /// <see cref="FeatureChannelKind.Indicator"/> channel; <see langword="null"/> otherwise (including
    /// every <see cref="PredictionFeatureMode.OhlcvMinMax"/> run and every Price-only
    /// <see cref="PredictionFeatureMode.ComposedFeatures"/> run). Lets <c>run_training.py</c> read
    /// each symbol's C#-computed indicator-channel values instead of re-deriving indicator formulas
    /// in Python (single formula source: the <c>ICoreIndicator</c> implementations).
    /// </summary>
    public IReadOnlyDictionary<string, string>? IndicatorChannelExportPaths { get; init; }
