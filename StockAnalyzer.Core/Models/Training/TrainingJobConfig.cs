...
public sealed record TrainingJobConfig
{
    ...
    /// <summary>Feature normalization strategy. Only <see cref="PredictionFeatureMode.OhlcvMinMax"/> is supported in this release.</summary>
    ...
    /// <c>{scope}_{timeframe}_{task}_{arch}_{feattag}_{yyyyMMdd-HHmmss}</c>.
    /// </summary>
    ...
    public void Validate()
    {
        ...
        if (FeatureMode != PredictionFeatureMode.OhlcvMinMax)
        {
            throw new InvalidOperationException(
                $"TrainingJobConfig: FeatureMode '{FeatureMode}' is not supported yet; " +
                "only OhlcvMinMax is available in this release.");
        }
