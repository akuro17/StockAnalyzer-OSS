...
public static class FeatureChannelConverter
{
    ...
    /// <summary>
    /// The single point both the offline exporter (<see cref="Services.IndicatorChannelExporter"/>) and
    /// the online prediction service (<see cref="Services.PredictionService"/>) resolve a
    /// <see cref="FeatureChannelKind.Indicator"/> channel's live <see cref="CoreIndicatorParameterBase"/>
    /// from -- always via <see cref="BuildIndicatorSettings"/>, including for a channel with empty
    /// <see cref="FeatureChannel.Params"/>. This used to be unsafe for a channel with empty Params: some
    /// indicators' <see cref="ICoreIndicator.GetDefaultSettings"/> baseline (naming-convention parameter
    /// class default) disagreed with the indicator class's own hardcoded field default (e.g.
    /// <c>CoreSmaIndicator.Period</c>=20 vs. the pre-fix <c>CoreSmaParameter.Period</c>=14), so routing an
    /// already-shipped empty-Params channel through here would have silently changed its computed values.
    /// <c>CoreIndicatorBase.GetDefaultSettings</c>'s <c>SeedPeriodFromIndicatorDefault</c> fix closed that
    /// gap for <c>Period</c>, and
    /// <c>Tests.StockAnalyzer.Core.Tests.Models.IndicatorParameterDefaultParityTests</c> proves, across
    /// every registered <see cref="IndicatorType"/>, that no other matching-named scalar property
    /// disagrees either -- so this single method is now safe for both empty and non-empty Params, and
    /// replaces the two independent <c>channel.Params.Count &gt; 0</c> branches previously duplicated in
    /// <see cref="Services.IndicatorChannelExporter"/> and <see cref="Services.PredictionService"/>.
    /// </summary>
    public static CoreIndicatorParameterBase? ResolveParameterObject(FeatureChannel channel, IIndicatorFactory factory, out IReadOnlyList<string> warnings)
    {
        var settings = BuildIndicatorSettings(channel, factory, out warnings);
        return settings.ParameterObject;
    }
    ...
    /// <summary>
    /// Enumerates the public instance scalar (see <see cref="IsScalar"/>) properties of
    /// <paramref name="type"/>. <c>internal</c> (not <c>private</c>) so
    /// <c>Tests.StockAnalyzer.Core.Tests.Models.IndicatorParameterDefaultParityTests</c> can reuse this
    /// exact definition of "which properties <see cref="FeatureChannel.Params"/> can carry" instead of
    /// maintaining its own duplicate copy (CLAUDE.md: duplicate shared definitions prohibited).
    /// </summary>
    internal static PropertyInfo[] ScalarProperties(Type type)
    ...
    /// <summary>
    /// <c>internal</c> (not <c>private</c>) for the same reason as <see cref="ScalarProperties"/>.
    /// </summary>
    ...
    /// <summary>
    /// <see langword="true"/> when <paramref name="type"/> is a <see cref="Nullable{T}"/> wrapping a
    /// scalar type per <see cref="IsScalar"/>. Used only by
    /// <c>IndicatorParameterDefaultParityTests.AllRegisteredParameterClasses_DeclareNoNullableScalarProperty</c>
    /// to guard <see cref="ExtractParams"/>'s documented "currently unreachable" Nullable-scalar gap.
    /// </summary>
    internal static bool IsNullableScalar(Type type)
