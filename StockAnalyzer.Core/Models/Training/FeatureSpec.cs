...
public sealed record FeatureSpec
{
    ...
    /// <summary>
    /// D03 resource bound (StockAnalyzer/CLAUDE.md decision register): the compiled fallback
    /// channel-count bound used only by the parameterless <see cref="IsValid(out string?)"/>
    /// overload, for callers with no runtime-configured value available (no DI context). The
    /// actually-enforced value in every settings-aware path (<c>IStockAnalyzerSettings
    /// .PredictionMaxComposedChannels</c>, configured via appsettings.json's "Prediction"
    /// section) is expected to match this default but is not read from here. Mirrored on the
    /// Python side by the <c>SA_MAX_COMPOSED_CHANNELS</c> environment variable's default.
    /// </summary>
    ...
    /// <summary>
    /// <see langword="true"/> when <see cref="Channels"/> is non-empty and every channel is
    /// valid, using the compiled <see cref="MaxChannels"/> as the channel-count bound. This
    /// overload exists for callers with no runtime-configured bound available (e.g. domain-level
    /// validation with no DI context); callers that have <c>IStockAnalyzerSettings</c> available
    /// should prefer <see cref="IsValid(out string?, int)"/> with the configured value instead.
    /// </summary>
    public bool IsValid(out string? error) => IsValid(out error, MaxChannels);

    /// <summary>
    /// <see langword="true"/> when <see cref="Channels"/> is non-empty, its count does not
    /// exceed <paramref name="maxChannels"/> (the D03 resource bound, sourced from
    /// configuration by the caller rather than compiled here), and every channel is valid.
    /// </summary>
    public bool IsValid(out string? error, int maxChannels)
    {
        ...
        {
            error = $"FeatureSpec has {Channels.Count} channels, exceeding the MaxChannels bound of {maxChannels}.";
            return false;
        }
