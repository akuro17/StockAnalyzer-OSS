...
namespace StockAnalyzer.Core.Models.Parameters
{
    public class CoreStructuralDtwParameter : CoreIndicatorParameterBase
    {
        ...
        private int _warpingRadius = IndicatorDefaultConstants.DtwDefaultWarpingRadius;

        [DisplayName("Warping Radius")]
        [Description("Sakoe-Chiba constraint limiting time-warping in DTW. -1 = unconstrained, 0 = diagonal only (no warping), N = allow up to N candles of shift.")]
        [Category("DTW Constraint")]
        [CoreParameterRange(DtwMath.UnconstrainedRadius, 50)]
        ...
        public override void Validate()
        {
            ...
            if (WarpingRadius < DtwMath.UnconstrainedRadius) throw new ArgumentOutOfRangeException(nameof(WarpingRadius), $"WarpingRadius must be >= {DtwMath.UnconstrainedRadius} ({DtwMath.UnconstrainedRadius} = unconstrained)");
        }
