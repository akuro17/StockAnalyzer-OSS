...
public class WorkspaceSettings
{
    ...
    /// <summary>
    /// Schema version of the workspace format for forward/backward compatibility validation.
    /// </summary>
    public int SchemaVersion { get; set; } = CurrentSchemaVersion;

    /// <summary>
    /// Legacy version property maintained for backward compatibility with Version 1 schema.
    /// </summary>
    public int Version
    {
        get => SchemaVersion;
    ...
    /// <summary>
    /// Whether imputation of missing metadata indicators is enabled by default in the sync window.
    /// </summary>
    public bool IsImputeMissingMetadataEnabled { get; set; } = false;
    ...
    /// <summary>
    /// Whether to force download time-series data anew within the Lookback Period range instead of incremental sync.
    /// </summary>
    public bool IsForcePeriodDownloadEnabled { get; set; } = false;

    /// <summary>
    /// Number of latest time series records to delete before fetching new data in the sync window.
    /// Volatile safety default: always initialized to 0 (do not delete) per session to prevent accidental re-deletion.
    /// </summary>
    public int DeleteLatestBarsCount { get; set; } = 0;
    ...
    /// <summary>
    /// Custom column widths mapped by column member name.
    /// </summary>
    public Dictionary<string, string> TickerListColumnWidths { get; set; } = new();

    /// <summary>
    /// Column Customization template selected per ticker list (sidebar node Id -> ColumnTemplate Id).
    /// A list without an entry uses "Active Columns" (the working set stored in <see cref="TickerListVisibleColumns"/>).
    /// </summary>
    ...
    /// <summary>
    /// User-added custom parent categories for the Tickers tab's left-column category list.
    /// Does not include the built-in All Ticker/Watchlist/Portfolio categories, which are fixed.
    /// </summary>
    public List<TickerParentCategorySettings> CustomTickerCategories { get; set; } = new();
}
...
public class DetachedTabInfo
{
    ...
    public double X { get; set; } = StockAnalyzer.Core.Constants.LayoutConstants.DefaultDetachedWindowX;
    public double Y { get; set; } = StockAnalyzer.Core.Constants.LayoutConstants.DefaultDetachedWindowY;
    public double Width { get; set; } = StockAnalyzer.Core.Constants.LayoutConstants.DefaultDetachedWindowWidth;
    public double Height { get; set; } = StockAnalyzer.Core.Constants.LayoutConstants.DefaultDetachedWindowHeight;
