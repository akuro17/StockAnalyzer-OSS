...
public sealed record GlobalChartSettings
{
    ...
    /// <summary>
    /// Whether linking drawings from the Layers Panel snaps the children's anchor points to the parent's
    /// anchor point or keeps their current relative position.
    /// </summary>
    public Models.DrawingLinkMode DrawingLinkMode { get; init; } = Models.DrawingLinkMode.SnapToParentAnchorPoint;
    ...
    // --- Dot ---
    public string DotUpColor { get; init; } = ChartSettingsConstants.DefaultBullishColor;
    public string DotDownColor { get; init; } = ChartSettingsConstants.DefaultBearishColor;
    public string DotNaturalColor { get; init; } = ChartSettingsConstants.DefaultNeutralColor;
    public double DotBaseRadius { get; init; } = ChartSettingsConstants.DefaultDotBaseRadius;
    public Models.DotShapeType DotShape { get; init; } = Models.DotShapeType.Circle;
    public Models.PriceType DotPriceType { get; init; } = Models.PriceType.Close;
    ...
    public GlobalChartSettings Validate()
    {
        ...
        { 
            ...
            DrawingLinkMode = Enum.IsDefined(DrawingLinkMode) ? DrawingLinkMode : DrawingLinkMode.SnapToParentAnchorPoint,
            ...
            DotBaseRadius = double.IsNaN(DotBaseRadius) || double.IsInfinity(DotBaseRadius)
                ? ChartSettingsConstants.DefaultDotBaseRadius
                : Math.Clamp(DotBaseRadius, ChartSettingsConstants.MinDotBaseRadius, ChartSettingsConstants.MaxDotBaseRadius),
            DotUpColor = IsValidHexColor(DotUpColor) ? DotUpColor : ChartSettingsConstants.DefaultBullishColor,
            DotDownColor = IsValidHexColor(DotDownColor) ? DotDownColor : ChartSettingsConstants.DefaultBearishColor,
            DotNaturalColor = IsValidHexColor(DotNaturalColor) ? DotNaturalColor : ChartSettingsConstants.DefaultNeutralColor,
            DotShape = Enum.IsDefined(DotShape) ? DotShape : DotShapeType.Circle,
            DotPriceType = Enum.IsDefined(DotPriceType) ? DotPriceType : PriceType.Close,
