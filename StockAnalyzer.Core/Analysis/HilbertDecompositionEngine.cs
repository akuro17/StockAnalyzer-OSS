        ...
        HilbertDecompositionParameters? parameters = null)
    {
        ...
        try
        {
            ...
            for (int t = 0; t < n; t++)
            {
                ...
                // Clamp to the documented [-1, 1] unit-phasor range: amplitude is a double->decimal
                // sqrt of (i2^2 + q2^2), so i2/amplitude can overshoot 1 by a rounding ULP.
                decimal normI = isAboveThreshold ? Math.Clamp(i2[t] / amplitude, -1m, 1m) : 0m;
                decimal normQ = isAboveThreshold ? Math.Clamp(q2[t] / amplitude, -1m, 1m) : 0m;
                ...
                double phaseDeg = phaseRad * MathConstants.RadToDeg;
                ...
                double unwrappedDeg = (phaseRad + unwrappedOffset) * MathConstants.RadToDeg;
