...
public sealed record HilbertDecompositionParameters(
    ...
    int MinimumCyclePeriod = 2)
{
    // MinimumCyclePeriod is the effective Nyquist floor (in bars) consumed by the polar
    // decomposition's IsFrequencyValid gate; the Hilbert engine itself does not read it.
    ...
    public void Validate()
    {
        ...
        if (MinimumCyclePeriod < 2)
            throw new ArgumentOutOfRangeException(nameof(MinimumCyclePeriod), "MinimumCyclePeriod must be at least 2 (Nyquist floor).");
        if (MinimumCyclePeriod > MaxPeriod)
            throw new ArgumentOutOfRangeException(nameof(MinimumCyclePeriod), "MinimumCyclePeriod must not exceed MaxPeriod.");
    }
}
