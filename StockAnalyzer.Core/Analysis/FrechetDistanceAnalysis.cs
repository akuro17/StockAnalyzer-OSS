...
public static class FrechetDistanceAnalysis
{
        ...
        double confidenceMultiplier = PredictionBandMultiplier)
    {
        ...
        Span<double> queryPrices = length <= MathBufferLimits.StackAllocThreshold
        ...
        Span<double> queryNorm = length <= MathBufferLimits.StackAllocThreshold
            ? stackalloc double[length]
        ...
        try
        {
            ...
            Span<double> candPrices = length <= MathBufferLimits.StackAllocThreshold
                ? stackalloc double[length]
                : (candHeapBuffer = ArrayPool<double>.Shared.Rent(length)).AsSpan(0, length);

            double[]? candNormHeapBuffer = null;
            Span<double> candNorm = length <= MathBufferLimits.StackAllocThreshold
                ? stackalloc double[length]
