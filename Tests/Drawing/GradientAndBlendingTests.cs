...
public class GradientAndBlendingTests
{
    ...
    {
        var transform = new DummyCoordinateTransform();
        // Center (200, 150), corner (300, 150): due east of center (dy=0), so rotationAngle=0 and
        // rx=distance=100; ry=rx*AspectRatio(default 0.5)=50, reproducing the pre-rotation a=100/b=50
        // geometry (center+corner click model) with no rotation to keep this test's assertions simple.
        var p1 = transform.ScreenToChart(new Point(200, 150));
        var p2 = transform.ScreenToChart(new Point(300, 150));

        var ellipseFilled = new EllipseObject(p1, p2) { IsFilled = true, IsCircular = false };
        var ellipseOutline = new EllipseObject(p1, p2) { IsFilled = false, IsCircular = false };
    ...
    {
        var referenceCorner = new ChartPoint(new DateTime(2025, 2, 10), 230m);
        var activationCorner = new ChartPoint(new DateTime(2025, 2, 12), 240m);
        ...
        Assert.Equal(4, restoredEllipse.Points.Count); // constructor now always adds circumference handles (Points[2]/[3])
        ...
        Assert.False(restoredEllipse.IsCircular);
        Assert.True(restoredEllipse.IsArcEnabled);
        Assert.True(restoredEllipse.ShowRadiusLines);
        Assert.True(restoredEllipse.ShowChordLine);
        Assert.Equal(0.35, restoredEllipse.AspectRatio);
        Assert.True(restoredEllipse.DynamicAspectRatioByDistance);
        Assert.Equal(referenceCorner, restoredEllipse.DynamicAspectRatioReferenceCorner);
        Assert.Equal(activationCorner, restoredEllipse.EllipticityActivationCorner);
        Assert.Equal(Colors.Teal, restoredEllipse.ControlPointColor);
        Assert.Equal(0.4, restoredEllipse.InnerRadiusRatio);
        Assert.True(restoredEllipse.ShowTangentLines);
        Assert.True(restoredEllipse.ExtendTangentLinesToChart);
    }
}
