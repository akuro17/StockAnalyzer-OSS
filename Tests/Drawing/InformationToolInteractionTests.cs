...
public class InformationToolInteractionTests
{
    ...
    public void InformationObject_IndividualProperties_DefaultsAndOverrides()
    {
        ...
        Assert.Equal(10, infoObj.FillOpacity);
    ...
    [Fact]
    public void InformationObject_Render_ZeroOpacity_RendersCleanly()
    {
        using var bitmap = new SkiaSharp.SKBitmap(800, 600);
        using var canvas = new SkiaSharp.SKCanvas(bitmap);
        var transform = new LinearCoordinateTransform(
        ...
        using var infoObj = new InformationObject();
        infoObj.FillOpacity = 0;
        var time = new DateTime(2026, 1, 15);
        var candle = new CandleInformation("2026/01/15", "100.000", "105.000", "99.000", "104.000", "1,000", "+4.000", "+4.00%", StockAnalyzer.Core.Theme.ThemeColors.Dark.SemanticPlus);
        infoObj.Snapshot = new ChartInformationSnapshot(time, candle, Array.Empty<IndicatorInformationItem>(), Array.Empty<DrawingInformationItem>());

        infoObj.Render(canvas, transform);
        canvas.Flush();

        Assert.True(infoObj.LastRenderedBounds.Width > 0);
        Assert.True(infoObj.LastRenderedBounds.Height > 0);
    }

    [Fact]
    public void InformationObject_DrawingDocumentCodec_CaptureAndSerialize_ExcludesEphemeralProperties()
    {
        var manager = new ChartObjectManager();
        using var infoObj = new InformationObject();
        infoObj.Points.Add(new ChartPoint(new DateTime(2026, 1, 1), 100m));
        var time = new DateTime(2026, 1, 15);
        var candle = new CandleInformation("2026/01/15", "100.000", "105.000", "99.000", "104.000", "1,000", "+4.000", "+4.00%", StockAnalyzer.Core.Theme.ThemeColors.Dark.SemanticPlus);
        infoObj.Snapshot = new ChartInformationSnapshot(time, candle, Array.Empty<IndicatorInformationItem>(), Array.Empty<DrawingInformationItem>());

        manager.AddObject(infoObj);

        var key = new DrawingDocumentKey("7203", TimeframeType.Daily);
        var state = DrawingDocumentCodec.Capture(manager, key, revision: 1);
        string json = DrawingDocumentCodec.SerializeToJson(state);

        Assert.NotNull(json);
        Assert.DoesNotContain("\"snapshot\"", json, StringComparison.OrdinalIgnoreCase);
        Assert.DoesNotContain("\"lastRenderedBounds\"", json, StringComparison.OrdinalIgnoreCase);

        // Deserialization and Materialization should succeed without unrecognized parameter errors
        var deserialized = DrawingDocumentCodec.DeserializeFromJson(json);
        using var doc = DrawingDocumentCodec.Materialize(deserialized);
        Assert.Single(doc.Objects[ChartDrawingContextType.Standard]);
        var restoredInfo = Assert.IsType<InformationObject>(doc.Objects[ChartDrawingContextType.Standard][0]);
        Assert.Null(restoredInfo.Snapshot);
    }

    [Fact]
    public void InformationObject_DrawingEditCoordinator_ExecuteEdit_DoesNotThrow()
    {
        var manager = new ChartObjectManager();
        var key = new DrawingDocumentKey("7203", TimeframeType.Daily);
        ...
        infoObj.Snapshot = new ChartInformationSnapshot(
            new DateTime(2026, 1, 15),
            new CandleInformation("2026/01/15", "100.000", "105.000", "99.000", "104.000", "1,000", "+4.000", "+4.00%", StockAnalyzer.Core.Theme.ThemeColors.Dark.SemanticPlus),
            Array.Empty<IndicatorInformationItem>(),
            Array.Empty<DrawingInformationItem>()
        );

        // Simulates the exact call in ChartInteractionController when placing the Information tool
        var result = coordinator.ExecuteEdit(DrawingOperationKind.Add, () =>
        {
            manager.AddObject(infoObj);
            manager.SelectObject(infoObj.Id);
        });

        Assert.True(result.IsSuccess);
        Assert.Equal(1, coordinator.UndoCount);
        Assert.Same(infoObj, manager.InformationObject);
        ...
        Assert.NotNull(manager.InformationObject);
    }

    [Fact]
    public void InformationObject_DrawingEditCoordinator_Cancel_PreservesSnapshot()
    {
        var manager = new ChartObjectManager();
        var key = new DrawingDocumentKey("7203", TimeframeType.Daily);
        var session = new DrawingDocumentSession(key);
        ...
        var infoObj = new InformationObject();
        var originalSnapshot = new ChartInformationSnapshot(
            new DateTime(2026, 1, 15),
            new CandleInformation("2026/01/15", "100.000", "105.000", "99.000", "104.000", "1,000", "+4.000", "+4.00%", StockAnalyzer.Core.Theme.ThemeColors.Dark.SemanticPlus),
            Array.Empty<IndicatorInformationItem>(),
            Array.Empty<DrawingInformationItem>()
        );
        infoObj.Snapshot = originalSnapshot;
        ...
        // Simulate beginning a Move drag operation (as happens on double-tap) then cancelling it
        var begin = coordinator.BeginEdit(DrawingOperationKind.Move);
        Assert.True(begin.IsSuccess);

        coordinator.Cancel(begin.Token);

        // After cancel and state restoration, manager.InformationObject must retain the non-null Snapshot
        Assert.NotNull(manager.InformationObject);
        Assert.NotNull(manager.InformationObject.Snapshot);
        Assert.Same(originalSnapshot, manager.InformationObject.Snapshot);
    }
}
