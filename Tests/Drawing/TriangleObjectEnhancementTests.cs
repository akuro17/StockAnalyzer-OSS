...
public class TriangleObjectEnhancementTests
{
    ...
    [Fact]
    public void InteriorAngleText_UsesTriangleColorAndDrawingFontSize()
    {
        ...
        const float expectedDrawingSize = 19f;
        const float unrelatedDetailSize = 27f;

        try
        {
            textColorProperty.SetValue(null, SKColors.Magenta);
            detailFontSizeProperty.SetValue(null, unrelatedDetailSize);
            drawingFontSizeProperty.SetValue(null, expectedDrawingSize);

            var triangle = CreateTriangle();
            triangle.Color = global::Avalonia.Media.Color.FromRgb(12, 34, 56);
            triangle.ShowInteriorAngles = true;
            using var bitmap = Render(triangle, CreateTransform());
            int drawingFontPixelCount = CountPixelsOfColor(bitmap, expectedColor);
            Assert.True(drawingFontPixelCount > 0);
            Assert.Equal(0, CountPixelsOfColor(bitmap, SKColors.Magenta));

            detailFontSizeProperty.SetValue(null, expectedDrawingSize);
            using var detailFontChanged = Render(triangle, CreateTransform());
            Assert.Equal(drawingFontPixelCount, CountPixelsOfColor(detailFontChanged, expectedColor));

            drawingFontSizeProperty.SetValue(null, unrelatedDetailSize);
            using var drawingFontChanged = Render(triangle, CreateTransform());
            Assert.True(CountPixelsOfColor(drawingFontChanged, expectedColor) > drawingFontPixelCount);
        }
        finally
        {
