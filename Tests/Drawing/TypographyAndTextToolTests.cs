...
public class TypographyAndTextToolTests
{
    ...
    [Fact]
    public void LineTextObject_IsTransparent_SkipsDrawingLineGeometry()
    {
        var transform = new DummyCoordinateTransform();
        var p1 = transform.ScreenToChart(new Point(20, 50));
        var p2 = transform.ScreenToChart(new Point(180, 50));

        using var transparentObj = new LineTextObject(p1, p2) { Color = Colors.Red, Thickness = 6, IsTransparent = true };
        using var opaqueObj = new LineTextObject(p1, p2) { Color = Colors.Red, Thickness = 6, IsTransparent = false };

        using var surfaceTransparent = SKSurface.Create(new SKImageInfo(200, 100));
        using var surfaceOpaque = SKSurface.Create(new SKImageInfo(200, 100));

        // Fill background with blue to verify underlying content is visible (untouched)
        surfaceTransparent.Canvas.Clear(SKColors.Blue);
        surfaceOpaque.Canvas.Clear(SKColors.Blue);

        transparentObj.Render(surfaceTransparent.Canvas, transform);
        opaqueObj.Render(surfaceOpaque.Canvas, transform);

        using var bmpTransparent = SKBitmap.FromImage(surfaceTransparent.Snapshot());
        using var bmpOpaque = SKBitmap.FromImage(surfaceOpaque.Snapshot());

        var transparentPixel = bmpTransparent.GetPixel(100, 50);
        var opaquePixel = bmpOpaque.GetPixel(100, 50);

        // Under transparentObj, the underlying canvas remains Blue (not overwritten with line color or ChartBackground)
        Assert.Equal(SKColors.Blue, transparentPixel);
        // Under opaqueObj, the red line is drawn
        Assert.Equal(SKColors.Red, opaquePixel);

        // Verify backward compatibility: MatchBackgroundColor delegates to IsTransparent
#pragma warning disable CS0618
        transparentObj.MatchBackgroundColor = false;
        Assert.False(transparentObj.IsTransparent);
        transparentObj.MatchBackgroundColor = true;
        Assert.True(transparentObj.IsTransparent);
#pragma warning restore CS0618
    }

    [Fact]
    public void CurveLineTextObject_IsTransparent_SkipsDrawingCurveGeometry()
    {
        var transform = new DummyCoordinateTransform();
        var p1 = transform.ScreenToChart(new Point(20, 50));
        var p2 = transform.ScreenToChart(new Point(100, 50));
        var p3 = transform.ScreenToChart(new Point(180, 50));

        using var transparentCurve = new CurveLineTextObject(new[] { p1, p2, p3 }) { Color = Colors.Red, Thickness = 6, IsTransparent = true };
        using var opaqueCurve = new CurveLineTextObject(new[] { p1, p2, p3 }) { Color = Colors.Red, Thickness = 6, IsTransparent = false };
        ...
        transparentCurve.Render(surfaceTransparent.Canvas, transform);
        opaqueCurve.Render(surfaceOpaque.Canvas, transform);
