...
public class DeferredComputationRecalculatorTests
{
    ...
    [Fact]
    public void TryRecalculate_AnchoredVwapObject_RecalculatesAndReturnsTrue()
    {
        var anchor = new ChartPoint(new DateTime(2025, 1, 3), 100m);
        var obj = new AnchoredVwapObject(anchor);

        Assert.Empty(obj.VwapCurve);

        bool handled = DeferredComputationRecalculator.TryRecalculate(obj, BuildCandles());

        Assert.True(handled);
        Assert.NotEmpty(obj.VwapCurve);
        // BuildCandles() has 10 days (Jan 1 to Jan 10) -> 8 candles from Jan 3 inclusive
        Assert.Equal(8, obj.VwapCurve.Count);
        Assert.Equal(new DateTime(2025, 1, 3), obj.VwapCurve[0].Time);
    }

    [Fact]
    public void TryRecalculate_AnchoredVwapObject_WhenAnchorMoved_RecalculatesFromNewAnchor()
    {
        var candles = BuildCandles();
        var obj = new AnchoredVwapObject(new ChartPoint(new DateTime(2025, 1, 2), 100m));
        DeferredComputationRecalculator.TryRecalculate(obj, candles);
        Assert.Equal(9, obj.VwapCurve.Count);

        // Simulate moving the control point to Jan 6 (release/decision point)
        obj.Points[0] = new ChartPoint(new DateTime(2025, 1, 6), 150m);
        bool handled = DeferredComputationRecalculator.TryRecalculate(obj, candles);

        Assert.True(handled);
        Assert.Equal(5, obj.VwapCurve.Count);
        Assert.Equal(new DateTime(2025, 1, 6), obj.VwapCurve[0].Time);
    }
}
