...
public class ParameterLocalizationTests
{
    ...
    public void Build_JapaneseLocale_ResolvesCategoryAndPropertyNames()
    {
        try
        {
            ...
            Assert.Contains("転換線", labelTexts);
            Assert.Contains("基準線", labelTexts);
            Assert.Contains("先行スパンB", labelTexts);
            Assert.Contains("変位（遅行スパン）", labelTexts);
        }
        finally
        {
    ...
    public void Build_EnglishLocale_ResolvesCategoryAndPropertyNames()
    {
        try
        {
            ...
            Assert.Contains("Tenkan-sen", labelTexts);
            Assert.Contains("Kijun-sen", labelTexts);
            Assert.Contains("Senkou Span B", labelTexts);
            Assert.Contains("Displacement", labelTexts);
        }
        finally
        {
            LocalizationManager.Instance.Initialize("en");
