...
public class DrawingSettingsDialogTests
{
    ...
    public void ColorPickers_StretchToFullContentWidth_ForFrechetProjectionObject()
    {
        ...
        try
        {
            ...
            Assert.False(double.IsNaN(genericColorPicker!.Width));
            Assert.False(double.IsNaN(unmatchedPicker!.Width));
            Assert.False(double.IsNaN(fillColorPicker!.Width));
            Assert.Equal(contentPanel.Bounds.Width, genericColorPicker.Width, 3);
            Assert.Equal(contentPanel.Bounds.Width, unmatchedPicker.Width, 3);
            Assert.Equal(contentPanel.Bounds.Width, fillColorPicker.Width, 3);
        }
        finally
        {
    ...
    public void Wheel_OverBlankGapBetweenControls_AlsoScrollsTheForm()
    {
        ...
        try
        {
            ...
            var contentPanel = dialog.FindControl<StackPanel>("SettingsContentPanel")!;
            var firstChild = contentPanel.Children
                .OfType<Control>()
            ...
            var gapPoint = contentPanel.TranslatePoint(
