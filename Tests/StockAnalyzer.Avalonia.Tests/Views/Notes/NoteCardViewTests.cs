...
public class NoteCardViewTests
{
    ...
    /// <summary>sa_implement (Note Card Visual Polish, Phase 1): the ticker code sits in a
    /// rounded-corner frame and stays Bold; a Note without a ticker hides the whole frame.</summary>
    [AvaloniaFact]
    public void TickerCode_IsWrappedInRoundedBorderAndBold_AndFrameHiddenWithoutTicker()
    {
        var withTicker = new Note(Guid.NewGuid(), "body", DateTime.Now, DateTime.Now) { RelatedTicker = "AAPL" };
        var (_, window, view) = MountCard(withTicker);
        try
        {
            var frame = view.FindControl<Border>("TickerFrameBorder")!;
            var text = view.FindControl<TextBlock>("TickerTextBlock")!;
            Assert.Same(frame, text.Parent);
            Assert.True(frame.CornerRadius.TopLeft > 0);
            Assert.True(frame.BorderThickness.Left > 0);
        ...
        var (_, window2, view2) = MountCard(new Note(Guid.NewGuid(), "body", DateTime.Now, DateTime.Now));
        try
        {
            Assert.False(view2.FindControl<Border>("TickerFrameBorder")!.IsVisible);
        }
        finally
        {
            window2.Close();
    ...
    /// <summary>sa_implement (Note Card Visual Polish, Phase 2): hashtag and URL inline text is Bold
    /// with no underline; the posted-time text has no underline.</summary>
    [AvaloniaFact]
    public void InlineHashtagUrlAndPostedTime_HaveNoUnderline_AndHashtagUrlAreBold()
    {
        var note = new Note(Guid.NewGuid(), "a #ai https://example.com/a b", DateTime.Now, DateTime.Now)
        {
            Hashtags = ImmutableArray.Create("ai"),
            LinkUrls = ImmutableArray.Create("https://example.com/a"),
        };
        ...
        try
        {
            var buttons = view.FindControl<TextBlock>("BodyTextBlock")!.Inlines!
                .OfType<InlineUIContainer>().Select(c => c.Child).OfType<Button>().ToList();
            Assert.Equal(2, buttons.Count);
            foreach (var button in buttons)
            {
                var content = Assert.IsType<TextBlock>(button.Content);
                Assert.Equal(FontWeight.Bold, content.FontWeight);
                Assert.True(content.TextDecorations is null || content.TextDecorations.Count == 0);
            }

            var posted = view.FindControl<TextBlock>("PostedTimeTextBlock")!;
            Assert.True(posted.TextDecorations is null || posted.TextDecorations.Count == 0);
        }
        finally
        {
            window.Close();
    ...
    /// <summary>sa_implement (Note Card Visual Polish, Phase 3): the 4 action buttons are flat
    /// (transparent background/border) and their PathIcon fills the whole button.</summary>
    [AvaloniaFact]
    public void ActionButtons_AreFlat_AndIconFillsButton()
    {
        var note = new Note(Guid.NewGuid(), "body", DateTime.Now, DateTime.Now);
        var iconButtonSize = Assert.IsType<double>(Application.Current!.FindResource("Note.ActionIconButtonSize"));
        var (item, window, view) = MountCard(note);
        try
        {
            var buttons = view.GetVisualDescendants().OfType<Button>()
                .Where(b => b.Content is PathIcon).ToList();
            ...
            foreach (var button in buttons)
            {
                ...
                Assert.Equal(Colors.Transparent, Assert.IsAssignableFrom<ISolidColorBrush>(button.BorderBrush).Color);
                Assert.Equal(new Thickness(0), button.Padding);
                Assert.Equal(new Thickness(0), button.BorderThickness);
                Assert.Equal(iconButtonSize, button.Width);
                Assert.Equal(iconButtonSize, button.Height);
                Assert.Equal(iconButtonSize, icon.Width);
                Assert.Equal(iconButtonSize, icon.Height);
                var bodyText = item.BodyTextColor;
                Assert.Equal(
                    Color.FromArgb(bodyText.A, bodyText.R, bodyText.G, bodyText.B),
