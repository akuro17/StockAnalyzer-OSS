    ...
    public async Task ComposePanel_ManyPendingAttachments_StaysHeightCapped_ViaInternalScroll()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // sa_minimal_fix (stale-comment correction): SidebarScrollViewerTheme's ControlTemplate
                // Grid is ColumnDefinitions="*,Auto" (consolidated from a separate "Auto,Auto"
                // sidebar-only theme - see the SidebarScrollViewerTheme doc comment in Assets/Styles.axaml,
                // "SAで制約確認 remediation, Finding 3"). What actually keeps this WrapPanel-based content
                // wrapping instead of laying out on one unbounded-width row is PART_ContentPresenter's
                // CanHorizontallyScroll="False", not the column's Auto/Star mode - that same doc comment
                // shows both column modes measure this kind of fixed-size content identically. Asserts
                // wrapping still occurs.
                ...
                // sa_minimal_fix (common UI spec for layout-width optimization and scrollbar display control):
