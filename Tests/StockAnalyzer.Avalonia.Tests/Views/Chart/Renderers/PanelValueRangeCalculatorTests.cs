...
namespace StockAnalyzer.Avalonia.Tests.Views.Chart.Renderers
{
    public class PanelValueRangeCalculatorTests
    {
        ...
        [Fact]
        public void Calculate_ForPolarPhase_ExcludesMainDegreesAndPhaseStabilityFromRange()
        {
            // Polar Phase: Main is 0-360 degrees and PhaseStability is a degree-scaled diagnostic;
            // neither is charted. Only NormalizedInPhase/NormalizedQuadrature (-1..1) are drawn.
            var setting = new CoreIndicatorSettings
            {
                Id = "polar_phase_test_1",
            ...
            {
                { "Main", new List<decimal?> { 10m, 90m, 180m, 270m, 350m } },
                { "NormalizedInPhase", new List<decimal?> { -0.9m, 0.1m, 0.8m, -0.3m, 0.5m } },
                { "NormalizedQuadrature", new List<decimal?> { -0.7m, 0.6m, -0.4m, 0.2m, -0.1m } },
                { "PhaseStability", new List<decimal?> { null, null, 8.2m, 4.1m, 40.0m } }
            };
            var result = IndicatorResult.Success(seriesDict);
            ...
            Assert.True(maxVal <= 1.5m, $"Max value ({maxVal}) should not include the 0-360 Main or degree-scaled PhaseStability.");
            Assert.True(minVal >= -1.5m, $"Min value ({minVal}) should not include the 0-360 Main or degree-scaled PhaseStability.");
        }
