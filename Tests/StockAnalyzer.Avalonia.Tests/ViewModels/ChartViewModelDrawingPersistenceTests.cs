...
public class ChartViewModelDrawingPersistenceTests : IDisposable
{
    ...
    // Clean-up grace period for fire-and-forget writes that have no awaitable handle and no observable effect (see the tests that use it).
    private static readonly TimeSpan QueuedSaveDrainDelay = TimeSpan.FromMilliseconds(ChartConstants.DataLoadDebounceDelay + 350);

    private int PersistedObjectCount(string ticker)
        => _sharedRepository.Load(ticker, TimeframeType.Daily)?.GetValueOrDefault(ChartDrawingContextType.Standard)?.Count ?? 0;

    [Fact]
    public async Task SwitchingSymbolAwayAndBack_PreservesDrawing()
    {
        _sut.Symbol = TickerA;
        await ChartViewModelTestWaits.AwaitLoadsAsync(_sut);
        ...
        // No wait: AddObject alone persists nothing; the drawing is written by the awaited "save the outgoing drawings" step of the switch below.
    ...
    public async Task DeleteThenImmediatelySwitchSymbolAwayAndBack_KeepsDrawingDeleted()
    {
        ...
        // No wait here: switch immediately, before the fire-and-forget save necessarily lands.
        ...
        _sut.Symbol = TickerA;
        await ChartViewModelTestWaits.AwaitLoadsAsync(_sut); // the load that would restore the stale drawing has completed, so "empty" below is a real result
    ...
    public async Task Dispose_FlushesPendingDrawingSynchronously_WithoutWaitingForFireAndForgetSave()
    {
        ...
        var trendLine = new TrendLineObject(
            ...
            new ChartPoint(new DateTime(2024, 1, 2), 12m));
        _sut.ObjectManager.AddObject(trendLine);
        // No wait: Dispose() must flush this synchronously, not rely on the fire-and-forget save.
        ...
        // explicit Dispose() flush above) land before the fixture deletes TickerC's file,
        // so it can't resurrect the file after cleanup and leak into a later test run.
        // The queued write has no awaitable handle and its effect (the file already exists from the flush) cannot be observed, so this is a bounded
        // grace period for CLEAN-UP only: it asserts nothing, so a slow run can at worst leave a stray file, never fail this test.
        await Task.Delay(QueuedSaveDrainDelay);
    }
    ...
    public async Task FlushPendingDrawings_PersistsWithoutDisposing_AndViewModelRemainsUsable()
    {
        ...
        var trendLine = new TrendLineObject(
            ...
            new ChartPoint(new DateTime(2024, 1, 2), 12m));
        _sut.ObjectManager.AddObject(trendLine);
        // No wait: FlushPendingDrawings() must persist this synchronously on its own, exactly
        ...
        // AddObject's own fire-and-forget auto-save is queued too; clean-up grace period only (see Dispose_FlushesPendingDrawingSynchronously...), it asserts nothing.
        await Task.Delay(QueuedSaveDrainDelay);
    }
    ...
    public async Task SecondChartViewModelInstance_RacingSameTickerWithoutEverSeeingRealContent_DoesNotClobberFirstInstancesDrawing()
    {
        ...
        try
        {
            ...
            await ChartViewModelTestWaits.AwaitLoadsAsync(panel);

            // Draw on the "main" instance only.
            var trendLine = new TrendLineObject(
                new ChartPoint(new DateTime(2024, 1, 1), 10m),
                new ChartPoint(new DateTime(2024, 1, 2), 12m));
            if (_sut.DrawingEditCoordinator != null)
            {
                _sut.DrawingEditCoordinator.ExecuteEdit(StockAnalyzer.Core.Models.Drawing.DrawingOperationKind.Add, () => _sut.ObjectManager.AddObject(trendLine));
            }
            else
            {
                _sut.ObjectManager.AddObject(trendLine);
                _sut.PersistCurrentDrawings();
            }
            await ChartViewModelTestWaits.EventuallyAsync(() => Assert.Equal(1, PersistedObjectCount(TickerD))); // let the main instance's fire-and-forget save land on disk
            ...
            try
            {
                ...
                await ChartViewModelTestWaits.AwaitLoadsAsync(afterRestart);
    ...
    [Fact]
    public async Task SyncFromManager_PruningDormantLayer_PersistsCleanedFileToDisk()
    {
        var seededPayload = new ChartDrawingPayload
        {
            Objects = new System.Collections.Generic.Dictionary<ChartDrawingContextType, System.Collections.Generic.List<IChartObject>>
            {
                [ChartDrawingContextType.Standard] = new System.Collections.Generic.List<IChartObject>()
            },
            Layers = new System.Collections.Generic.Dictionary<ChartDrawingContextType, System.Collections.Generic.List<DrawingLayerRecord>>
            {
                [ChartDrawingContextType.Standard] = new System.Collections.Generic.List<DrawingLayerRecord>
                {
                    new DrawingLayerRecord(Guid.NewGuid(), "Main Layer", PanelKey.Main, isVisible: true, isEditLocked: false, Array.Empty<Guid>()),
                    new DrawingLayerRecord(Guid.NewGuid(), "Dormant Layer", PanelKey.Indicator("dormant-ind-1"), isVisible: true, isEditLocked: false, Array.Empty<Guid>())
                }
            }
        };

        await _sharedRepository.SavePayloadAsync(TickerD, TimeframeType.Daily, seededPayload);

        _sut.Symbol = TickerD;
        await ChartViewModelTestWaits.AwaitLoadsAsync(_sut);

        using var objectsVm = new DrawingObjectsViewModel(_sut, new SynchronousDispatcherService());
        objectsVm.SyncFromManager();

        // The cleaned file is written by a fire-and-forget save, so the disk state is awaited by condition (a positive one: "the file now holds one layer").
        await ChartViewModelTestWaits.EventuallyAsync(() =>
        {
            Assert.Single(objectsVm.Layers);
            Assert.Equal(PanelKey.Main, objectsVm.Layers[0].Panel);

            var reloaded = _sharedRepository.LoadPayload(TickerD, TimeframeType.Daily);
            Assert.NotNull(reloaded);
            Assert.Single(reloaded!.Layers[ChartDrawingContextType.Standard]);
            Assert.Equal(PanelKey.Main, reloaded.Layers[ChartDrawingContextType.Standard][0].Panel);
        });
    }
}
