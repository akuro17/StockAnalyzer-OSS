...
public class IndicatorSettingsDialogViewModelTests
{
    ...
    public void AddSelectedLibraryIndicatorCommand_AddsConfiguredIndicatorToIndicatorsList()
    {
        ...
        Assert.True(added.UseShortName);
    ...
    [Fact]
    public void AddSelectedLibraryIndicatorCommand_WhenLongName_AddsWithLongNameAndPeriod()
    {
        var vm = CreateViewModel();
        vm.Initialize(new List<CoreIndicatorSettings>());
        ...
        Assert.Single(vm.Indicators);
        var added = vm.Indicators[0];
        Assert.Equal("Simple Moving Average (50)", added.DisplayName);
        Assert.False(added.UseShortName);
    }
    ...
    [Fact]
    public void SelectedIndicator_VolumeProfile_ShowPriceTypeAndSourceIndicator_IsFalse()
    {
        var vm = CreateViewModel();
        var vp = new CoreIndicatorSettings
        {
            TypeEnum = IndicatorType.VolumeProfile,
            DisplayName = "Volume Profile"
        };

        vm.Initialize(new[] { vp });
        vm.SelectedIndicator = vp;

        Assert.False(vm.ShowPriceTypeAndSourceIndicator);
        Assert.False(vm.IsPriceTypeEnabled);
        Assert.Null(vm.PriceTypeToolTip);
    }

    [Fact]
    public void SelectedIndicator_TimeAtPrice_ShowPriceTypeAndSourceIndicator_IsTrue()
    {
        var vm = CreateViewModel();
        var tap = new CoreIndicatorSettings
        {
            TypeEnum = IndicatorType.TimeAtPrice,
            DisplayName = "Time at Price"
        };

        vm.Initialize(new[] { tap });
        vm.SelectedIndicator = tap;

        Assert.True(vm.ShowPriceTypeAndSourceIndicator);
        Assert.True(vm.IsPriceTypeEnabled);
        Assert.Null(vm.PriceTypeToolTip);
    }

    [Fact]
    public void SelectedIndicator_TimeAtPrice_WhenBaseIndicatorSelected_IsPriceTypeEnabled_IsFalse()
    {
        var vm = CreateViewModel();
        ...
        var tap = new CoreIndicatorSettings
        {
            ...
            TypeEnum = IndicatorType.TimeAtPrice,
            DisplayName = "Time at Price",
            SourceIndicatorId = "rsi-1"
        };

        vm.Initialize(new[] { rsi, tap });
        vm.SelectedIndicator = tap;

        Assert.True(vm.ShowPriceTypeAndSourceIndicator);
        Assert.False(vm.IsPriceTypeEnabled);
        Assert.Equal(LocalizationManager.Instance["Tooltip_PriceTypeDisabled"], vm.PriceTypeToolTip);
    }

    [Fact]
    public void SelectedIndicator_Null_PropertiesAreFalseAndToolTipIsNull()
    {
        var vm = CreateViewModel();
        vm.SelectedIndicator = null;
