...
public class ExportChartImageDialogViewModelTests
{
    ...
    private static ExportChartImageDialogViewModel CreateLayeredSut(
        Mock<IChartImageExportService> exportMock,
        LayeredDrawingScene? scene)
    {
        var candles = new List<CoreCandleData>();
        for (int i = 0; i < 10; i++)
        {
        ...
        var snapshot = new ChartDataSnapshot(candles: candles, startIndex: 0, count: 10);
        var layout = ChartLayoutService.CreateLayout(new Rect(0, 0, 800, 600), ChartType.Candlestick);
        var transform = new GenericCoordinateTransform(ChartAxisMode.Time, 800, 600);
        var themeManager = new ThemeManager();
        var config = new CandlestickRenderConfig(
            ThemeManager: themeManager, ChartType: ChartType.Candlestick, CurrentPrice: 105.0,
            BullishColor: IndicatorColor.FromRgb(0, 200, 0), BearishColor: IndicatorColor.FromRgb(200, 0, 0),
            NeutralColor: IndicatorColor.FromRgb(128, 128, 128),
            ReversalLabelColor: new NamedColor("Green", "#00FF00"), PriceLabelColor: new NamedColor("White", "#FFFFFF"),
            VisibleStartIndex: 0, VisibleCandleCount: 10, Transform: transform,
            MousePosition: new StockAnalyzer.Core.Models.Point(0, 0),
            ShowMultiWavePatterns: false, ShowGhostProjections: false, GhostProjectionFontSize: 12f,
            ShowGhostLabelsOnHoverOnly: false, RenderScaling: 1.0, IsSubWindowVisible: false, InvertOscillator: false,
            DefaultDrawingThickness: 1.0, CrosshairLabelVisible: false, MultiWavePatternMaxLines: 0,
            MultiWaveBullishColor: null, MultiWaveBearishColor: null);

        return new ExportChartImageDialogViewModel(
            snapshot, layout, transform, new ChartObjectManager(), new CandleStickRenderer(), config,
            ChartType.Candlestick, new RulerRenderer(), themeManager, exportMock.Object,
            new Mock<IDialogService>().Object, new Mock<ITemplateService>().Object,
            "7203", "Company", "Daily", null, scene);
    }

    [Fact]
    public async Task SaveCommand_WithLayeredScene_AndLayeredCapableService_UsesLayeredRenderPath()
    {
        var exportMock = new Mock<IChartImageExportService>();
        var layeredMock = exportMock.As<ILayeredChartImageExportService>();
        layeredMock
            .Setup(s => s.RenderLayeredChartImageAsync(
                It.IsAny<ChartDataSnapshot>(), It.IsAny<ChartLayoutContext>(), It.IsAny<ICoordinateTransform>(),
                It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
                It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
                It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>(), It.IsAny<LayeredDrawingScene?>()))
            .ReturnsAsync(new byte[] { 0x89, 0x50, 0x4E, 0x47 });
        exportMock.Setup(s => s.ExportToFileAsync(It.IsAny<string>(), It.IsAny<byte[]>())).ReturnsAsync(true);

        var scene = new LayeredDrawingScene();
        var sut = CreateLayeredSut(exportMock, scene);
        sut.SaveDirectory = Path.GetTempPath();
        sut.FileName = $"TestExport_{Guid.NewGuid():N}";
        sut.RequestClose = _ => { };
        ...
        layeredMock.Verify(s => s.RenderLayeredChartImageAsync(
            It.IsAny<ChartDataSnapshot>(), It.IsAny<ChartLayoutContext>(), It.IsAny<ICoordinateTransform>(),
            It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
            It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
            It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>(), scene), Times.Once);
        exportMock.Verify(s => s.RenderChartImageAsync(
            It.IsAny<ChartDataSnapshot>(), It.IsAny<ChartLayoutContext>(), It.IsAny<ICoordinateTransform>(),
            It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
            It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
            It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>()), Times.Never);
    }

    [Fact]
    public async Task SaveCommand_WithoutLayeredScene_UsesLegacyRenderPath()
    {
        var exportMock = new Mock<IChartImageExportService>();
        var layeredMock = exportMock.As<ILayeredChartImageExportService>();
        exportMock
            .Setup(s => s.RenderChartImageAsync(
                ...
                It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
                It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
                It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>()))
            .ReturnsAsync(new byte[] { 0x89, 0x50, 0x4E, 0x47 });
        exportMock.Setup(s => s.ExportToFileAsync(It.IsAny<string>(), It.IsAny<byte[]>())).ReturnsAsync(true);

        var sut = CreateLayeredSut(exportMock, null);
        sut.SaveDirectory = Path.GetTempPath();
        sut.FileName = $"TestExport_{Guid.NewGuid():N}";
        sut.RequestClose = _ => { };
        ...
        exportMock.Verify(s => s.RenderChartImageAsync(
            ...
            It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
            It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
            It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>()), Times.Once);
        layeredMock.Verify(s => s.RenderLayeredChartImageAsync(
            It.IsAny<ChartDataSnapshot>(), It.IsAny<ChartLayoutContext>(), It.IsAny<ICoordinateTransform>(),
            It.IsAny<ChartImageExportOptions>(), It.IsAny<IThemeManager>(), It.IsAny<ChartObjectManager>(),
            It.IsAny<IChartRenderer>(), It.IsAny<IChartRenderConfig>(), It.IsAny<ChartType>(),
            It.IsAny<RulerRenderer>(), It.IsAny<ChartImageMetadata>(), It.IsAny<LayeredDrawingScene?>()), Times.Never);
    }
