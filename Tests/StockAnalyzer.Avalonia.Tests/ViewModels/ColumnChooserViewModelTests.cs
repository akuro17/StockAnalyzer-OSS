...
public class ColumnChooserViewModelTests
{
    ...
    [Fact]
    public async Task Templates_Save_Load_Delete_Operations()
    {
        // Arrange
        var activeColumns = new[] { "Symbol", "ReturnOnEquity", "TrailingPE" };
        var sut = new ColumnChooserViewModel(_allColumns, activeColumns);
        
        // Ensure background LoadTemplatesAsync settled
        for (int i = 0; i < 50; i++)
        {
            await Task.Delay(10);
        }
        ...
        // Act: Save template
        await sut.SaveTemplateCommand.ExecuteAsync(null);

        // Assert: Template added
        var savedTemplate = sut.Templates.FirstOrDefault(t => t.Name == "TestTemplate");
        Assert.NotNull(savedTemplate);
        Assert.Equal("TestTemplate", savedTemplate.Name);
        Assert.Equal(3, savedTemplate.ColumnNames.Count);
        ...
        // Act: Load template
        await sut.LoadTemplateCommand.ExecuteAsync(savedTemplate);
        ...
        sut.SelectedTemplate = savedTemplate;
        ...
        // Act: Delete template
        await sut.DeleteTemplateCommand.ExecuteAsync(savedTemplate);

        // Assert: Removed
        Assert.DoesNotContain(sut.Templates, t => t.Name == "TestTemplate");
        Assert.Null(sut.SelectedTemplate);
    }

    [Fact]
    public async Task AppendTemplateCommand_PreservesExistingActiveColumnsAndAppendsNewOnes()
    {
        // Arrange: Start with "Symbol" and "ReturnOnEquity"
        var activeColumns = new[] { "Symbol", "ReturnOnEquity" };
        var sut = new ColumnChooserViewModel(_allColumns, activeColumns);
        ...
        var template = new StockAnalyzer.Core.Models.Templates.ColumnTemplate
        {
            Id = Guid.NewGuid(),
            SchemaVersion = 1,
            Name = "ValuationPreset",
            ColumnNames = new List<string> { "TrailingPE", "ReturnOnAssets" }
        };
        sut.Templates.Add(template);

        // Act: Append template
        await sut.AppendTemplateCommand.ExecuteAsync(template);

        // Assert: Both original and appended columns must be active
        var activeNow = sut.GetActiveColumnNames();
        Assert.Equal(4, activeNow.Count);
        Assert.Contains("Symbol", activeNow);
        Assert.Contains("ReturnOnEquity", activeNow);
        Assert.Contains("TrailingPE", activeNow);
        Assert.Contains("ReturnOnAssets", activeNow);
    }

    [Fact]
    public void Dispose_UnhooksEventHandlers_NoExceptions()
    {
        // Arrange
        var activeColumns = new[] { "Symbol", "ReturnOnEquity" };
        var sut = new ColumnChooserViewModel(_allColumns, activeColumns);
        ...
        // Mutating items after dispose should not throw
        var item = sut.AllItems.First(x => x.MemberName == "ReturnOnEquity");
        item.IsActive = false;
        sut.AllItems.Add(ColumnItemViewModel.Create(new WatchlistColumnMetadata("Col_Test", "Test"), false));
    ...
    [Fact]
    public async Task LoadTemplateAsync_InvalidTemplate_DoesNotMutateState()
    {
        // Arrange
        var activeColumns = new[] { "Symbol", "ReturnOnEquity" };
        var sut = new ColumnChooserViewModel(_allColumns, activeColumns);

        var invalidTemplate = new StockAnalyzer.Core.Models.Templates.ColumnTemplate
        {
            Id = Guid.Empty, // Invalid ID -> validation will fail
            Name = "Invalid",
            ColumnNames = new List<string> { "TrailingPE" }
        };

        // Act
        await sut.LoadTemplateCommand.ExecuteAsync(invalidTemplate);

        // Assert: Active columns remain unchanged
        var activeNow = sut.GetActiveColumnNames();
        Assert.Equal(2, activeNow.Count);
        Assert.Contains("Symbol", activeNow);
        Assert.Contains("ReturnOnEquity", activeNow);
        Assert.DoesNotContain("TrailingPE", activeNow);
    }
}
