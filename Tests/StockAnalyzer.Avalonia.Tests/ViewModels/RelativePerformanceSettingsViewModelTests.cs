...
public class RelativePerformanceSettingsViewModelTests
{
    ...
    [Fact]
    public void AppendPresetCommand_PreservesExistingSymbolsAndAppendsNewNonDuplicates()
    {
        // Arrange
        var settingsManager = new MockChartSettingsManager();
        var vm = new RelativePerformanceSettingsViewModel(
        ...
        {
            Name = "TechAndSemis",
            Targets = new List<string> { "MSFT", "NVDA", "GOOGL" }
        };
        vm.SavedPresets.Add(preset);

        // Act
        vm.AppendPresetCommand.Execute(preset);

        // Assert: AAPL (original), MSFT (duplicate ignored), NVDA (added), GOOGL (added)
        Assert.Equal(new List<string> { "AAPL", "MSFT", "NVDA", "GOOGL" }, vm.SymbolList);
        Assert.True(vm.IsModified);
    }
}
