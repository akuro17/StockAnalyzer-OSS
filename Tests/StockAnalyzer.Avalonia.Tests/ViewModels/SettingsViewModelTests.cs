...
public class SettingsViewModelTests
{
    ...
    private sealed class CountingInspector : StockAnalyzer.Core.Services.IPythonEnvironmentInspector
    {
        public int Probes;

        public System.Threading.Tasks.Task<StockAnalyzer.Core.Services.PythonEnvironmentSnapshot> InspectAsync(
            System.Collections.Generic.IReadOnlyCollection<string> distributionNames,
            System.Threading.CancellationToken ct = default)
        {
            System.Threading.Interlocked.Increment(ref Probes);
            return System.Threading.Tasks.Task.FromResult(StockAnalyzer.Core.Services.PythonEnvironmentSnapshot.NotInstalled);
        }
    }

    [Fact]
    public void NavigatingBackToTheCachedAboutPage_ProbesTheEnvironmentAgain()
    {
        var inspector = new CountingInspector();
        var services = new ServiceCollection();
        services.AddSingleton<IThemeManager>(_themeManager);
        services.AddTransient<ThemeSettingsViewModel>();
        services.AddSingleton<StockAnalyzer.Core.Services.IPythonEnvironmentInspector>(inspector);
        services.AddTransient<AboutSettingsViewModel>();
        var sut = new SettingsViewModel(services.BuildServiceProvider());
        var about = SettingsConstants.Categories.First(c => c.Key == SettingsConstants.Keys.About);
        var theme = SettingsConstants.Categories.First(c => c.Key == SettingsConstants.Keys.Theme);

        sut.SelectedCategory = about;
        Assert.Equal(1, inspector.Probes); // first visit: the constructor probe only

        sut.SelectedCategory = theme;
        sut.SelectedCategory = about;

        Assert.Equal(2, inspector.Probes); // cached page: packages may have been installed on another page meanwhile
    }
    ...
    [Fact]
    public void ButtonBackground_Modification_UpdatesThemeManager()
    {
        // Arrange
        var themeCategory = SettingsConstants.Categories.First(c => c.Key == SettingsConstants.Keys.Theme);
        _sut.SelectedCategory = themeCategory;
        ...
        var originalColor = _themeManager.GetCurrentColors()[ThemeColorKey.ButtonBackground];
        var customColor = new IndicatorColor(255, 12, 34, 56);

        // Act
        themePage.ButtonBackground = customColor;

        // Assert: ThemeManager updated immediately
        Assert.Equal(customColor, _themeManager.GetCurrentColors()[ThemeColorKey.ButtonBackground]);

        // Cleanup
        _themeManager.UpdateSingleColor(ThemeColorKey.ButtonBackground, originalColor);
    }
}
