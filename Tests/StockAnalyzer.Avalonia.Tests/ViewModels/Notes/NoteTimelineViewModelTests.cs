...
public class NoteTimelineViewModelTests
{
    ...
    public async Task RefreshAsync_AppliesFilterCriteria_ChangesDisplayedCount()
    {
        ...
        try
        {
            ...
            timeline.FilterCriteria = new NoteFilterCriteria { SelectedTickers = ImmutableHashSet.Create("7203.T") };
            await timeline.RefreshAsync();
    ...
    /// <summary>sa_improve (Note Tab Polish, Task 4): the posted-Notes timeline list must not be
    /// visible while either the inline compose/edit panel or the individual detail page is showing
    /// instead - IsTimelineListVisible is the single computed source of truth the View binds to.</summary>
    [Fact]
    public async Task IsTimelineListVisible_IsTrueByDefault()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            Assert.True(timeline.IsTimelineListVisible);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    [Fact]
    public async Task IsTimelineListVisible_IsFalse_WhileComposing_AndTrueAgainAfterCancelEdit()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            Assert.False(timeline.IsTimelineListVisible);

            timeline.CancelEditCommand.Execute(null);
            Assert.True(timeline.IsTimelineListVisible);
        }
        finally
        {
    ...
    [Fact]
    public async Task IsTimelineListVisible_IsFalse_WhileDetailPageOpen_AndTrueAgainAfterBackToList()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
    ...
    /// contain one entry per Ticker, per Watchlist/Portfolio/Filter node and per registered tag,
    /// plus the always-present NoTicker sentinel (Task A, SAで実装 Note Tab Enhancements) - all
    /// six kinds represented, each carrying the right DisplayName.</summary>
    [Fact]
    public async Task LoadAvailableFilterOptionsAsync_PopulatesAvailableSearchSuggestions_WithAllSixKinds()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            Assert.Contains(timeline.AvailableSearchSuggestions, s => s.Kind == NoteScopeSuggestionKind.NoTicker);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// <summary>sa_implement Task 2: picking a Ticker suggestion adds a chip to SelectedTickerCodes
    /// and clears a previously-active Tag selection, matching the "only one active dimension at a time" Converse
    /// requirement in sa_analysis_report.md §6.</summary>
    [Fact]
    public async Task SelectSuggestion_Ticker_AddsChipToSelectedTickerCodes_AndClearsPriorTagSelection()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            Assert.Equal(new[] { "9984.T" }, timeline.SelectedTickerCodes);
            Assert.IsType<AllTickersNode>(timeline.SelectedScopeNode);
    ...
    /// <summary>Task F (SAで実装, Note Tab Enhancements): picking multiple Ticker suggestions
    /// accumulates them as chips (OR filter) instead of replacing a single value.</summary>
    [Fact]
    public async Task SelectSuggestion_MultipleTickers_AccumulateAsChips()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            var marketDataProvider = new FakeMarketDataProvider(availableTickers: new[] { "7203.T", "9984.T" });
            var (timeline, noteRepository, _) = await CreateTimelineAsync(tempDir, marketDataProvider: marketDataProvider);
            await noteRepository.CreateAsync(MakeNote("a", DateTime.Now, "7203.T"));
            await noteRepository.CreateAsync(MakeNote("b", DateTime.Now, "9984.T"));
            await noteRepository.CreateAsync(MakeNote("c", DateTime.Now, "6758.T"));

            timeline.SelectSuggestion(NoteScopeSuggestion.ForTicker("7203.T"));
            timeline.SelectSuggestion(NoteScopeSuggestion.ForTicker("9984.T"));
            await timeline.RefreshAsync();

            Assert.Equal(new[] { "7203.T", "9984.T" }, timeline.SelectedTickerCodes);
            Assert.Equal(2, timeline.DisplayedNotes.Count);

            timeline.RemoveTickerFilterCommand.Execute("7203.T");
            await timeline.RefreshAsync();

            Assert.Equal(new[] { "9984.T" }, timeline.SelectedTickerCodes);
            Assert.Single(timeline.DisplayedNotes);
            Assert.Equal("b", timeline.DisplayedNotes[0].Note.Body);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// SelectedScopeNode and clears previously-active Ticker chips.</summary>
    [Fact]
    public async Task SelectSuggestion_WatchlistNode_SetsSelectedScopeNode_AndClearsPriorTickerSelection()
    {
        ...
        try
        {
            ...
            var (timeline, _, _) = await CreateTimelineAsync(tempDir, watchlistManager);
            timeline.SelectedTickerCodes.Add("9984.T");
            ...
            Assert.Empty(timeline.SelectedTickerCodes);
            Assert.Null(timeline.FilterCriteria.RegisteredTagTickers);
        }
        finally
        {
    ...
    /// carrying that tag and resets the Ticker chips/scope selection.</summary>
    [Fact]
    ...
    /// <summary>Task A (SAで実装, Note Tab Enhancements): picking the "未分類" (NoTicker) sentinel
    /// suggestion filters the timeline to Notes with no RelatedTicker and clears a previously-active
    /// Ticker selection, same "only one active dimension at a time" contract as the other Kinds.</summary>
    [Fact]
    public async Task SelectSuggestion_NoTicker_FiltersToNotesWithNoRelatedTicker_AndClearsPriorTickerSelection()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            var (timeline, noteRepository, _) = await CreateTimelineAsync(tempDir);
            await noteRepository.CreateAsync(MakeNote("tickered", DateTime.Now, "7203.T"));
            await noteRepository.CreateAsync(MakeNote("untickered", DateTime.Now, null));
            timeline.SelectedTickerCodes.Add("7203.T");
            await timeline.RefreshAsync();

            timeline.SelectSuggestion(NoteScopeSuggestion.ForNoTickerSentinel());
            await timeline.RefreshAsync();

            Assert.Single(timeline.DisplayedNotes);
            Assert.Equal("untickered", timeline.DisplayedNotes[0].Note.Body);
            Assert.Empty(timeline.SelectedTickerCodes);
            Assert.True(timeline.FilterCriteria.NoTickerOnly);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// whose Hashtags contain that tag, and resets the Ticker chips/scope selection - the same
    ...
    /// <summary>Task E (SAで実装, Note Tab Enhancements): a [[note-image:{guid}]] placeholder token
    /// whose id is actually present in Note.AttachmentIds must become its own AttachedImageId
    /// segment, split out before URL/hashtag tokenization sees the surrounding text.</summary>
    [Fact]
    public void BodySegments_ImageTokenWithMatchingAttachmentId_YieldsAttachedImageIdSegment()
    {
        var attachmentId = Guid.NewGuid();
        var note = MakeNote($"見て{NoteImageTokenExtractor.Build(attachmentId)}良い", DateTime.Now)
            with { AttachmentIds = ImmutableArray.Create(attachmentId) };
        var item = new NoteTimelineItemViewModel(note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback);

        var segments = item.BodySegments;

        Assert.Contains(segments, s => s.AttachedImageId == attachmentId);
        Assert.Contains(segments, s => s.Text == "見て" && s.AttachedImageId == null);
        Assert.Contains(segments, s => s.Text == "良い" && s.AttachedImageId == null);
    }

    /// <summary>Task E, FR-07 defensive path: an image token whose id never made it into
    /// Note.AttachmentIds (a Save-time failure token that somehow survived) must be dropped
    /// entirely, not rendered as a broken/empty segment.</summary>
    [Fact]
    public void BodySegments_ImageTokenWithoutMatchingAttachmentId_IsDroppedEntirely()
    {
        var unsavedId = Guid.NewGuid();
        var note = MakeNote($"見て{NoteImageTokenExtractor.Build(unsavedId)}良い", DateTime.Now);
        var item = new NoteTimelineItemViewModel(note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback);

        var segments = item.BodySegments;

        Assert.DoesNotContain(segments, s => s.AttachedImageId != null);
        Assert.Equal("見て良い", string.Concat(segments.Select(s => s.Text)));
    }
    ...
    [Fact]
    public void Constructor_WithAttachmentsButNoAttachmentRepository_LeavesThumbnailsEmpty()
    {
        ...
        Assert.Empty(item.Thumbnails);
        Assert.Equal("\U0001F4CE 1", item.AttachmentCountText);
    }

    [Fact]
    public async Task Constructor_WithAttachmentIdThatDoesNotResolve_LeavesThumbnailsEmptyWithoutThrowing()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
    ...
    /// <summary>sa_implement (Note Thumbnail Limit): the constructor-snapshotted maxRenderedThumbnails
    /// value caps how many attachments LoadThumbnailsAsync actually decodes, not just the hardcoded
    /// default of 3 - here 5 attachments with a configured limit of 2 must load exactly 2.</summary>
    [AvaloniaFact]
    public async Task Constructor_WithCustomMaxRenderedThumbnails_CapsDecodedThumbnailCount()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            var note = MakeNote("body", DateTime.Now) with { AttachmentIds = attachmentIds.ToImmutableArray() };
            var item = new NoteTimelineItemViewModel(
                note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback,
                attachmentRepository, maxRenderedThumbnails: 2);

            await item.ThumbnailLoadTask!;

            Assert.Equal(2, item.Thumbnails.Count);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// <summary>sa_implement (Note Thumbnail Limit, Task 2 - clicking "+N" reveals the rest): the
    /// command must decode and append every attachment beyond MaxRenderedThumbnails into Thumbnails,
    /// and ExtraAttachmentCountText must go empty once done (nothing left to report as overflow).</summary>
    [AvaloniaFact]
    public async Task ExpandRemainingThumbnailsCommand_LoadsTheRest_AndClearsExtraAttachmentCountText()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            await item.ThumbnailLoadTask!;
            Assert.Equal(2, item.Thumbnails.Count);
            Assert.Equal("+3", item.ExtraAttachmentCountText);
            Assert.True(item.ExpandRemainingThumbnailsCommand.CanExecute(null));

            await item.ExpandRemainingThumbnailsCommand.ExecuteAsync(null);

            Assert.Equal(5, item.Thumbnails.Count);
            Assert.Equal(string.Empty, item.ExtraAttachmentCountText);
            Assert.False(item.ExpandRemainingThumbnailsCommand.CanExecute(null));
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// <summary>sa_implement (Note Thumbnail Limit, Task 2): with no overflow at all, the command must
    /// never become executable - there is nothing for it to reveal.</summary>
    [Fact]
    public void ExpandRemainingThumbnailsCommand_CannotExecute_WhenNoOverflowExists()
    {
    ...
    /// <summary>sa_improve (Inline Mode bottom thumbnail row sync): in Inline display mode,
    /// an attachment referenced by a [[note-image:{guid}]] token in Note.Body is rendered inline in the
    /// body and is excluded from the initial Bottom thumbnail row to prevent duplicate display.</summary>
    [AvaloniaFact]
    public async Task LoadThumbnailsAsync_InlineMode_AttachmentReferencedByInlineToken_IsExcludedFromThumbnailsInitially()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            var item = new NoteTimelineItemViewModel(
                note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback,
                attachmentRepository, imageDisplayMode: NoteImageDisplayMode.Inline);
    ...
    /// <summary>sa_minimal_fix (Inline Mode "+N" expand bug - reported: clicking "+" in Inline
    /// display mode does not reveal the remaining images): ExpandRemainingThumbnailsCommand decodes
    /// the overflow attachments into ThumbnailsByAttachmentId, but NoteCardView's code-behind
    /// (RebuildBody) only re-renders the body's inline images in response to a
    /// PropertyChanged(BodySegments) notification. The command must raise that notification once the
    /// newly-decoded images are ready, or the View never learns it needs to redraw even though the
    /// underlying data is already there.</summary>
    [AvaloniaFact]
    public async Task ExpandRemainingThumbnailsCommand_InlineMode_RaisesBodySegmentsChanged_SoTheViewRedraws()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            item.PropertyChanged += (_, e) => raisedPropertyNames.Add(e.PropertyName);

            await item.ExpandRemainingThumbnailsCommand.ExecuteAsync(null);

            Assert.Contains(nameof(NoteTimelineItemViewModel.BodySegments), raisedPropertyNames);
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    /// <summary>The same attachment must still appear in the Bottom row when Image Display Mode =
    /// AttachmentList - the fix must only suppress the duplicate while it's actually being rendered
    /// inline, not unconditionally whenever a body happens to contain a token (e.g. a Note attached
    /// while Inline was the active setting, viewed after switching the setting back).</summary>
    [AvaloniaFact]
    public async Task LoadThumbnailsAsync_AttachmentListMode_AttachmentReferencedByInlineToken_StillAppearsInThumbnails()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            var item = new NoteTimelineItemViewModel(
                note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback,
                attachmentRepository, imageDisplayMode: NoteImageDisplayMode.AttachmentList);
    ...
    /// <summary>sa_improve (Inline Mode bottom thumbnail row sync): in Inline display mode,
    /// initial inline-referenced attachments are excluded from Thumbnails (0 items), but clicking
    /// "+N" adds the overflow attachments into Thumbnails (1 item) so they render in the bottom row.</summary>
    [AvaloniaFact]
    public async Task LoadThumbnailsAsync_InlineMode_PopulatesThumbnailsAndExpandsRemaining()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            var item = new NoteTimelineItemViewModel(
                note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback,
                attachmentRepository, imageDisplayMode: NoteImageDisplayMode.Inline, maxRenderedThumbnails: 2);
    ...
    /// <summary>Task B (SAで実装, Note Tab Enhancements): clicking a decoded thumbnail's
    /// OpenAttachmentOriginalCommand relays that attachment's file path to the OS-open callback
    /// (fix request: confirm original upload size via the OS's own image viewer). AvaloniaFact
    /// (not plain Fact, unlike the rest of this file) - decoding a real Avalonia Bitmap requires
    /// the headless platform's bitmap loader, which only [AvaloniaFact] initializes.</summary>
    [AvaloniaFact]
    public async Task OpenAttachmentOriginal_InvokesCallback_WithTheAttachmentsFilePath()
    {
        var tempDir = CreateIsolatedTempDirectory();
        try
        {
            ...
            await schemaInitializer.InitializeAsync();
            var attachmentRepository = new AttachmentRepository(connectionManager, NullLogger<AttachmentRepository>.Instance);
            var saveResult = await attachmentRepository.SaveAsync(CreateTestPngBytes(), "original.png");
            Assert.True(saveResult.Success);

            var note = MakeNote("body", DateTime.Now) with { AttachmentIds = ImmutableArray.Create(saveResult.Attachment!.Id) };
            ...
            item.OpenAttachmentOriginalCommand.Execute(item.Thumbnails[0]);
    ...
    /// <summary>sa_minimal_fix (Note Tab Polish round, bug #5, 4th investigation pass): the previous
    /// fix (setting ProcessStartInfo.WorkingDirectory to the attachment's own folder) was disproven by
    /// the user's own JSON-structured diagnostic log - WorkingDirectory was confirmed correctly set,
    /// yet the identical System.ComponentModel.Win32Exception (error 1155, ERROR_NO_ASSOCIATION)
    private sealed class FakeExternalFileOpener : IExternalFileOpener
    {
    ...
    /// <summary>Tests that clicking an attachment thumbnail delegates to IExternalFileOpener to open the original file.</summary>
    [Fact]
    public void OpenAttachmentOriginal_DelegatesToExternalFileOpener()
    {
        var fakeOpener = new FakeExternalFileOpener();
        var item = new NoteTimelineItemViewModel(
            MakeNote("body", DateTime.Now),
            isExpanded: false,
            onTogglePinnedRequested: _ => Task.CompletedTask,
            onDeleteRequested: _ => Task.CompletedTask,
            onEditRequested: _ => { },
            onOpenUrlRequested: _ => { },
            onViewChartRequested: _ => { },
            onOpenDetailRequested: _ => { },
            onHashtagFilterRequested: _ => { },
            onHashtagSearchRequested: _ => { },
            attachmentRepository: null!,
            onOpenAttachmentRequested: path => fakeOpener.OpenFile(path));

        var thumbnail = new NoteThumbnailItem(null!, @"C:\test\img.png");
        item.OpenAttachmentOriginalCommand.Execute(thumbnail);

        Assert.Single(fakeOpener.OpenedFiles);
        Assert.Equal(@"C:\test\img.png", fakeOpener.OpenedFiles[0]);
    }

    /// <summary>Tests that clicking a URL in a note delegates to IExternalFileOpener to open the browser.</summary>
    [Fact]
    public void OpenUrl_DelegatesToExternalFileOpener()
    {
        var fakeOpener = new FakeExternalFileOpener();
        var item = new NoteTimelineItemViewModel(
            MakeNote("body", DateTime.Now),
            ...
            onDeleteRequested: _ => Task.CompletedTask,
            onEditRequested: _ => { },
            onOpenUrlRequested: url => fakeOpener.OpenUrl(url),
            onViewChartRequested: _ => { },
            onOpenDetailRequested: _ => { },
            onHashtagFilterRequested: _ => { },
            onHashtagSearchRequested: _ => { },
        ...
        item.OpenUrlCommand.Execute("https://example.com");

        Assert.Single(fakeOpener.OpenedUrls);
        Assert.Equal("https://example.com", fakeOpener.OpenedUrls[0]);
    }
    ...
    /// <summary>sa_implement (Note Thumbnail Limit): MaxRenderedThumbnails is now a ctor-snapshotted
    /// value (Settings &gt; Notes, defaulting to 3) rather than a hardcoded constant -
    /// ExtraAttachmentCountText must reflect whatever value was actually passed in.</summary>
    [Fact]
    public void ExtraAttachmentCountText_WithCustomMaxRenderedThumbnails_ReflectsTheConfiguredLimit()
    {
        var note = MakeNote("body", DateTime.Now) with
        {
            AttachmentIds = ImmutableArray.Create(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid())
        };
        var item = new NoteTimelineItemViewModel(
            note, false, NoOpAsyncCallback, NoOpAsyncCallback, NoOpItemCallback, NoOpUrlCallback, NoOpItemCallback, NoOpItemCallback, NoOpUrlCallback, NoOpUrlCallback,
            maxRenderedThumbnails: 5);
