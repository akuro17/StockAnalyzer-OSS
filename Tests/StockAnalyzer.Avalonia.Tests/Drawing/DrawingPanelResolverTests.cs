...
{
    [Fact]
    public void IsPanelPresent_MainAlwaysTrue_AbsentIndicatorFalse_MatchesTryGetPanelIndex()
    {
        var settings = new List<CoreIndicatorSettings>
        {
            new() { Id = "rsi-1", TypeEnum = IndicatorType.RSI, IsEnabled = true, IsOverlay = false }
        };
        var present = PanelKey.Indicator("rsi-1");
        var absent = PanelKey.Indicator("missing");

        Assert.True(DrawingPanelResolver.IsPanelPresent(PanelKey.Main, null, isSubWindowVisible: false));
        Assert.True(DrawingPanelResolver.IsPanelPresent(present, settings, isSubWindowVisible: true));
        Assert.False(DrawingPanelResolver.IsPanelPresent(absent, settings, isSubWindowVisible: true));
        Assert.False(DrawingPanelResolver.IsPanelPresent(present, settings, isSubWindowVisible: false));
        Assert.Equal(
            DrawingPanelResolver.TryGetPanelIndex(present, settings, true).HasValue,
            DrawingPanelResolver.IsPanelPresent(present, settings, true));
    }
