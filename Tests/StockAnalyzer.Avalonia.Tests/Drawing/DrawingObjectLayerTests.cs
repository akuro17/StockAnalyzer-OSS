...
public class DrawingObjectLayerTests
{
    ...
    [Fact]
    public void NotifyObjectChanged_FiresChangedAndSynced_ForCallerThatMutatedTheModelDirectly()
    {
        // Regression test: DrawingSettingsDialog writes Color/Thickness/panel-specific settings
        // straight onto the model, bypassing every mutator method above (ToggleLock, RenameObject,
        // etc.), so without an explicit notification neither persistence (Changed) nor the chart
        // canvas repaint (Synced -> ChartBaseControl.OnObjectManagerChanged -> InvalidateVisual)
        // ever fires for that kind of edit -- reported as "Apply doesn't visually update the chart".
        var manager = new ChartObjectManager();
        var obj = new MockChartObject();
        manager.AddObject(obj);
        ...
        int syncedCount = 0;
        manager.Changed += () => changedCount++;
        manager.Synced += () => syncedCount++;

        obj.Color = Colors.Red; // simulate the dialog's direct property write

        bool result = manager.NotifyObjectChanged(obj.Id);
    ...
    [Fact]
    public void NotifyObjectChanged_ReturnsFalse_ForUnknownOrEmptyId()
    {
        var manager = new ChartObjectManager();
        Assert.False(manager.NotifyObjectChanged(Guid.NewGuid()));
        Assert.False(manager.NotifyObjectChanged(Guid.Empty));
    }

    [Fact]
    public void HasExplicitMoveAxisMode_IsTrue_WhenObjectExplicitlySetBackToTheDefaultXYValue()
    {
        // Regression test: HasExplicitMoveAxisMode must distinguish "explicitly set" from "current
        // value happens to equal the default (XY)". Previously it only checked
        // GetMoveAxisMode(id) != XY, so an object explicitly cycled back to XY was indistinguishable
        // from one that was never touched -- ChartInteractionController then incorrectly fell back to
        // the chart-wide default axis mode instead of respecting the object's own explicit XY choice.
        var manager = new ChartObjectManager();
        var obj = new MockChartObject();
        manager.AddObject(obj);

        manager.SetMoveAxisMode(obj.Id, DrawingMoveAxisMode.X);
        Assert.True(manager.HasExplicitMoveAxisMode(obj.Id));

        manager.SetMoveAxisMode(obj.Id, DrawingMoveAxisMode.XY);

        Assert.Equal(DrawingMoveAxisMode.XY, manager.GetMoveAxisMode(obj.Id));
        Assert.True(manager.HasExplicitMoveAxisMode(obj.Id));
    }

    [Fact]
    public void HasExplicitMoveAxisMode_IsFalse_ForObjectNeverToldToSetMoveAxisMode()
    {
        var manager = new ChartObjectManager();
        var obj = new MockChartObject();
        manager.AddObject(obj);

        Assert.False(manager.HasExplicitMoveAxisMode(obj.Id));
    }
