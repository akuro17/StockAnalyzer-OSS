...
public class AboutSettingsViewModelTests
{
    ...
    public void AboutSettingsViewModel_ExposesVersionAndDisclaimerText()
    {
        ...
        Assert.True(vm.EnvironmentStatusLines.Count >= 10); // Python line + 4 packages + ONNX runtime + 3 model files + training tools
        ...
        Assert.DoesNotContain(vm.EnvironmentStatusLines, line => line.Contains("scikit-learn") || line.StartsWith("arch") || line.Contains("pandas-ta"));
    ...
    public async Task EnvironmentStatusLines_AfterInspection_ReflectRealVersions()
    {
        var snapshot = new PythonEnvironmentSnapshot(
            ...
            PackageVersions: new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase)
            {
                ["yfinance"] = "0.2.99",
                ["pandas"] = "2.9.9",
                ["polars"] = null,        // queried but not installed
                ["torch"] = "2.5.1",
                ["lightgbm"] = "4.5.0",
        ...
        Assert.Contains(vm.EnvironmentStatusLines, line => line == "Polars (Fast Parquet): Not Installed");
    ...
    [Fact]
    public async Task RefreshEnvironmentStatusAsync_AfterTrainingToolsAreInstalled_ShowsThemWithoutRecreatingThePage()
    {
        var installed = new Dictionary<string, string?>(StringComparer.OrdinalIgnoreCase);
        PythonEnvironmentSnapshot Snapshot() => new(true, "3.13.7", new Dictionary<string, string?>(installed, StringComparer.OrdinalIgnoreCase));
        ...
        await vm.RefreshEnvironmentStatusAsync();

        var training = Assert.Single(vm.EnvironmentStatusLines.Where(l => l.StartsWith("ONNX Model Training Tools")));
        Assert.Contains("Configured", training);
        Assert.Contains("PyTorch v2.14.0+cpu", training);
        Assert.Contains("LightGBM v4.7.0", training);
    }

    [Fact]
    public async Task RefreshEnvironmentStatusAsync_WhileProbeIsRunning_ReusesTheRunningProbe()
    {
        int probes = 0;
        var gate = new TaskCompletionSource<PythonEnvironmentSnapshot>();
        var vm = new AboutSettingsViewModel(new FakeInspector(_ =>
        {
            Interlocked.Increment(ref probes);
            return gate.Task;
        }));

        var first = vm.RefreshEnvironmentStatusAsync();
        var second = vm.RefreshEnvironmentStatusAsync();
