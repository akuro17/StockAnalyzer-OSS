...
public class DrawingSettingsViewModelTests
{
    ...
    [Fact]
    public void ViewModel_InitializesWithDefaultLinkMode()
    {
        var settingsManager = new MockChartSettingsManager();
        var vm = new DrawingSettingsViewModel(settingsManager);

        Assert.Equal(DrawingLinkMode.SnapToParentAnchorPoint, vm.DrawingLinkMode);
        Assert.False(vm.IsModified);
    }

    [Fact]
    public void ViewModel_IsModified_TracksLinkModeChanges()
    {
        var settingsManager = new MockChartSettingsManager();
        var vm = new DrawingSettingsViewModel(settingsManager);

        vm.DrawingLinkMode = DrawingLinkMode.PreserveRelativePosition;
        Assert.True(vm.IsModified);

        vm.DrawingLinkMode = DrawingLinkMode.SnapToParentAnchorPoint;
        Assert.False(vm.IsModified);
    }

    [Fact]
    public async Task SaveChangesAsync_PersistsLinkMode()
    {
        var settingsManager = new MockChartSettingsManager();
        var vm = new DrawingSettingsViewModel(settingsManager);
        ...
        Assert.Equal(DrawingLinkMode.PreserveRelativePosition, settingsManager.Current.DrawingLinkMode);
    }
}
