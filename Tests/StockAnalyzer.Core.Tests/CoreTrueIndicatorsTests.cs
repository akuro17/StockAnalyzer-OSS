...
public class CoreTrueIndicatorsTests
{
    ...
    public void TrueIndicators_CalculateSeries_ProducesValidOutput()
    {
        ...
        Assert.Null(tr.Values[0]); // Bar 0 has no preceding price -> null (Strict specification)
        Assert.Equal(10m, tr.Values[1]); // |110 - 100|
        Assert.Equal(5m, tr.Values[2]);  // |105 - 110|
        Assert.Equal(15m, tr.Values[3]); // |120 - 105|

        // Test null propagation
        var seriesWithNull = new List<decimal?> { 100m, null, 120m };
        var resNullTr = tr.CalculateSeries(seriesWithNull);
        Assert.True(resNullTr.IsSuccessful);
        Assert.Equal(3, tr.Values.Count);
        Assert.Null(tr.Values[0]); // Bar 0
        Assert.Null(tr.Values[1]); // current is null
        Assert.Null(tr.Values[2]); // prev was null
    }
}
