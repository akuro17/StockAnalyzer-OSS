...
namespace StockAnalyzer.Core.Tests
{
    public class CoreMesaIndicatorTests
    {
        ...
        {
            var indicator = new CoreMesaIndicator();
            var mockPythonService = new MesaMockPythonService();
        ...
        public async Task CalculateAsync_WithValidData_ReturnsSuccessAndValues()
        {
            ...
            Assert.Equal(_testData.Count, indicator.Fama.Count);
        ...
        private class RealPythonTestSettings : IStockAnalyzerSettings
        {
            ...
            public string PipeName => "testmesapipe";
        ...
        private class MesaMockPythonService : MockPythonServiceBase
        {
            public override Task<string> CalculateMesaAsync(decimal fastLimit = 0.5m, decimal slowLimit = 0.05m)
            {
                // Generate dummy response matching input size (20)
                var mama = string.Join(",", Enumerable.Repeat("100.5", 20));
                var fama = string.Join(",", Enumerable.Repeat("100.2", 20));
                return Task.FromResult($"{{\"status\":\"ok\",\"result\":{{\"mama\":[{mama}],\"fama\":[{fama}]}}}}");
            }
        }
    }
}
