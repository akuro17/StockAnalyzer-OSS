...
public class HilbertDecompositionEngineTests
{
    ...
    [Fact]
    public void Decompose_NormalizedComponents_AlwaysWithinUnitInterval()
    {
        var prices = GenerateSinePrices(300, period: 20.0);
        var result = HilbertDecompositionEngine.Decompose(prices);

        for (int i = 0; i < result.Count; i++)
        {
            Assert.InRange(result[i].NormalizedInPhase, -1m, 1m);
            Assert.InRange(result[i].NormalizedQuadrature, -1m, 1m);
        }
    }
