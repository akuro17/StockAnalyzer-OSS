...
inference contract of StockAnalyzer.Core.Services.PredictionService:
    ...
    trend_predictor_regression.onnx  input "input" float32 [batch, 10, 5]
           ...
           regardless of input - PredictionServiceTests asserts SimpleReturnPercent against
           100*(exp(REGRESSION_FIXTURE_Y)-1) without having to replicate matrix math.
...
def main() -> None:
    fixtures = (
        ...
        ("trend_predictor_regression.onnx", _with_regression_contract()),
    )
