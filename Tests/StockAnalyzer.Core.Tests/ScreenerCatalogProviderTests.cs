...
public class ScreenerCatalogProviderTests
{
    ...
    [Fact]
    public void NormalizeOutputSeriesName_PrimaryDisplayAliasesResolveToMain()
    {
        var provider = new ScreenerCatalogProvider();

        Assert.Equal(IndicatorResult.MainSeriesName, provider.NormalizeOutputSeriesName(IndicatorType.SMA, "SMA"));
        Assert.Equal(IndicatorResult.MainSeriesName, provider.NormalizeOutputSeriesName(IndicatorType.RSI, "RSI"));
        Assert.Equal("Signal", provider.NormalizeOutputSeriesName(IndicatorType.MACD, "Signal"));
    }
