...
namespace StockAnalyzer.Core.Tests
{
    public class CoreConnorsRsiIndicatorTests
    {
        ...
        {
            // Open uses a different shape (not a parallel shift of Close) so that the
            // streak/RSI/ROC calculations, some of which are shift-invariant, genuinely differ.
            var candles = new List<CoreCandleData>();
            for (int i = 0; i < 110; i++)
            {
                decimal close = 100 + i % 20;
                decimal open = 50 + (i % 7) * 4;
                candles.Add(new CoreCandleData(new System.DateTime(2023, 1, 1).AddDays(i), open, close + 10, close - 10, close, 1000 + i * 10));
            }

            var closeBased = new CoreConnorsRsiIndicator { PercentRankPeriod = 100, PriceSource = PriceType.Close };
            var openBased = new CoreConnorsRsiIndicator { PercentRankPeriod = 100, PriceSource = PriceType.Open };
