...
public class TrainingConfigJsonTests
{
    ...
    [Fact]
    public void TrainingJobConfig_RoundTrip_IndicatorChannelExportPaths_PreservesSymbolToPathMap()
    {
        var original = new TrainingJobConfig
        {
            Symbols = new[] { "7203-T", "9984-T" },
            ...
            WindowSize = 60,
            Horizon = 5,
            IndicatorChannelExportPaths = new Dictionary<string, string>
            {
                ["7203-T"] = @"C:\Temp\sa_indicator_channels_run1_7203-T.parquet",
                ["9984-T"] = @"C:\Temp\sa_indicator_channels_run1_9984-T.parquet",
            },
        };
        ...
        // The wire key ("indicator_channel_export_paths") is the exact string
        // run_training.py's JobConfig.from_json reads via raw.get(...); dictionary keys (the
        // symbols) are never snake_cased by PropertyNamingPolicy, so they must round-trip verbatim.
        Assert.Contains("\"indicator_channel_export_paths\"", json);
        Assert.Equal(original.IndicatorChannelExportPaths, roundTripped.IndicatorChannelExportPaths);
    }

    [Fact]
    public void TrainingJobConfig_Serialize_NullIndicatorChannelExportPaths_OmittedFromWireDocument()
    {
        var config = new TrainingJobConfig { Symbols = new[] { "7203-T" }, Architecture = "lstm", WindowSize = 60, Horizon = 5 };

        var json = TrainingConfigJson.Serialize(config);

        Assert.DoesNotContain("\"indicator_channel_export_paths\"", json);
        Assert.Null(TrainingConfigJson.DeserializeConfig(json).IndicatorChannelExportPaths);
    }
