...
public class TrainingJobConfigTests
{
    ...
    [Fact]
    public void Validate_IndicatorChannelExportPathsSet_DoesNotThrow()
    {
        // IndicatorChannelExportPaths is an orchestrator-assigned field (set by
        // TrainingOrchestrator.StartTrainingAsync after IndicatorChannelExporter.ExportAsync runs,
        // never by the Wizard UI), modeled on RunId's own "no Validate() check" treatment -- it is
        // not cross-checked against FeatureSpec here by design.
        var config = Valid() with
        {
            IndicatorChannelExportPaths = new Dictionary<string, string> { ["7203-T"] = "/tmp/sa_indicator_channels_run1_7203-T.parquet" },
        };
