...
public class FeatureSpecJsonTests
{
    ...
    private static List<FeatureChannel> RepeatedPriceChannels(int count)
    {
        var channels = new List<FeatureChannel>(count);
    ...
    [Fact]
    public void IsValid_ChannelCountAtMaxChannels_ReturnsTrue()
    {
        var spec = new FeatureSpec { Channels = RepeatedPriceChannels(FeatureSpec.MaxChannels) };

        Assert.True(spec.IsValid(out var error));
        Assert.Null(error);
    }

    [Fact]
    public void IsValid_ChannelCountOverMaxChannels_ReturnsFalse()
    {
        var spec = new FeatureSpec { Channels = RepeatedPriceChannels(FeatureSpec.MaxChannels + 1) };

        Assert.False(spec.IsValid(out var error));
        Assert.Contains("MaxChannels", error);
    }
