...
public class ParameterMetadataCoverageTests
{
    ...
    public void ParameterMetadataAudit_ReportAllMissingMetadata()
    {
        ...
        foreach (var type in types)
        {
            ...
            // Check dynamic period tag
            var periodProp = type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .FirstOrDefault(p => string.Equals(p.Name, "Period", StringComparison.OrdinalIgnoreCase) && p.PropertyType == typeof(int));
            if (periodProp != null)
            {
                var tagAttrs = periodProp.GetCustomAttributes<ParameterTagAttribute>();
                if (!tagAttrs.Any(t => t.Tag == ParameterTags.DynamicPeriodSensitive))
                {
                    missingList.Add($"[Missing DynamicPeriodSensitive Tag] {type.Name}.{periodProp.Name}");
                }
            }
        }
