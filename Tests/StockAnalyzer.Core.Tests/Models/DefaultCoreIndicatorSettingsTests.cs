...
public class DefaultCoreIndicatorSettingsTests
{
    ...
    public void GetDefault_ShouldReturnExpectedNumberOfIndicators()
    {
        ...
        // 67 base + 3 Polar Coordinate Transform Decomposition indicators
        // + 1 Autocorrelation + 1 EfficiencyRatio + 1 DMI + 1 FDI + 1 TimeAtPrice.
        Assert.Equal(75, defaults.Count);
    }
    ...
    [Theory]
    [InlineData(IndicatorType.PolarAmplitudeRatio)]
    [InlineData(IndicatorType.PolarPhase)]
    public void GetDefault_PolarIndicators_AreDisabledUntilExplicitlyEnabled(IndicatorType type)
    {
        var setting = DefaultCoreIndicatorSettings.GetDefault()
            .Single(s => s.TypeEnum == type);
...
#pragma warning restore CS0618
    ...
    [Fact]
    public void GetDefault_PolarCycleAngularFrequencyDefaults_ShouldExposeMainPlusTwoDiagnosticSeries()
    {
        var polar = DefaultCoreIndicatorSettings.GetDefault()
            .First(s => s.TypeEnum == IndicatorType.PolarCycleAngularFrequency);

        Assert.IsType<CorePolarCycleAngularFrequencyParameter>(polar.ParameterObject);
        Assert.False(polar.IsOverlay);
        // Main (cycle angular frequency) + PhaseAngularVelocity + PhaseFrequencyError diagnostics,
        // all on the same rad/bar sub-panel (no fixed Y range).
        Assert.Null(polar.MinValue);
        Assert.Null(polar.MaxValue);
        Assert.Equal(3, polar.SeriesColors.Count);
        Assert.Contains(polar.SeriesColors, c => c.TargetSeries.Contains("Main") && c.DisplayName == "Polar Cycle Angular Frequency");
        Assert.Contains(polar.SeriesColors, c => c.TargetSeries.Contains("PhaseAngularVelocity") && c.DisplayName == "Phase Angular Velocity");
        Assert.Contains(polar.SeriesColors, c => c.TargetSeries.Contains("PhaseFrequencyError") && c.DisplayName == "Phase Frequency Error");
    }
    ...
    [Fact]
    public void CoreIndicatorSettings_UpdateDisplayName_RespectsUseShortNameAndIncludesPeriod()
    {
        var settings = new CoreIndicatorSettings
        {
        ...
        settings.UpdateDisplayName();
        Assert.Equal("Simple Moving Average (25)", settings.DisplayName);
        Assert.Equal("SMA(25)", settings.ShortDisplayName);

        settings.UseShortName = true;
        Assert.Equal("SMA(25)", settings.DisplayName);
    }

    [Fact]
    public void GetDefault_EfficiencyRatio_ShouldHaveExpectedDefaults()
    {
        var er = DefaultCoreIndicatorSettings.GetDefault()
            .First(s => s.TypeEnum == IndicatorType.EfficiencyRatio);

        Assert.Equal(CoreIndicatorCategory.Oscillator, er.Category);
        Assert.False(er.IsOverlay);
        Assert.Equal(IndicatorDefaultConstants.EfficiencyRatioMinValue, er.MinValue);
        Assert.Equal(IndicatorDefaultConstants.EfficiencyRatioMaxValue, er.MaxValue);
        Assert.IsType<CoreEfficiencyRatioParameter>(er.ParameterObject);
        var param = (CoreEfficiencyRatioParameter)er.ParameterObject!;
        Assert.Equal(IndicatorDefaultConstants.EfficiencyRatioPeriod, param.Period);
    }

    [Fact]
    public void GetDefault_DmiDefaults_ShouldMatchConstants()
    {
        var dmi = DefaultCoreIndicatorSettings.GetDefault()
            .First(s => s.TypeEnum == IndicatorType.DMI);

        Assert.Equal(CoreIndicatorCategory.Trend, dmi.Category);
        Assert.False(dmi.IsOverlay);
        var param = Assert.IsType<CoreDmiParameter>(dmi.ParameterObject);
        Assert.Equal(IndicatorDefaultConstants.DmiDefaultPeriod, param.Period);
        Assert.Equal(IndicatorDefaultConstants.DmiPlusDiColor.R, dmi.Color.R);
        Assert.Equal(IndicatorDefaultConstants.DmiPlusDiColor.G, dmi.Color.G);
        Assert.Equal(IndicatorDefaultConstants.DmiPlusDiColor.B, dmi.Color.B);

        Assert.NotNull(dmi.SeriesColors);
        Assert.Equal(2, dmi.SeriesColors.Count);

        var plusDi = dmi.SeriesColors[0];
        Assert.Equal("PlusDI", plusDi.Name);
        Assert.Equal("+DI", plusDi.DisplayName);
        Assert.Equal(IndicatorDefaultConstants.DmiPlusDiColor.R, plusDi.Color.R);
        Assert.Contains("PlusDI", plusDi.TargetSeries);

        var minusDi = dmi.SeriesColors[1];
        Assert.Equal("MinusDI", minusDi.Name);
        Assert.Equal("-DI", minusDi.DisplayName);
        Assert.Equal(IndicatorDefaultConstants.DmiMinusDiColor.R, minusDi.Color.R);
        Assert.Contains("MinusDI", minusDi.TargetSeries);
    }

    [Fact]
    public void GetDefault_TimeAtPriceDefaults_ShouldMatchConstants()
    {
        var tap = DefaultCoreIndicatorSettings.GetDefault()
            .First(s => s.TypeEnum == IndicatorType.TimeAtPrice);

        var param = Assert.IsType<CoreTimeAtPriceParameter>(tap.ParameterObject);
        Assert.Equal(IndicatorDefaultConstants.VolumeProfilePeriod, param.Period);
        Assert.Equal(IndicatorDefaultConstants.VolumeProfileRowCount, param.RowCount);
        Assert.Equal(PriceType.Close, param.PriceType);
        Assert.True(tap.IsOverlay);
        Assert.Equal(CoreIndicatorCategory.Chart, tap.Category);
    }
}
