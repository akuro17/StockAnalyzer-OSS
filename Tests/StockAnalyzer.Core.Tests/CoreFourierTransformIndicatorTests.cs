...
namespace StockAnalyzer.Core.Tests
{
    ...
    public class CoreFourierTransformIndicatorTests
    {
        ...
        public CoreFourierTransformIndicatorTests()
        {
        ...
        {
            var indicator = new CoreFourierTransformIndicator();
            var mockPythonService = new FourierTransformMockPythonService();
        ...
        {
            var settings = new RealPythonTestSettings();
            ...
            var indicator = new CoreFourierTransformIndicator { TargetPeriod = 4 };
        ...
        private class RealPythonTestSettings : IStockAnalyzerSettings
        {
            ...
            public string PipeName => "testfouriertransformindicatorpipe";
        ...
        private class FourierTransformMockPythonService : MockPythonServiceBase
        {
            public override Task<string> CalculateFourierTransformAsync(int targetPeriod = ChartConstants.FourierTransformDefaultTargetPeriod)
            {
                // Generate dummy response matching input size (20): first bar null (unfilled window), rest = 5.0
                var values = new List<string> { "null" };
                values.AddRange(Enumerable.Repeat("5.0", 19));
                var amplitude = string.Join(",", values);
                return Task.FromResult($"{{\"status\":\"ok\",\"result\":{{\"amplitude\":[{amplitude}]}}}}");
            }
        }
    }
}
