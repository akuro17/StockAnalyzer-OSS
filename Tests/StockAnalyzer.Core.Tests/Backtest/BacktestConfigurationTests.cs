...
public class BacktestConfigurationTests
{
    ...
    [Fact]
    public void InitialCapital_Omitted_ThrowsOnValidate()
    {
        // The init accessor only runs when the property is assigned; an omitted InitialCapital silently stays 0 and must still be rejected.
        ...
        ArgumentOutOfRangeException ex = Assert.Throws<ArgumentOutOfRangeException>(() => config.Validate());
        Assert.Equal(nameof(BacktestConfiguration.InitialCapital), ex.ParamName);
    }

    [Fact]
    public void InitialCapital_Omitted_DoesNotMaskEarlierValidationFailures()
    {
        // Previously-validated failures keep their exception and order: margin ordering is still reported first.
        var config = new BacktestConfiguration
        {
            InitialMarginRatio = 0.20m,
        ...
        ArgumentOutOfRangeException ex = Assert.Throws<ArgumentOutOfRangeException>(() => config.Validate());
        Assert.Equal(nameof(BacktestConfiguration.MaintenanceMarginRatio), ex.ParamName);
    }
