...
public class BacktestInputTests
{
    ...
    [Fact]
    public void AdditionalTimeframeBars_OmittedByExistingCallSite_DefaultsToNull()
    {
        // Every pre-existing call site (production and this test file's own Build helper) omits the
        // 2026-09-18 trailing optional parameter — this is the "existing behavior preservation" proof
        // for that safe extension (plan section 3.2.4 / Task 2a).
        var input = Build(ThreeValidBars());
        Assert.Null(input.AdditionalTimeframeBars);
    }

    [Fact]
    public void AdditionalTimeframeBars_WhenSupplied_RoundTrips()
    {
        var weeklyBars = ImmutableArray.Create(MakeBar(Bar0, 100m, 105m, 95m, 102m));
        var additional = new Dictionary<TimeFrame, ImmutableArray<CandleData>> { [TimeFrame.W1] = weeklyBars };

        var input = new BacktestInput(
            ThreeValidBars(), "AAPL", TimeFrame.D1, BacktestInput.CurrentDataVersion,
            Bar0, Bar0.AddDays(30), 0, 0, additional);

        Assert.NotNull(input.AdditionalTimeframeBars);
        Assert.Equal(additional[TimeFrame.W1], input.AdditionalTimeframeBars![TimeFrame.W1]);
    }

    [Fact]
    public void AdditionalTimeframeBars_SourceDictionaryMutation_DoesNotChangeInput()
    {
        var weeklyBars = ImmutableArray.Create(MakeBar(Bar0, 100m, 105m, 95m, 102m));
        var additional = new Dictionary<TimeFrame, ImmutableArray<CandleData>> { [TimeFrame.W1] = weeklyBars };
        var input = new BacktestInput(
        ...
        additional[TimeFrame.W1] = ImmutableArray<CandleData>.Empty;
        additional.Clear();

        Assert.Equal(weeklyBars, input.AdditionalTimeframeBars![TimeFrame.W1]);
    }

    [Fact]
    public void AdditionalTimeframeBars_DefaultArray_Throws()
    {
        var additional = new Dictionary<TimeFrame, ImmutableArray<CandleData>> { [TimeFrame.W1] = default };
        Assert.Throws<ArgumentException>(() => new BacktestInput(
            ThreeValidBars(), "AAPL", TimeFrame.D1, BacktestInput.CurrentDataVersion,
            Bar0, Bar0.AddDays(30), 0, 0, additional));
    }

    [Fact]
    public void AdditionalTimeframeBars_Unsorted_Throws()
    {
        var additional = new Dictionary<TimeFrame, ImmutableArray<CandleData>>
        {
            [TimeFrame.W1] = ImmutableArray.Create(
                MakeBar(Bar0.AddDays(1), 100m, 105m, 95m, 102m),
                MakeBar(Bar0, 100m, 105m, 95m, 102m)),
        };
        Assert.Throws<ArgumentException>(() => new BacktestInput(
    ...
    [Fact]
    public void AdditionalTimeframeBars_NegativeVolume_Throws()
    {
        var additional = new Dictionary<TimeFrame, ImmutableArray<CandleData>>
        {
            [TimeFrame.W1] = ImmutableArray.Create(MakeBar(Bar0, 100m, 105m, 95m, 102m, -1)),
        };
        Assert.Throws<ArgumentException>(() => new BacktestInput(
            ThreeValidBars(), "AAPL", TimeFrame.D1, BacktestInput.CurrentDataVersion,
            Bar0, Bar0.AddDays(30), 0, 0, additional));
