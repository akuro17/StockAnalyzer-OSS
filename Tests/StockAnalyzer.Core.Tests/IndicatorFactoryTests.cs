...
#pragma warning disable CS0618 // Deliberately testing the deprecated (Obsolete) enum member
    ...
    [Fact]
    public void Create_WithObsoleteAma_ShouldReturnNullNotThrow()
    {
        // AMA is a duplicate of KAMA and de-registered from the factory,
        // but the enum member is kept (Obsolete) for ordinal stability.
        IIndicatorFactory factory = new IndicatorFactory();

        Assert.False(factory.IsRegistered(IndicatorType.AMA));
        Assert.Null(factory.Create(IndicatorType.AMA));
    }
