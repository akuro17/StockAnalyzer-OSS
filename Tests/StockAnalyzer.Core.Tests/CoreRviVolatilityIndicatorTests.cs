...
namespace StockAnalyzer.Core.Tests
{
    public class CoreRviVolatilityIndicatorTests
    {
        ...
        {
            // Open uses a different shape (not a parallel shift of Close) so that
            // standard-deviation-based statistics, which are shift-invariant, genuinely differ.
            var candles = new List<CoreCandleData>();
            for (int i = 0; i < 40; i++)
            {
                decimal close = 100 + 10 * (decimal)System.Math.Sin(i / 5.0);
                decimal open = 100 + (i % 7) * 3;
                candles.Add(new CoreCandleData(new System.DateTime(2023, 1, 1).AddDays(i), open, close + 5, close - 5, close, 1000));
            }

            var closeBased = new CoreRviVolatilityIndicator { Period = 10, PriceSource = PriceType.Close };
            var openBased = new CoreRviVolatilityIndicator { Period = 10, PriceSource = PriceType.Open };
