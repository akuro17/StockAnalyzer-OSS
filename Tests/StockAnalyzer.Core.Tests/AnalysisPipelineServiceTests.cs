...
public class AnalysisPipelineServiceTests
{
    ...
    [Fact]
    public void CalculateIndicators_WithAutocorrelationDynamicPeriodDriver_AutomaticallyResolvesPeriodSeries()
    {
        var service = new AnalysisPipelineService(
            pythonService: null,
            indicatorFactory: IndicatorFactory.Default);
        ...
        var autocorrSetting = new CoreIndicatorSettings
        {
            Id = "AutoCorr1",
            IsEnabled = true,
        ...
        var drivenSma = new CoreIndicatorSettings
        {
            Id = "DrivenSMA",
            IsEnabled = true,
            TypeEnum = IndicatorType.SMA,
            ParameterObject = new CoreSmaParameter { Period = 10 },
            DynamicPeriodIndicatorId = "AutoCorr1"
            // OutputSeriesName is null/empty, should automatically resolve to "Period"
        };

        var settings = new List<CoreIndicatorSettings> { autocorrSetting, drivenSma };

        var result = service.CalculateIndicators(candles, settings);

        Assert.True(result.ContainsKey("AutoCorr1"));
        Assert.True(result["AutoCorr1"].IsSuccessful);
        Assert.True(result.ContainsKey("DrivenSMA"));
        Assert.True(result["DrivenSMA"].IsSuccessful);
        Assert.Equal(count, result["DrivenSMA"].MainValues.Count);
    }

    [Fact]
    public void CalculateIndicators_WithParameterPriceSource_SynchronizesSettingAndCalculatesCorrectly()
    {
        var service = new AnalysisPipelineService(
            pythonService: null,
            indicatorFactory: IndicatorFactory.Default);
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            decimal high = close + 10m + (decimal)Math.Cos(i * 1.2) * 4m;
        ...
        var setting = new CoreIndicatorSettings
        {
            ...
            IsEnabled = true,
            TypeEnum = IndicatorType.Autocorrelation,
            PriceSource = PriceType.Close, // initially Close
            ParameterObject = new CoreAutocorrelationParameter
            {
                Period = 20,
                MinLag = 5,
                MaxLag = 25,
                PriceSource = PriceType.High // Parameter set to High
            }
        };

        var result = service.CalculateIndicators(candles, new[] { setting });

        Assert.True(result["AutoCorrHigh"].IsSuccessful);
        Assert.Equal(PriceType.High, setting.PriceSource); // Setting should be synchronized to High
    }

    [Fact]
    public void CalculateIndicators_IchimokuChikouSpanAsDynamicPeriodDriver_FallsBackToClose_NoLookAhead()
    {
        // Ichimoku's ChikouSpan carries the FUTURE close (Close[t + Displacement]) at bar t. Used as an adaptive-period driver it would leak future data,
        // so the pipeline must substitute the current Close - the result must equal the one driven by a plain Close series.
        var service = new AnalysisPipelineService(pythonService: null, indicatorFactory: IndicatorFactory.Default);
        ...
        for (int i = 0; i < count; i++)
        {
            decimal close = 100m + (decimal)Math.Sin(i * 0.3) * 8m + i * 0.1m;
            candles.Add(new CoreCandleData(startDate.AddDays(i), close, close + 1m, close - 1m, close, 1000L));
        }

        CoreIndicatorSettings Driven(string id, string driverId) => new()
        {
            Id = id,
            IsEnabled = true,
            TypeEnum = IndicatorType.SMA,
            ParameterObject = new CoreSmaParameter { Period = 10 },
            DynamicPeriodIndicatorId = driverId,
        };

        var chikouDriver = new CoreIndicatorSettings
        {
            Id = "Ichi",
            IsEnabled = true,
            TypeEnum = IndicatorType.Ichimoku,
            ParameterObject = new CoreIchimokuParameter(),
            OutputSeriesName = StockAnalyzer.Core.Models.Indicators.Trend.CoreIchimokuIndicator.ChikouSpanSeriesName,
        };
        var closeDriver = new CoreIndicatorSettings { Id = "CloseDriver", IsEnabled = true, TypeEnum = IndicatorType.Price, PriceSource = PriceType.Close };

        var viaChikou = service.CalculateIndicators(candles, new[] { chikouDriver, Driven("A", "Ichi") });
        var viaClose = service.CalculateIndicators(candles, new[] { closeDriver, Driven("B", "CloseDriver") });

        Assert.True(viaChikou["A"].IsSuccessful);
        Assert.True(viaClose["B"].IsSuccessful);
        Assert.Equal(viaClose["B"].MainValues, viaChikou["A"].MainValues);
    }
}
