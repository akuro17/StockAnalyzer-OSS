...
public class DuckDBTest : System.IDisposable
{
    private readonly DuckDBConnectionManager _dbManager;
    private readonly ParquetMarketDataProvider _provider;

    // Self-contained fixture directory (never the developer's local Data/TestMarketData, which
    // is gitignored and not guaranteed to be populated on every machine or CI runner).
    private readonly string _fixtureDir;

    public DuckDBTest()
    {
        _dbManager = new DuckDBConnectionManager(NullLogger<DuckDBConnectionManager>.Instance);
        _fixtureDir = Path.Combine(Path.GetTempPath(), "sa_duckdb_fixture_" + System.Guid.NewGuid());
        Directory.CreateDirectory(_fixtureDir);
        ParquetFixtureBuilder.SeedDailyOhlcv(_fixtureDir, "MSFT");
        ParquetFixtureBuilder.SeedDailyOhlcv(_fixtureDir, "AAPL");
        ParquetFixtureBuilder.SeedDailyOhlcv(_fixtureDir, "GOOG");

        var settings = Options.Create(new MarketDataSettings
        {
            DailyDataPath = _fixtureDir
        });
    ...
    {
        _dbManager?.Dispose();
        try { Directory.Delete(_fixtureDir, true); } catch { /* best-effort cleanup */ }
    }
    ...
    {
        var parquetPath = Path.Combine(_fixtureDir, "MSFT.parquet");
    ...
    {
        var settings = Options.Create(new MarketDataSettings { DailyDataPath = _fixtureDir });
    ...
    {
        var settings = Options.Create(new MarketDataSettings { DailyDataPath = _fixtureDir, TickerListPath = null });
        using var dbManager = new DuckDBConnectionManager(NullLogger<DuckDBConnectionManager>.Instance);
        var provider = new ParquetMarketDataProvider(dbManager, new MockPythonServiceBase(), settings, NullLogger<ParquetMarketDataProvider>.Instance);
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public async Task DeleteLatestTickerDataAsync_WhenCountZeroOrNegative_ReturnsZeroWithoutModifyingData(int count)
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "sa_delete_latest_zero_" + System.Guid.NewGuid());
        Directory.CreateDirectory(tempDir);
        try
        {
            var fixturePath = Path.Combine(tempDir, "ZTEST.parquet").Replace("\\", "/");
            ...
            var deletedCount = await provider.DeleteLatestTickerDataAsync("ZTEST", count);
    ...
    [Fact]
    public async Task DeleteLatestTickerDataAsync_RemovesSpecifiedNumberOfLatestRows()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "sa_delete_latest_count_" + System.Guid.NewGuid());
        Directory.CreateDirectory(tempDir);
        try
        {
            var fixturePath = Path.Combine(tempDir, "ZTEST.parquet").Replace("\\", "/");
            ...
            // Delete latest 2 bars (2024-01-05 and 2024-01-04)
            var deletedCount = await provider.DeleteLatestTickerDataAsync("ZTEST", 2);
            ...
            Assert.Contains(remaining, c => c.Timestamp.Date == new System.DateTime(2024, 1, 1));
            Assert.Contains(remaining, c => c.Timestamp.Date == new System.DateTime(2024, 1, 2));
            Assert.Contains(remaining, c => c.Timestamp.Date == new System.DateTime(2024, 1, 3));
            Assert.DoesNotContain(remaining, c => c.Timestamp.Date == new System.DateTime(2024, 1, 4));
            Assert.DoesNotContain(remaining, c => c.Timestamp.Date == new System.DateTime(2024, 1, 5));
        }
        finally
        {
            try { Directory.Delete(tempDir, true); } catch { /* best-effort cleanup */ }
    ...
    [Fact]
    public async Task DeleteLatestTickerDataAsync_WhenFileMissing_IsSafeNoOp()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "sa_delete_latest_missing_" + System.Guid.NewGuid());
        Directory.CreateDirectory(tempDir);
        try
        {
            var settings = Options.Create(new MarketDataSettings { DailyDataPath = tempDir });
            ...
            var deletedCount = await provider.DeleteLatestTickerDataAsync("NONEXISTENT", 5);
            Assert.Equal(0, deletedCount);
        }
        finally
        {
