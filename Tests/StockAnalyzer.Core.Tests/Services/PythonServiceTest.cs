...
namespace StockAnalyzer.Core.Tests.Services
{
    ...
    public class PythonServiceTest : IAsyncLifetime
    {
        ...
        [Fact]
        public void SyncSessionConfig_DefaultAndCustomValues_ShouldHoldProperties()
        {
            var defaultConfig = new SyncSessionConfig(
                IsTimeSeriesSyncEnabled: true,
                IsMetadataSyncEnabled: true,
                IsAutoSyncEnabled: false,
                IsFullHistoryEnabled: false,
                DelayMinSeconds: 3.0m,
                DelayMaxSeconds: 5.0m,
                StartSyncPeriodYears: 5
            );

            Assert.False(defaultConfig.IsImputeMissingMetadataEnabled);
            Assert.False(defaultConfig.IsForcePeriodDownloadEnabled);

            var customConfig = defaultConfig with
            {
                IsImputeMissingMetadataEnabled = true
            };

            Assert.True(customConfig.IsImputeMissingMetadataEnabled);
            Assert.True(customConfig.IsMetadataSyncEnabled);
        }
    }
}
