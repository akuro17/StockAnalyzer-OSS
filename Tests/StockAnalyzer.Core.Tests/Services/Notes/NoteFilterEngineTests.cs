...
public class NoteFilterEngineTests
{
    ...
    public void Matches_TickerLessNote_ExcludedFromEachTickerScopedDimension(bool scope, bool tag, bool selected)
    {
        var criteria = new NoteFilterCriteria
        {
            ...
            SelectedTickers = selected ? ImmutableHashSet.Create("7203.T") : null,
        };
    ...
    public void Filter_AppliesMatchesAcrossCollection()
    {
        ...
        var criteria = new NoteFilterCriteria { SelectedTickers = ImmutableHashSet.Create("7203.T") };
    ...
    /// <summary>Task F (SAで実装, Note Tab Enhancements): multiple Ticker chips OR-match, same
    /// contract as ScopeTickers/RegisteredTagTickers - reuses NoteFilterEngine.MatchesTickerScope.</summary>
    [Fact]
    public void Matches_SelectedTickers_IsOrAcrossMultipleChips()
    {
        var criteria = new NoteFilterCriteria { SelectedTickers = ImmutableHashSet.Create("7203.T", "9984.T") };

        Assert.True(NoteFilterEngine.Matches(MakeNote("body", "7203.T"), criteria));
        Assert.True(NoteFilterEngine.Matches(MakeNote("body", "9984.T"), criteria));
        Assert.False(NoteFilterEngine.Matches(MakeNote("body", "6758.T"), criteria));
        Assert.False(NoteFilterEngine.Matches(MakeNote("body"), criteria));
    }
