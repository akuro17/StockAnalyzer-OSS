    ...
    public class MainWindowPage
    {
        ...
        public AutomationElement OpenAddIndicatorDialog()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                {
                    // The menu-bar "Tools" header intermittently reports an empty bounding
                    // rectangle (no clickable point) while the previous strategy's popup is
                    // still collapsing. Without this catch the exception aborts the whole
                    // method and the keyboard strategy (i == 2, needs no clickable point)
                    // is never reached. Fall through so the loop tries the next strategy.
                    _testBase.LogInfo($"Strategy {i + 1} could not point-click the Tools menu ({ex.Message}); falling through to next strategy.");
                }
